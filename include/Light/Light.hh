...
// be used at all. And for lights which have only one color, there is a
// reference variable 'color' which points to the warm color. This can be used
// without having to worry about warm and cool colors Every light also has a
// position (although it might be meaningless for directional lights) and a
// boolean variable which indicates active/inactive state of light
...
public:
  // Default constructor
  Light()
      : BaseObject(), position(0, 0, 0), warmcolor(1, 1, 1), coolcolor(1, 1, 1),
        color(warmcolor), intensity(1), state(true) {}

  // 1-arg constructor
  Light(const Vector3d &pos)
      : BaseObject(), position(pos), warmcolor(1, 1, 1), coolcolor(1, 1, 1),
        color(warmcolor), intensity(1), state(true) {}

  // 1-arg constructor. specifies color
  Light(const RGBColor &col)
      : BaseObject(), position(0, 0, 0), warmcolor(col), coolcolor(col),
        color(warmcolor), intensity(1), state(true) {}

  // Copy constructor
  Light(const Light &light)
      : BaseObject(light), position(light.position), warmcolor(light.warmcolor),
        coolcolor(light.coolcolor), color(warmcolor),
  ...
  Light &operator=(const Light &light) {
    ...
    warmcolor = light.warmcolor;
    coolcolor = light.coolcolor;
  ...
  // Compute the cosine factor for given point/normal. Meaningful only for some
  // lights
  virtual double cosfactor(const Vector3d &p, const Vector3d &n) const = 0;

  // Illuminate a given point with given normal using this light and return the
  // color
  virtual RGBColor illuminate(const Vector3d &p, const Vector3d &n) const = 0;

  // Illuminate a given point with given normal using this light and return the
  // color The eye position is also given to allow specular computations
  virtual RGBColor illuminate(const Vector3d &p, const Vector3d &n,
                              const Vector3d &e) const = 0;
};
