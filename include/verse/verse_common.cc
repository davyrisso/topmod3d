...
char *verse_client_name(void) {
  ...
  if (hostname)
    name_length += strlen(hostname);
#endif

  client_name =
      (char *)MEM_mallocN(sizeof(char) * name_length, "verse client name");
  client_name[0] = '\0';
 ...
 */
void post_node_create(VNode *vnode) { /*	allqueue(REDRAWOOPS, 0);*/ }
...
/*
 * this function is called, when connestion with verse server is
 * ended/terminated/etc.
 */
...
void post_connect_update(VerseSession *session) {
  if (session->flag & VERSE_CONNECTING) {
    ...
    if (session->counter > MAX_UNCONNECTED_EVENTS) {
      ...
      // 			/* popup menu*/
      // 			str =
      // malloc(sizeof(char)*(strlen(session->address)+35)); 			str[0]='\0';
