...
/* callback functions */
static void cb_g_layer_create(void *user_data, VNodeID node_id,
                              VLayerID layer_id, const char *name,
                              VNGLayerType type, uint32 def_integer,
                              real64 def_real);
static void cb_g_layer_destroy(void *user_data, VNodeID node_id,
                               VLayerID layer_id);
static void cb_g_vertex_set_xyz_real32(void *user_data, VNodeID node_id,
                                       VLayerID layer_id, uint32 vertex_id,
                                       real32 x, real32 y, real32 z);
static void cb_g_polygon_set_corner_uint32(void *user_data, VNodeID node_id,
                                           VLayerID layer_id, uint32 polygon_id,
                                           uint32 v0, uint32 v1, uint32 v2,
                                           uint32 v3);
static void cb_g_vertex_delete_real32(void *user_data, VNodeID node_id,
                                      uint32 vertex_id);
static void cb_g_polygon_delete(void *user_data, VNodeID node_id,
                                uint32 polygon_id);
static void cb_g_crease_set_edge(void *user_data, VNodeID node_id,
                                 const char *layer, uint32 def_crease);
static void cb_g_crease_set_vertex(void *user_data, VNodeID node_id,
                                   const char *layer, uint32 def_crease);

/* other static functions */

static VLayer *create_vlayer(struct VerseSession *session, VNodeID node_id,
                             VLayerID layer_id, const char *name,
                             VNGLayerType type, uint32 def_integer,
                             real64 def_real);

static uint8_item *get_face_smooth_item(struct VerseFace *vface);
static void free_unneeded_verseverts_of_verseface(struct VNode *vnode,
...
static VerseVert *find_verse_vert_in_queue(struct VLayer *vlayer,
                                           VNodeID node_id, VLayerID layer_id,
                                           uint32 vertex_id, real32 x, real32 y,
                                           real32 z);
static VerseFace *find_verse_face_in_queue(struct VLayer *vlayer,
                                           VNodeID node_id, VLayerID layer_id,
                                           uint32 polygon_id, uint32 v0,
                                           uint32 v1, uint32 v2, uint32 v3);

static unsigned short test_incoming_verseface(struct VGeomData *geom,
                                              struct VerseFace *vface);
...
static void move_face_orphan_to_dlist(struct VNode *vnode,
  ...
  while (vface) {
    ...
    /*		if(vface->vvert3) {
                            CalcNormFloat4(vface->vvert0->co, vface->vvert1->co,
                                            vface->vvert2->co,
       vface->vvert3->co, vface->no); VecAddf(vface->vvert3->no,
       vface->vvert3->no, vface->no);
                    }
                    else
                            CalcNormFloat(vface->vvert0->co, vface->vvert1->co,
                                            vface->vvert2->co, vface->no);
    */
    /* calculate vertex normals ... it is averadge of all face normals using the
     * vertex */
...
/*
 * move VerseFace from list of orphans to dlist of VerseFaces (if VerseFace was
 * only changed then this VerseFace is only removed from list of orphans)
 */
static void move_face_orphan_to_dlist(VNode *vnode, VLayer *vlayer,
                                      VerseFace *vface) {
 ...
 * at VerseVerts
 */
static unsigned short test_incoming_verseface(VGeomData *geom,
                                              VerseFace *vface) {
 ...
 */
static VerseFace *find_verse_face_in_queue(VLayer *vlayer, VNodeID node_id,
                                           VLayerID layer_id, uint32 polygon_id,
                                           uint32 v0, uint32 v1, uint32 v2,
                                           uint32 v3) {
  ...
  while (vface) {
    if ((vface->v0 == v0) && (vface->v1 == v1) && (vface->v2 == v2) &&
        (vface->v3 == v3)) {
      vface->id = polygon_id;
 ...
 */
static VerseVert *find_verse_vert_in_queue(VLayer *vlayer, VNodeID node_id,
                                           VLayerID layer_id, uint32 vertex_id,
                                           real32 x, real32 y, real32 z) {
  ...
  while (vvert) {
    if ((vvert->node_id == node_id) && (vvert->co[0] == x) &&
        (vvert->co[1] == y) && (vvert->co[2] == z)) {
      /* set layers */
      vvert->id = vertex_id;
...
static void send_verse_layer(VLayer *vlayer) {
  ...
  verse_send_g_layer_create(vlayer->node_id, vlayer->id, vlayer->name,
                            vlayer->type, vlayer->def_int, vlayer->def_real);
}
...
void send_verse_vertex(VerseVert *vvert) {
  ...
  verse_send_g_vertex_set_xyz_real32(vvert->node_id, vvert->layer_id, vvert->id,
                                     vvert->co[0], vvert->co[1], vvert->co[2]);
}
...
static uint8_item *get_face_smooth_item(VerseFace *vface) {
  ...
  vnode = (VNode *)TLI_dlist_find_link(&(session->nodes), vface->node_id);
  vlayer =
      find_verse_layer_type((VGeomData *)vnode->data, POLYGON_SMOOTH_LAYER);
  if (vlayer)
    return (uint8_item *)TLI_dlist_find_link(&(vlayer->dl), vface->id);
  else
    return NULL;
}
...
void send_verse_face_smoothness(VerseFace *vface, uint8 value) {
  ...
  verse_send_g_polygon_set_face_uint8(vface->node_id, smooth_layer->id,
                                      vface->id, value);
}
...
static void send_verse_face(VerseFace *vface) {
  ...
  if (vface->v3 != -1) {
    verse_send_g_polygon_set_corner_uint32(
        vface->node_id, vface->layer_id, vface->id, vface->v0,
        vface->v3,             /* verse use clock-wise winding */
        vface->v2, vface->v1); /* verse use clock-wise winding */
  } else {
    verse_send_g_polygon_set_corner_uint32(
        vface->node_id, vface->layer_id, vface->id, vface->v0,
 ...
 */
static void free_unneeded_verseverts_of_verseface(VNode *vnode,
                                                  VerseFace *vface) {
  struct VLayer *vlayer;
 ...
 */
VerseVert *create_verse_vertex(VSession vsession, VNodeID node_id,
                               VLayerID layer_id, uint32 vertex_id, real32 x,
                               real32 y, real32 z) {
  VerseVert *vvert;
 ...
 */
VerseFace *create_verse_face(VSession vsession, VNodeID node_id,
                             VLayerID layer_id, uint32 polygon_id, uint32 v0,
                             uint32 v1, uint32 v2, uint32 v3) {
  struct VerseFace *vface;
 ...
 */
static VLayer *create_vlayer(VerseSession *session, VNodeID node_id,
                             VLayerID layer_id, const char *name,
                             VNGLayerType type, uint32 def_integer,
                             real64 def_real) {
  ...
  vlayer->id = layer_id;
  vlayer->name = (char *)MEM_mallocN(sizeof(char) * (sizeof(name) + 1),
                                     "Verse Layer name");
  ...
  else if ((type == VN_G_LAYER_POLYGON_FACE_UINT8) &&
 ...
 */
static void cb_g_polygon_set_face_real32(void *user_def, VNodeID node_id,
                                         VLayerID layer_id, uint32 polygon_id,
                                         real32 value) {
  /*	struct VerseSession *session = (VerseSession*)current_verse_session();
          struct VNode *vnode;*/
 ...
 */
static void cb_g_polygon_set_face_uint32(void *user_def, VNodeID node_id,
                                         VLayerID layer_id, uint32 polygon_id,
                                         uint32 value) {
  /*	struct VerseSession *session = (VerseSession*)current_verse_session();
          struct VNode *vnode;*/
 ...
 */
static void cb_g_polygon_set_face_uint8(void *user_def, VNodeID node_id,
                                        VLayerID layer_id, uint32 polygon_id,
                                        uint8 value) {
  struct VerseSession *session = (VerseSession *)current_verse_session();
  struct VNode *vnode;
  ...
  vlayer = (VLayer *)TLI_dlist_find_link(&(((VGeomData *)vnode->data)->layers),
  ...
  polygon_layer =
      find_verse_layer_type((VGeomData *)vnode->data, POLYGON_LAYER);
 ...
 */
static void cb_g_polygon_set_corner_real32(void *user_def, VNodeID node_id,
                                           VLayerID layer_id, uint32 polygon_id,
                                           real32 v0, real32 v1, real32 v2,
                                           real32 v3) {}
...
static void cb_g_polygon_set_corner_uint32(void *user_data, VNodeID node_id,
  ...
  vface =
      (VerseFace *)TLI_dlist_find_link(&(vlayer->dl), (unsigned int)polygon_id);

  if (!vface) {
    /* try to find VerseFace in list of VerseVaces created by me and set up
     * polygon and layer ids */
    vface = find_verse_face_in_queue(vlayer, node_id, layer_id, polygon_id, v0,
                                     v1, v2, v3);
    ...
    if (vface) {
      ...
      vface = create_verse_face(session->vsession, node_id, layer_id,
    ...
    if (vface->counter < 1) {
      ...
      TLI_dlist_add_item_index(&(vlayer->dl), (void *)vface,
                               (unsigned int)polygon_id);
      increase_verse_verts_references(vface);
      recalculate_verseface_normals(vnode);
      ((VGeomData *)vnode->data)->post_polygon_create(vnode, vface);
    } else {
      /* when all needed VerseVertexes weren't received, then VerseFace is moved
       * to the list of orphans waiting on needed vertexes */
      vface->flag |= FACE_RECEIVED;
      TLI_addtail(&(vlayer->orphans), (void *)vface);
    }
  } else {
    /* VerseVertexes of existing VerseFace were changed (VerseFace will use some
     * different VerseVertexes or it will use them in different order) */
    ...
    /* set up new pointers at VerseVerts and return number of missing
     * VerseVertexes */
    vface->counter = test_incoming_verseface((VGeomData *)vnode->data, vface);
    ...
    if (vface->counter < 1) {
      ...
      /* when all needed VerseVertexes weren't received, then VerseFace is added
       * to the list of orphans waiting on needed vertexes */
 ...
 */
static void cb_g_vertex_set_real32(void *user_def, VNodeID node_id,
                                   VLayerID layer_id, uint32 vertex_id,
                                   real32 value) {
  /*	struct VerseSession *session = (VerseSession*)current_verse_session();
          struct VNode *vnode;*/
 ...
 */
static void cb_g_vertex_set_uint32(void *user_def, VNodeID node_id,
                                   VLayerID layer_id, uint32 vertex_id,
                                   uint32 value) {
  /*	struct VerseSession *session = (VerseSession*)current_verse_session();
          struct VNode *vnode;*/
...
static void cb_g_vertex_delete_real32(void *user_data, VNodeID node_id,
  ...
  vvert =
      (VerseVert *)TLI_dlist_find_link(&(vlayer->dl), (unsigned int)vertex_id);
...
/*
 * callback function: position of one vertex was changed or new vertex was
 * created
 */
static void cb_g_vertex_set_xyz_real32(void *user_data, VNodeID node_id,
                                       VLayerID layer_id, uint32 vertex_id,
  ...
  if (vlayer->id == 0) {
    ...
    vvert = (VerseVert *)TLI_dlist_find_link(&(vlayer->dl),
    ...
    if (vvert) {
      ...
      vvert = find_verse_vert_in_queue(vlayer, node_id, layer_id, vertex_id, x,
                                       y, z);
      ...
      if (vvert) {
        ...
        TLI_dlist_add_item_index(&(vlayer->dl), (void *)vvert,
                                 (unsigned int)vertex_id);
        /* set VerseVert flags */
        ...
        vvert = create_verse_vertex(session->vsession, node_id, layer_id,
                                    vertex_id, x, y, z);
...
static void cb_g_layer_create(void *user_data, VNodeID node_id,
  ...
  /* temp hack due to siggraph presentation ... client subscribe for all verse
   * layers */
  verse_send_g_layer_subscribe(node_id, layer_id, (VNRealFormat)0);

  /* create new VerseLayer */
  vlayer = create_vlayer(session, node_id, layer_id, name, type, def_integer,
                         def_real);
  /* add layer to the list of VerseLayers */
  TLI_dlist_add_item_index(&(((VGeomData *)vnode->data)->layers),
                           (void *)vlayer, (unsigned int)layer_id);
  ...
  if (vnode->owner_id == VN_OWNER_MINE) {
    ...
    case VN_G_LAYER_VERTEX_XYZ:
      if (layer_id == 0)
        push_all_verts(vnode, vlayer);
      break;
    case VN_G_LAYER_POLYGON_CORNER_UINT32:
      if (layer_id == 1)
        push_all_faces(vnode, vlayer);
      break;
