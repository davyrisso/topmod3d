...
/* button callback function, it test object name and send new name to verse
 * server */
...
void t_verse_push_object(VerseSession *session, Object *ob) {
  ...
  // 	obj_vnode= create_verse_node(session->vsession, -1 , V_NT_OBJECT,
  // VN_OWNER_MINE);
  // 	/* create object data */
  ...
  // 	obj_vnode->name =
  // (char*)MEM_mallocN(sizeof(char*)*(strlen(ob->id.name)-1), "object node
  // name"); 	obj_vnode->name[0] = '\0'; 	strcat(obj_vnode->name, ob->id.name+2);
  //
  ...
  // 		geom_vnode= create_verse_node(session->vsession, -1 , V_NT_GEOMETRY,
  // VN_OWNER_MINE);
  // 		/* create new geometry data */
  // 		geom_vnode->data = create_geometry_data();
  //
  // 		/* create new link between VereseNodes */
  // 		vlink = create_verse_link(session->vsession, obj_vnode, geom_vnode,
  // -1, -1, "geometry");
  // 		/* send link to verse server */
  // 		add_item_to_send_queue(&(((VObjectData*)obj_vnode->data)->queue),
  // vlink, VERSE_LINK);
  //
  // 		/* set up name of VerseNode */
  // 		geom_vnode->name =
  // (char*)MEM_mallocN(sizeof(char*)*(strlen(((Mesh*)ob->data)->id.name)-1),
  // 				"object node name");
  // 		geom_vnode->name[0] = '\0';
  ...
  //
  // 		/* add node to sending queue */
  // 		add_item_to_send_queue(&(session->queue), geom_vnode,
  // VERSE_NODE);
  // 	}
}
...
void t_verse_pop_node(VNode *vnode) {
  ...
  // 		printf("\talready subscribed to object node ... can't subscribe
  // again\n"); 		return;
  // 	}
  //
...
/*
 * when VerseLink is deleted, then bindings between Object and data should be
 * removed
 */
...
void post_transform(VNode *vnode) {
  ...
  /*	DAG_object_flush_update(G.scene, (Object*)obj_data->object,
   * OB_RECALC_OB);*/
...
void t_verse_send_transformation(Object *ob) {
  ...
  // 	if(!vnode) return;
  //
  // 	/* if last sent position wasn't received yet, then next change of
  // position
  // 	 * can't be send until last send change is received */
  // 	if( ((VObjectData*)vnode->data)->flag & POS_SEND_READY ) {
  // 		if((pos[0]!=ob->loc[0]) || (pos[1]!=ob->loc[1]) ||
  // (pos[2]!=ob->loc[2])) { 			VECCOPY(pos, ob->loc);
  // 			send_verse_object_position(vnode);
  // 		}
  // 	}
  //
  // 	/* if last sent rotation wasn't received yet, then next change of
  // rotation
  // 	 * can't be send until last send change is received */
  ...
  // 	}
  //
  // 	/* if last sent object size wasn't received yet, then next change of
  // object size
  // 	 * can't be send until last send change is received */
  // 	if( ((VObjectData*)vnode->data)->flag & SCALE_SEND_READY ) {
  // 		if((scale[0]!=ob->size[0]) || (scale[1]!=ob->size[1]) ||
  // (scale[2]!=ob->size[2])) { 			VECCOPY(scale, ob->size);
