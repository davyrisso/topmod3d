...
static void cb_connect_accept(void *user_data, uint32 avatar, void *address,
                              void *connection, const uint8 *host_id);
...
static void cb_connect_accept(void *user_data, uint32 avatar, void *address,
  ...
  // VerseTopMod::Instance()->write("\nTopMod was connected to verse server:
  // ");//" + address + "\n");
  // VerseTopMod::Instance()->write(QString("VerseSession->counter:
  // %1\n").arg(session->counter));
...
void set_verse_session_callbacks(void) {
  ...
  verse_callback_set((void *)verse_send_connect_terminate,
                     (void *)cb_connect_terminate, NULL);
}
 ...
 */
VerseSession *create_verse_session(const char *name, const char *pass,
                                   const char *address, uint8 *expected_key) {
  struct VerseSession *session;
  VSession vsession;
  ...
  session->address = (char *)MEM_mallocN(sizeof(char) * (strlen(address) + 1),
                                         "session adress name");
...
void t_verse_connect(char *address) {
  ...
  if ((session_list.first == NULL) && (session_list.last == NULL)) {
    VerseTopMod::Instance()->write(
        "establishing first connection to verse server...");
    set_all_callbacks();
  }
  ...
  if (session) {
    ...
    /*			add_screenhandler(G.curscreen, SCREEN_HANDLER_VERSE,
     * 1);*/
  }
}
