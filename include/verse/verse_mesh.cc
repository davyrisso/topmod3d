...
static void sync_verseface_with_editface(struct VLayer *vlayer,
 ...
 */
void restore_verseface_from_undoface(VNode *vnode, EditFace *efa,
                                     VerseFace *old_vface,
  ...
  //
  // 		/* send information about smoothness, when information in efa
  // differs
  // 		 * from infomration stored in vface */
  // 		if((vface->smooth) && (vface->smooth->value != (efa->flag &
  // ME_SMOOTH))) 			b_verse_set_face_smooth(efa);
  // 	}
  // 	else {
 ...
 */
void push_mface_to_verse_server(MFace *fa, unsigned int face_id, VNode *vnode,
                                VLayer *vlayer) {
  // VerseFace *vface;
  // VerseVert *vvert;
  //
  // vface = (VerseFace*)create_verse_face(vnode->vsession, vnode->id,
  // vlayer->id, face_id, 		fa->v1, fa->v2, fa->v3, fa->v4);
  //
  // vface->face = (void*)fa;
  //
 ...
 * put VerseFace to queue ... send to verse host (server)
 */
void push_edit_face_to_verse_server(EditFace *efa, VNode *vnode,
                                    VLayer *vlayer) {
  // struct VerseFace *vface;
  //
  // vface = (VerseFace*)create_verse_face(vnode->vsession, vnode->id,
...
static void sync_verseface_with_editface(VLayer *vlayer, VerseFace *vface) {
  ...
  // old_verts[2], old_verts[3]); 	printf("\tsending CHANGED FACE\n");
  // 	printf("\t\tnew: %d %d %d %d\n", vface->v0, vface->v1, vface->v2,
  // vface->v3); 	vface->flag |= FACE_CHANGED; 	TLI_dlist_rem_item(&(vlayer->dl),
  // vface->id); 	add_item_to_send_queue(&(vlayer->queue), (void*)vface,
  // VERSE_FACE);
  // }
}
 ...
 */
void restore_versevert_from_undovert(VNode *vnode, EditVert *eve,
                                     VerseVert *old_vvert,
...
static void sync_verseverts_with_editverts(VerseVert *vvert) {
  ...
  // if(!eve) return;
  //
  // if((vvert->co[0] != eve->co[0]) || (vvert->co[1] != eve->co[1]) ||
  // (vvert->co[2] != eve->co[2])) { 	printf("\tVERT sync\n"); 	VECCOPY(vvert->co,
  // eve->co);
  //
 ...
 */
void push_editvert_to_verse_server(EditVert *eve, VNode *vnode,
                                   VLayer *vlayer) {
  // struct VerseVert *vvert;
  //
 ...
 * put VerseVert to queue ... send to verse host (server)
 */
void push_mvert_to_verse_server(MVert *ve, unsigned int vert_id, VNode *vnode,
                                VLayer *vlayer) {
  // struct VerseVert *vvert;
  //
  // vvert = create_verse_vertex(vnode->vsession, vnode->id, vlayer->id,
  // vert_id, 		ve->co[0], ve->co[1], ve->co[2]);
  //
...
/*
 * create new verse vertexes due to all vertexes and send all of them to verse
 * server
 */
...
void post_vertex_create(VNode *vnode, VerseVert *vvert) {
  ...
  // /*		DAG_object_flush_update(G.scene,
  // (Object*)((VObjectData*)obj_vnode->data)->object, OB_RECALC_DATA);*/
  //
...
void post_polygon_create(VNode *vnode, VerseFace *vface) {
  ...
  //
  // 		vlayer = find_verse_layer_type((VGeomData*)vnode->data,
  // VERTEX_LAYER);
  //
  // 		for(i=0; i<4; i++) {
  // 			if(vert_ids[i] != -1) {
  // 				vvert = TLI_dlist_find_link(&(vlayer->dl),
  // vert_ids[i]); 				if(vvert) eves[i] = (EditVert*)vvert->vertex;
  // 			}
  // 		}
  //
  // 		/* to prevent never ending loop of sending and receiving
  ...
  // 		 * server if em->vnode isn't NULL */
  // 		em->vnode = NULL;
  // /*		efa = addfacelist(eves[0], eves[1], eves[2], eves[3], NULL,
  // NULL);*/ 		em->vnode = vnode;
  //
  // 		efa->vface = vface;
  // 		vface->face = efa;
...
void post_polygon_delete(VNode *vnode, VerseFace *vface) {
  ...
  // 		vface->face = NULL;
  //
  // 		/* following two crazy loops wouldn't be neccessary if verse
  // spec
  // 		 * would support edges */
  //
  // 		/* unmark needed edges */
...
void destroy_unused_geometry(VNode *vnode) {
  ...
  // }
  //
  // face_vlayer = find_verse_layer_type((VGeomData*)vnode->data,
  // POLYGON_LAYER); vface = face_vlayer->dl.lb.first; while(vface) {
...
void create_edit_mesh_from_verse_node(VNode *vnode) {
  ...
  //
  // 	vert_layer = find_verse_layer_type((VGeomData*)vnode->data,
  // VERTEX_LAYER); 	face_layer = find_verse_layer_type((VGeomData*)vnode->data,
  // POLYGON_LAYER);
  //
  ...
  // 		if(efa) {
  // 			efa->f = 0;
  // 			efa->flag = ((vface->smooth) &&
  // (vface->smooth->value))?ME_SMOOTH:0; 			efa->h = 0; 			efa->vface = (void*)vface;
  // 			vface->face = (void*)efa;
  // 		}
  // 		vface = vface->next;
  // 	}
...
void push_mesh_geometry(VNode *vnode) {
  ...
  //
  // face_vlayer = find_verse_layer_type((VGeomData*)vnode->data,
  // POLYGON_LAYER); push_all_faces(vnode, face_vlayer);
}
...
void destroy_verse_mesh(VNode *vnode) {
  ...
  //
  // face_vlayer = find_verse_layer_type((VGeomData*)vnode->data,
  // POLYGON_LAYER); vface = face_vlayer->dl.lb.first;
  //
  // while(vface) {
  // 	((EditFace*)vface->face)->vface = NULL;
  // 	vface->face = NULL;
