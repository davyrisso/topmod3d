...
typedef struct VerseFace {
  ...
  unsigned int v0, v1, v2,
      v3; /* indexes of VerseVerts ... needed during receiving */
  /* blender internals */
  short
      flag; /* flags: FACE_SEND_READY, FACE_SENT, FACE_RECEIVED, FACE_CHANGED*/
...
typedef struct VLayer {
  ...
  VNGLayerType type; /* type of layer (VN_G_LAYER_VERTEX_XYZ,
                        VN_G_LAYER_POLYGON_CORNER_UINT32) */
  ...
  short content;         /* type of content (VERTEX_LAYER, POLYGON_LAYER,
                            POLYGON_SMOOTH_LAYER) */
  struct DynamicList dl; /* vertexes, polygons, etc. */
  struct ListBase queue; /* queue of vertexes, polygons, etc. waiting for
                            sending to verse server */
  struct ListBase
      orphans; /* list of versedata (polygons, etc.), that can be added to the
                * DynamicList due to not received VerseVerts */
...
typedef struct VLink {
  ...
  VSession vsession; /* session pointer */
  struct VNode
      *source; /* object VerseNode "pointing" at some other VerseNode */
...
typedef struct VGeomData {
  ...
  struct VLink
      *vlink; /* pointer at VerseLink connecting object node and geom node */
  struct ListBase
      queue;  /* queue of our layers waiting for receiving from verse server */
  void *mesh; /* pointer at Mesh (object node) */
  void *editmesh;            /* pointer at EditMesh (edit mode) */
  unsigned int smooth_layer; /* number of verse layer containing information
                                about face smoothness */
  /* client dependent methods */
  ...
  void (*post_vertex_free_constraint)(struct VNode *vnode,
  ...
  void (*post_polygon_free_constraint)(struct VNode *vnode,
...
typedef struct VObjectData {
  struct DynamicList
      links; /* dynamic list with access array of links between other nodes */
  struct ListBase queue; /* queue of links waiting for sending and receiving
                            from verse server */
...
typedef struct VTag {
  ...
  VNTagType
      type;   /* type: VN_TAG_BOOLEAN, VN_TAG_UINT32, VN_TAG_REAL64,
                 VN_TAG_REAL64_VEC3,   VN_TAG_LINK, VN_TAG_ANIMATION, VN_TAG_BLOB */
...
typedef struct VTagGroup {
  ...
  struct ListBase
      queue; /* list of tags waiting for receiving from verse server */
  /* client dependent methods */
  void (*post_tag_change)(struct VNode *vnode, struct VTag *vatg);
  void (*post_taggroup_create)(struct VNode *vnode,
...
typedef struct VNode {
  ...
  /* blender internals */
  struct DynamicList
      taggroups; /* dynamic list with access array of taggroups */
  struct ListBase
      queue;  /* list of taggroups waiting for receiving from verse server */
...
typedef struct VerseSession {
  ...
  char *address; /* string containg IP/domain name of verse server and number of
                    port */
...
struct VerseSession *create_verse_session(const char *name, const char *pass,
                                          const char *address,
                                          uint8 *expected_key);
...
struct VNode *create_verse_node(VSession vsession, VNodeID node_id, uint8 type,
                                VNodeID owner_id);
void set_node_callbacks(void);

/* functions from verse_object_node.c */
struct VLink *find_unsent_link(struct VerseSession *session,
                               struct VNode *vnode);
struct VLink *create_verse_link(VSession vsession, struct VNode *source,
                                struct VNode *target, uint16 link_id,
                                uint32 target_id, const char *label);
...
struct VerseFace *create_verse_face(VSession vsession, VNodeID node_id,
