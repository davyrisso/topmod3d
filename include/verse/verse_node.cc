...
static struct VTag *find_tag_in_queue(struct VTagGroup *vtaggroup,
                                      const char *name);
static struct VTag *create_verse_tag(VNodeID node_id, uint16 group_id,
                                     uint16 tag_id, const char *name,
                                     VNTagType type, const VNTag *tag);
/* for verse tag groups */
static void free_verse_taggroup_data(struct VTagGroup *taggroup);
static struct VTagGroup *find_taggroup_in_queue(struct VNode *vnode,
                                                const char *name);
static struct VTagGroup *create_verse_taggroup(VNodeID node_id, uint16 group_id,
                                               const char *name);
/* for verse nodes */
static void move_verse_node_to_dlist(struct VerseSession *session,
                                     VNodeID vnode_id);
/* function prototypes of node callback functions */
static void cb_tag_destroy(void *user_data, VNodeID node_id, uint16 group_id,
                           uint16 tag_id);
static void cb_tag_create(void *user_data, VNodeID node_id, uint16 group_id,
                          uint16 tag_id, const char *name, VNTagType type,
                          const VNTag *tag);
static void cb_tag_group_destroy(void *user_data, VNodeID node_id,
                                 uint16 group_id);
static void cb_tag_group_create(void *user_data, VNodeID node_id,
                                uint16 group_id, const char *name);
static void cb_node_name_set(void *user_data, VNodeID node_id,
                             const char *name);
static void cb_node_destroy(void *user_data, VNodeID node_id);
static void cb_node_create(void *user_data, VNodeID node_id, uint8 type,
                           VNodeID owner_id);
 ...
 */
static VTag *create_verse_tag(VNodeID node_id, uint16 group_id, uint16 tag_id,
                              const char *name, VNTagType type,
                              const VNTag *tag) {
  // struct VTag *vtag;
  //
  ...
  // vtag->id = tag_id;
  // vtag->name = (char*)MEM_mallocN(sizeof(char)*(strlen(name)+1), "VTag
  // name"); strcpy(vtag->name, name); vtag->type = type;
  //
 ...
 */
static VTagGroup *create_verse_taggroup(VNodeID node_id, uint16 group_id,
                                        const char *name) {
  ...
  // taggroup->name = (char*)MEM_mallocN(sizeof(char)*(strlen(name)+1),
  // "VTagGroup name"); strcpy(taggroup->name, name);
  //
 ...
 */
VNode *create_verse_node(VSession vsession, VNodeID node_id, uint8 type,
                         VNodeID owner_id) {
  // struct VNode *vnode;
  //
...
static void cb_tag_destroy(void *user_data, VNodeID node_id, uint16 group_id,
  ...
  // if(!session) return;
  //
  // vnode = (VNode*)TLI_dlist_find_link(&(session->nodes), (unsigned
  // int)node_id); if(!vnode) return;
  //
...
static void cb_tag_create(void *user_data, VNodeID node_id, uint16 group_id,
  ...
  // 	if(!vtag) vtag = create_verse_tag(node_id, group_id, tag_id, name, type,
  // tag); 	else vtag->id = tag_id;
  //
...
static void cb_tag_group_create(void *user_data, VNodeID node_id,
  ...
  //
  // if(!vtaggroup) {
  // 	/* subscribe to tag group (when new tag will be created, then blender
  // will
  ...
  // 	if(!vtaggroup) vtaggroup = create_verse_taggroup(node_id, group_id,
  // name); 	else vtaggroup->id = group_id;
  //
  // 	/* add tag group to dynamic list with access array */
  // 	TLI_dlist_add_item_index(&(vnode->taggroups), (void*)vtaggroup,
...
static void cb_node_name_set(void *user_data, VNodeID node_id,
  ...
  //
  // vnode = (VNode*)TLI_dlist_find_link(&(session->nodes), (unsigned
  // int)node_id); if(vnode && name) { 	if(!vnode->name) { 		vnode->name =
  // (char*)MEM_mallocN(sizeof(char)*(strlen(name)+1), "VerseNode name");
  // 	}
  // 	else if(strlen(name) > strlen(vnode->name)) {
  // 		MEM_freeN(vnode->name);
  // 		vnode->name = (char*)MEM_mallocN(sizeof(char)*(strlen(name)+1),
  // "VerseNode name");
  // 	}
...
static void cb_node_create(void *user_data, VNodeID node_id, uint8 type,
  ...
  case V_NT_OBJECT:
    if (owner_id == VN_OWNER_MINE) {
      ...
      if (session->queue.first)
        send_verse_node((VNode *)session->queue.first);
      ...
      TLI_dlist_add_item_index(&(session->nodes), (void *)vnode,
  ...
  case V_NT_GEOMETRY:
    if (owner_id == VN_OWNER_MINE) {
      ...
      verse_send_g_layer_create(node_id, 0, "vertex", VN_G_LAYER_VERTEX_XYZ, 0,
                                0);
      verse_send_g_layer_create(node_id, 1, "polygon",
                                VN_G_LAYER_POLYGON_CORNER_UINT32, 0, 0);
      verse_send_g_layer_create(node_id, -1, "smooth",
                                VN_G_LAYER_POLYGON_FACE_UINT8, 0, 0);
    } else {
      /* create new VerseNode*/
      vnode = create_verse_node(session->vsession, node_id, type, owner_id);
...
void set_node_callbacks(void) {
  /* new node created */
  verse_callback_set((void *)verse_send_node_create, (void *)cb_node_create,
                     NULL);
  /* node was deleted */
  verse_callback_set((void *)verse_send_node_destroy, (void *)cb_node_destroy,
                     NULL);
  /* name of node was set */
  verse_callback_set((void *)verse_send_node_name_set, (void *)cb_node_name_set,
                     NULL);

  /* new tag group was created */
  verse_callback_set((void *)verse_send_tag_group_create,
                     (void *)cb_tag_group_create, NULL);
  /* tag group was destroy */
  verse_callback_set((void *)verse_send_tag_group_destroy,
                     (void *)cb_tag_group_destroy, NULL);

  /* new tag was created */
  verse_callback_set((void *)verse_send_tag_create, (void *)cb_tag_create,
                     NULL);
  /* tag was destroy */
  verse_callback_set((void *)verse_send_tag_destroy, (void *)cb_tag_destroy,
                     NULL);
}
