...
void TLI_addhead(ListBase *listbase, void *vlink) {
  ...
  if (listbase->first)
    ((struct Link *)listbase->first)->prev = link;
  if (listbase->last == 0)
    listbase->last = link;
  listbase->first = link;
}
...
void TLI_addtail(ListBase *listbase, void *vlink) {
  ...
  if (listbase->last)
    ((struct Link *)listbase->last)->next = link;
  if (listbase->first == 0)
    listbase->first = link;
  listbase->last = link;
}
...
static void add_array_item(struct DynamicArray *da, void *item,
                           unsigned int index) {
  ...
  if (da->max_item_index < index) {
    ...
    do {
      da->max_item_index +=
          PAGE_SIZE; /* OS can allocate only PAGE_SIZE Bytes */
    } while (da->max_item_index <= index);

    nitems = (void **)MEM_mallocN(sizeof(void *) * (da->max_item_index + 1),
                                  "dlist access array");
    for (i = 0; i <= max; i++)
      nitems[i] = da->items[i];
...
static void init_array(DynamicArray *da) {
  ...
  da->items = (void **)MEM_mallocN(sizeof(void *) * (da->max_item_index + 1),
                                   "dlist access array");
  for (i = 0; i <= da->max_item_index; i++)
    da->items[i] = NULL;
}
 ...
 * recomended to use TLI_dlist_ methods for adding/removing items from dynamic
 * list unless you can end with inconsistence system !!! */
DynamicList *TLI_dlist_from_listbase(ListBase *lb) {
  ...
  dlist->da.items =
      (void **)MEM_mallocN(sizeof(void *) * count, "temp dlist access array");
...
/* take out ListBase from DynamicList and destroy all temporary structures of
 * DynamicList */
ListBase *TLI_listbase_from_dlist(DynamicList *dlist, ListBase *lb) {
  ...
  if (!lb)
    lb = (ListBase *)MEM_mallocN(sizeof(ListBase), "ListBase");
...
void *TLI_dlist_add_item_index(DynamicList *dlist, void *item,
                               unsigned int index) {
  if (!dlist)
    return NULL;
...
char *TLI_strdup(char *str) { return TLI_strdupn(str, strlen(str)); }
...
int TLI_streq(char *a, char *b) { return (strcmp(a, b) == 0); }
int TLI_strcaseeq(char *a, char *b) { return (TLI_strcasecmp(a, b) == 0); }
