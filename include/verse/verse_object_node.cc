...
/* callback functions */
static void cb_o_transform_pos_real32(void *user_data, VNodeID node_id,
                                      uint32 time_s, uint32 time_f,
                                      const real32 *pos, const real32 *speed,
                                      const real32 *accelerate,
                                      const real32 *drag_normal, real32 drag);
static void cb_o_transform_rot_real32(void *user_data, VNodeID node_id,
                                      uint32 time_s, uint32 time_f,
                                      const VNQuat32 *rot,
                                      const VNQuat32 *speed,
                                      const VNQuat32 *accelerate,
                                      const VNQuat32 *drag_normal, real32 drag);
static void cb_o_transform_scale_real32(void *user_data, VNodeID node_id,
                                        real32 scale_x, real32 scale_y,
                                        real32 scale_z);
static void cb_o_link_set(void *user_data, VNodeID node_id, uint16 link_id,
                          VNodeID link, const char *label, uint32 target_id);
static void cb_o_link_destroy(void *user_data, VNodeID node_id, uint16 link_id);

/* other functions */
static void set_target_node_link_pointer(struct VNode *vnode,
                                         struct VLink *vlink);
...
/*
 * set up pointer at VerseLink of target node (geometry node, material node,
 * etc.)
 */
 ...
 */
VLink *create_verse_link(VSession vsession, VNode *source, VNode *target,
                         uint16 link_id, uint32 target_id, const char *label) {
  // struct VLink *vlink;
  //
  ...
  //
  // vlink->label = (char*)MEM_mallocN(sizeof(char)*(strlen(label)+1),
  // "VerseLink label"); vlink->label[0] = '\0'; strcat(vlink->label, label);
  //
  // vlink->flag = 0;
  //
...
static void cb_o_transform_pos_real32(void *user_data, VNodeID node_id,
  ...
  // if( (vnode->owner_id==VN_OWNER_MINE) && !(((VObjectData*)vnode->data)->flag
  // & POS_RECEIVE_READY) ) {
 ...
 */
static void
cb_o_transform_rot_real32(void *user_data, VNodeID node_id, uint32 time_s,
                          uint32 time_f, const VNQuat32 *rot,
                          const VNQuat32 *speed, const VNQuat32 *accelerate,
                          const VNQuat32 *drag_normal, real32 drag) {
  // struct VerseSession *session = (VerseSession*)current_verse_session();
  // struct VNode *vnode;
...
static void cb_o_link_set(void *user_data, VNodeID node_id, uint16 link_id,
  ...
  // 	TLI_dlist_add_item_index(&(((VObjectData*)source->data)->links), vlink,
  ...
  // else {
  // 	/* create new VerseLink */
  // 	vlink = create_verse_link(session->vsession, source, target, link_id,
  // target_id, label);
  // 	/* add VerseLink to dynamic list of VerseLinks */
  // 	TLI_dlist_add_item_index(&(((VObjectData*)source->data)->links), vlink,
  // (unsigned int)link_id);
 ...
 */
static void cb_o_link_destroy(void *user_data, VNodeID node_id,
                              uint16 link_id) {
  // struct VerseSession *session = (VerseSession*)current_verse_session();
  // struct VNode *vnode;
  // struct VLink *vlink;
  ...
  // vnode = TLI_dlist_find_link(&(session->nodes), (unsigned int)node_id);
  //
  // vlink = TLI_dlist_find_link(&(((VObjectData*)vnode->data)->links),
  // link_id);
  //
  // if(vlink) {
  // 	free_verse_link_data(vlink);
