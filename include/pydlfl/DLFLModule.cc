...
static PyMethodDef DLFLMethods[] = {
    ...
    // {"saveLG3d",       dlfl_save_lg3d,      METH_VARARGS,
    // "saveLG3d(string)"},
    ...
    {"createVertex", dlfl_create_vertex, METH_VARARGS,
     "Create a vertex at (x,y,z)"},
    {"removeVertex", dlfl_remove_vertex, METH_VARARGS, "Remove a vertex by ID"},
    /*{"destroyVertex",  dlfl_destroy_vertex, METH_VARARGS, "Remove a vertex
    from the object"},
    {"destroyEdge",    dlfl_destroy_edge,   METH_VARARGS, "Remove a edge from
    the object"},
    {"destroyFace",    dlfl_destroy_face,   METH_VARARGS, "Remove a face from
    the object"},*/
    {"createFace", dlfl_create_face, METH_VARARGS,
     "Create a face from a list of coordinates (specified as 3-tuples)"},
    /* IDs */
    {"faces", dlfl_faces, METH_VARARGS,
     "Grab all/selected the faces of the object"},
    {"edges", dlfl_edges, METH_VARARGS,
     "Grab all/selected the edges of the object"},
    {"verts", dlfl_verts, METH_VARARGS,
     "Grab all/selected the vertices of the object"},
    {"corners", dlfl_corners, METH_VARARGS,
     "Grab all/selected the face-vertices of the object"},
    {"walk", dlfl_walk, METH_VARARGS,
     "Walk around a face and get all of the vertices/edges in lists"},
    {"cornerWalk", dlfl_corner_walk, METH_VARARGS,
     "Walk around a face and get all of the corners in a list"},
    ...
    {"getCorner", dlfl_cornerFromEdgeFace, METH_VARARGS,
     "Get a corner given an edge and a face pointer"},
    {"saveCorner", dlfl_saveCorner, METH_VARARGS,
     "Saves a corner by grabbing a corner ID. This can then be restored later. "
     "This is useful when performing operations like insertEdge that change "
     "face ids"},
    {"restoreCorner", dlfl_restoreCorner, METH_VARARGS,
     "Restores a corner from a corner ID. Returns (faceid,vertexid). This is "
     "useful when performing operations like insertEdge that change face ids"},
    /*{"walkVertices",   dlfl_walk_vertices,  METH_VARARGS,
       "walk_vertices(int)"},
                  {"walkEdges",      dlfl_walk_edges,     METH_VARARGS,
       "walk_edges(int)"},*/
    /* Info */
    {"printObject", dlfl_print_object, METH_VARARGS,
     "Print object information"},
    {"vertexInfo", dlfl_vertexInfo, METH_VARARGS,
     "Position,Valence,etc into dictionary"},
    {"edgeInfo", dlfl_edgeInfo, METH_VARARGS,
     "Midpoint,Verts,etc into dictionary"},
    {"faceInfo", dlfl_faceInfo, METH_VARARGS,
     "Centroid,Verts,etc into dictionary"},
    {"cornerInfo", dlfl_cornerInfo, METH_VARARGS,
     "Face,Vertex,Edge,etc into dictionary"},
    ...
    {"subdivideFaces", dlfl_subdivide_faces, METH_VARARGS,
     "Subdivide a list of Faces"},
    {"dual", dlfl_dual, METH_VARARGS, "Dual of mesh"},
    {"connectEdges", dlfl_connectEdges, METH_VARARGS,
     "Connect an edge on one face with another"},
    {"connectCorners", dlfl_connectCorners, METH_VARARGS,
     "Connect two corners (i.e. Add Hole/Handle)"},
    {"connectFaces", dlfl_connectFaces, METH_VARARGS,
     "Connect two faces (i.e. Add Hole/Handle Closest Vertex)"},
    {"addHandle", dlfl_addHandle, METH_VARARGS,
     "Connect two faces (i.e. Add Hole/Handle Shape Interpolation). Choose "
     "either \"hermite\" or \"bezier\""},
    {"rind", dlfl_rindmodel, METH_VARARGS,
     "Rind modeling. Given a set of faces, create a crust"},
    {"wireframe", dlfl_wireframe, METH_VARARGS,
     "Creates a wireframe of the mesh"},
    {"column", dlfl_column, METH_VARARGS,
     "Creates a wireframe of the mesh via column modeling"},
...
static PyObject *dlfl_insert_edge(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "(ii)(ii)|b", &faceId1, &vertId1, &faceId2,
                        &vertId2, &check))
    return NULL;

  if (currObj) {
    edgeId =
        DLFL::insertEdge(currObj, faceId1, vertId1, faceId2, vertId2, faceId1b,
                         vertId1b, faceId2b, vertId2b, check, false);
    currObj->clearSelected();
  }

  return Py_BuildValue("i,(ii)(ii)(ii)(ii)", edgeId, faceId1, vertId1, faceId2,
                       vertId2, faceId1b, vertId1b, faceId2b, vertId2b);
}
...
static PyObject *dlfl_faces(PyObject *self, PyObject *args) {
  ...
  if (selected) {
    ...
    DLFL::DLFLFacePtrList::iterator it;
    int i;
...
static PyObject *dlfl_edges(PyObject *self, PyObject *args) {
  ...
  if (selected) {
    ...
    DLFL::DLFLEdgePtrList::iterator it;
    int i;
...
static PyObject *dlfl_verts(PyObject *self, PyObject *args) {
  ...
  if (selected) {
    ...
    DLFL::DLFLVertexPtrList::iterator it;
    int i;
...
static PyObject *dlfl_corners(PyObject *self, PyObject *args) {
  ...
  if (selected) {
    ...
    for (int i = 0; i < (int)fvpa.size(); i++) {
      fvert =
          Py_BuildValue("(ii)", fvpa[i]->getFaceID(), fvpa[i]->getVertexID());
...
static PyObject *dlfl_vertexInfo(PyObject *self, PyObject *args) {
  ...
  PyObject *info =
      Py_BuildValue("{s:i,s:s,s:(ddd),s:i}", "id", id, "type", type, "coords",
                    coords[0], coords[1], coords[2], "valence", valence);
...
static PyObject *dlfl_edgeInfo(PyObject *self, PyObject *args) {
  ...
  PyObject *info = Py_BuildValue(
      "{s:i,s:s,s:(ddd),s:(ddd),s:(ii),s:(ii),s:(ii),s:(ii),s:d}", "id", id,
      "type", type, "midpoint", midpoint[0], midpoint[1], midpoint[2], "normal",
      normal[0], normal[1], normal[2], "cornerA", fa, va, "cornerB", fb, vb,
      "cornerC", fc, vc, "cornerD", fd, vd, "length", length);
  delete[] type;
  return info;
}
...
static PyObject *dlfl_faceInfo(PyObject *self, PyObject *args) {
  ...
  PyObject *info =
      Py_BuildValue("{s:i,s:s,s:(ddd),s:(ddd),s:i}", "id", id, "type", type,
                    "centroid", centroid[0], centroid[1], centroid[2], "normal",
                    normal[0], normal[1], normal[2], "size", fp->size());
  delete[] type;
  return info;
}
...
static PyObject *dlfl_cornerInfo(PyObject *self, PyObject *args) {
  if (!currObj) {
    ...
    PyObject *info = Py_BuildValue("{s:i,s:i,s:i,s:s}", "face", fid, "vertex",
                                   vid, "edge", eid, "type", type);
    delete[] type;
    return info;
  }
}
...
static PyObject *dlfl_extrude(PyObject *self, PyObject *args) {
  ...
  const char *choices[] = {"doo-sabin", "dodeca",          "icosa",  "octa",
                           "stellate",  "double-stellate", "cubical"};
  ...
  double sf = 1.0;
  bool boolean = false;
  if (!PyArg_ParseTuple(args, "s#i|diddb", &extrudeType, &size, &faceid, &dist,
                        &num, &rotTwist, &sf, &boolean))
    return NULL;
  ...
  if (choice != -1 && fp != NULL) {
    ...
    case 0: // doo-sabin
      ...
      break;
    case 1: // dodeca
      ofp = DLFL::extrudeFaceDodeca(currObj, fp, dist, num, rotTwist, sf,
                                    boolean);
      break;
    case 2: // icosa
    ...
    case 3: // octa a.k.a. extrude dual face
      ofp =
          DLFL::extrudeDualFace(currObj, fp, dist, num, rotTwist, sf, boolean);
      break;
    case 4: // stellate
...
static PyObject *dlfl_subdivide(PyObject *self, PyObject *args) {
  ...
  int argc =
      PyArg_ParseTuple(args, "s#|ddbb", &subdivType, &size, &attrb1, &attrb2);
  if (!argc)
    return NULL;
  if (argc >= 3)
    battrb1 = true;
  ...
  if (choice != -1) {
    ...
    case 1: // checker
      if (battrb1)
        DLFL::checkerBoardRemeshing(currObj, attrb1);
      else
        DLFL::checkerBoardRemeshing(currObj);
      break;
    case 2: // simplest
    ...
    case 3: // vertex-cut
      if (battrb1)
        DLFL::vertexCuttingSubdivide(currObj, attrb1);
      else
        DLFL::vertexCuttingSubdivide(currObj);
      break;
    case 4: // pentagon
      DLFL::pentagonalSubdivide(
          currObj, attrb1); // since default is 0 don't need if...else
      break;
    case 5: // dual-pentagon
      if (battrb1)
        DLFL::pentagonalSubdivide2(currObj, attrb1);
      else
        DLFL::pentagonalSubdivide2(currObj);
      break;
    case 6: // honeycomb
    ...
    case 7: // doo-sabin
      if (battrb1)
        DLFL::dooSabinSubdivide(currObj, (bool)attrb1);
      else
        DLFL::dooSabinSubdivide(currObj);
      break;
    case 8: // doo-sabin-bc
      if (battrb1)
        DLFL::dooSabinSubdivideBC(currObj, (bool)attrb1);
      else
        DLFL::dooSabinSubdivideBC(currObj);
      break;
    ...
    case 12: // root4
      if (battrb1 && battrb2)
        DLFL::root4Subdivide(currObj, attrb1, attrb2);
      else if (battrb1)
        DLFL::root4Subdivide(currObj, attrb1);
      else
        DLFL::root4Subdivide(currObj);
      break;
    ...
    case 14: // star
      if (battrb1)
        DLFL::starSubdivide(currObj, attrb1);
      else
        DLFL::starSubdivide(currObj);
      break;
    case 15: // sqrt3
    ...
    case 16: // fractal
      if (battrb1)
        DLFL::fractalSubdivide(currObj, attrb1);
      else
        DLFL::fractalSubdivide(currObj);
      break;
    case 17: // stellate
    ...
    default: // allfaces
      if (battrb1)
        DLFL::subdivideAllFaces(currObj, (bool)attrb1);
      else
        DLFL::subdivideAllFaces(currObj);
    }
  }
  Py_INCREF(Py_None);
  return Py_None;
...
static PyObject *dlfl_connectEdges(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "(ii)(ii)|b", &edgeId1, &faceId1, &edgeId2,
                        &faceId2, &loopCheck))
    return NULL;
...
static PyObject *dlfl_connectCorners(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "(ii)(ii)|iis#", &f1, &v1, &f2, &v2, &numsegs,
                        &maxconn, dual, &size))
    return NULL;
...
static PyObject *dlfl_addHandle(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "s#(ii)(ii)i|ddii", type, &size, &f1, &v1, &f2,
                        &v2, &numsegs, &wt1, &wt2, &maxconn, &numtwists))
    return NULL;
  ...
  if (fp1 && fp2) {
    ...
    if (fvp1 && fvp2) {
      ...
      if (strncmp(type, "hermite", size) == 0) {
        DLFL::hermiteConnectFaces(currObj, fvp1, fvp2, numsegs, wt1, wt2,
                                  maxconn, numtwists);
...
static PyObject *dlfl_rindmodel(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!bd|d", &PyList_Type, &list, &useScaling,
                        &thickScale, &uniform))
    return NULL;
...
// static PyObject *dlfl_sierpinsky(PyObject *self, PyObject *args) { return
// NULL; }

// static PyObject *dlfl_multiface(PyObject *self, PyObject *args) { return
// NULL; }
...
static PyObject *dlfl_move(PyObject *self, PyObject *args) {
  ...
  bool relative = true;
  double x, y, z;
  if (!PyArg_ParseTuple(args, "O!(ddd)|b", &PyList_Type, &list, &x, &y, &z,
                        &relative))
    return NULL;
  int size = PyList_Size(list);
  if (size < 0)
...
static void PyDLFL_UsingGUI(bool ugui) { usingGUI = true; }

static void PyDLFL_PassObject(DLFL::DLFLObject *obj) { currObj = obj; }
