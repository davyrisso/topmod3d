...
void Camera::HandleMouseEvent(Qt::MouseButton button, QEvent::Type state, int x,
                              int y) {
...
void OrthoCamera::HandleMouseMotion(int x, int y, int WinX, int WinY) {
  if (ALT_DOWN == 1) {
    ...
    if (MOUSEBUTTON == Qt::LeftButton) {
      ...
      CENTER = CENTER - RIGHT * (x - MOUSE[0]) * pixelToWorld +
               UP * (y - MOUSE[1]) * pixelToWorld;
      EYE = EYE - RIGHT * (x - MOUSE[0]) * pixelToWorld +
            UP * (y - MOUSE[1]) * pixelToWorld;
    }
 ...
 *
 */
void OrthoCamera::enterSelectionMode(double x, double y, double w, double h,
                                     GLint *vp) {
  mousex = x;
  mousey = y;
  pickw = w;
  ...
  if (ALT_DOWN == 1 || MOUSEBUTTON == Qt::MidButton) {
    ...
    if (MOUSEBUTTON == Qt::LeftButton || MOUSEBUTTON == Qt::MidButton) {
      ...
      if (SHIFT_DOWN == 1) {
        ...
        float omega = -.005;
        float mouseDist = sqrt((x - MOUSE[0]) * (x - MOUSE[0]) +
                               (y - MOUSE[1]) * (y - MOUSE[1]));
        EYE = looking * cos(omega * mouseDist) +
              norm(looking) * RtRot * sin(omega * mouseDist);
        EYE = EYE + CENTER;
      ...
      EYE = EYE - (normalized(looking)) * (x - MOUSE[0] + y - MOUSE[1]) *
                      pixelToWorld;
    }
  }
  MOUSE[0] = x;
 ...
 *
 */
void PerspCamera::enterSelectionMode(double x, double y, double w, double h,
                                     GLint *vp) {
  mousex = x;
  mousey = y;
  pickw = w;
