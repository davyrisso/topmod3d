...
public:
  ...
  // Copy constructor
  Matrix3x3(const Matrix3x3 &mat) : BaseObject() { copyFrom(mat); }
  ...
  Vector3d &operator[](uint index) { return row[index]; }

  // Access a row of the matrix - no range checks : const version
  const Vector3d &operator[](uint index) const { return row[index]; }
