...
// in the source file, Vector2d.cc
...
public:
  // Default constructor
  Vector2d() : BaseObject() { elem[0] = elem[1] = 0.0; }

  // 1 argument constructor - intialize all elements with given value
  Vector2d(double val) : BaseObject() { elem[0] = elem[1] = val; }
  ...
  Vector2d(const Vector3d &vec) : BaseObject() { copyFrom(vec); }

  // Construct from a Vector4d - copies first 2 elements
  Vector2d(const Vector4d &vec) : BaseObject() { copyFrom(vec); }
  ...
  void set(double val) { elem[0] = elem[1] = val; }
  ...
  friend Vector2d operator*(double scalar,
                            const Vector2d &vec) // Pre-multiply by scalar
  {
    Vector2d prod(vec);
  ...
  friend Vector2d product(const Vector2d &vec1,
                          const Vector2d &vec2) // Elem-by-elem product
  {
  ...
  friend void swap(Vector2d &vec1,
                   Vector2d &vec2) // Swap elements of 2 Vector2ds
  {
  ...
  friend istream &operator>>(istream &i, Vector2d &vec) // Extraction operator
  {
    ...
    if (readTillFloat(i)) {
      ...
      if (readTillFloat(i))
        ...
        vec.elem[1] = vec.elem[0];
    }
    return i;
  }

  friend ostream &operator<<(ostream &o,
                             const Vector2d &vec) // Insertion operator
  {
