...
public:
  ...
  Matrix4x4(const Vector4d &r0, const Vector4d &r1, const Vector4d &r2,
            const Vector4d &r3)
      : BaseObject() {
    row[0] = r0;
    row[1] = r1;
  ...
  // Copy constructor
  Matrix4x4(const Matrix4x4 &mat) : BaseObject() { copyFrom(mat); }

  // Constructor from a 3x3 matrix
  Matrix4x4(const Matrix3_3 &mat3) : BaseObject() { copyFrom(mat3); }
  ...
  void set(const Vector4d &r0, const Vector4d &r1, const Vector4d &r2,
           const Vector4d &r3) {
    row[0] = r0;
    row[1] = r1;
    row[2] = r2;
  ...
  Vector4d &operator[](uint index) { return row[index]; }

  // Access a row of the matrix - no range checks : const version
  const Vector4d &operator[](uint index) const { return row[index]; }
  ...
  friend istream &operator>>(istream &i, Matrix4x4 &mat) {
    char rowsep, dummy;
    i >> dummy >> mat.row[0] >> rowsep >> mat.row[1] >> rowsep >> mat.row[2] >>
        rowsep >> mat.row[3] >> dummy;
    return i;
  }
};
