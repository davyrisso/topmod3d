...
public:
  ...
  Quaternion() : BaseObject(), v3Vec(), dScalar(1.0) {}

  // Construct from a vector. Scalar is set to 0
  Quaternion(const Vector3d &vec) : BaseObject(), v3Vec(vec), dScalar(0.0) {}
  ...
  // Arithmetic operators and various other functions, implemented as friend
  // functions
  ...
  friend double length(const Quaternion &quat) { return sqrt(lengthsqr(quat)); }

  // For consistency define norm and normsqr also
  friend double normsqr(const Quaternion &quat) { return lengthsqr(quat); }

  friend double norm(const Quaternion &quat) { return length(quat); }
  ...
  // Same as above, except Vector3d is promoted to quaternion with scalar value
  // 0.0
  ...
  Matrix3_3 toMatrix(void) const {
    ...
    mat[0].set(1.0 - 2.0 * (y * y + z * z), 2.0 * (x * y - s * z),
               2.0 * (x * z + s * y));
    mat[1].set(2.0 * (x * y + s * z), 1.0 - 2.0 * (x * x + z * z),
               2.0 * (y * z - s * x));
    mat[2].set(2.0 * (x * z - s * y), 2.0 * (y * z + s * x),
               1.0 - 2.0 * (x * x + y * y));
  ...
  friend Matrix3_3 toMatrix(const Quaternion &quat) { return quat.toMatrix(); }
  ...
  double getAngle(void) const { return 2.0 * acos(dScalar); }
  ...
  void scaleAngle(double scale_factor) { setAngle(getAngle() * scale_factor); }
...
/*
  ...
  Caught by Shannon - result was order independent, but should be order
  dependent
