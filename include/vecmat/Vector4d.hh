...
// Functions which need the definition of the other Vector classes are defined
// in the source file, Vector4d.cc
...
public:
  ...
  Vector4d(double val1, double val2, double val3, double val4) : BaseObject() {
    elem[0] = val1;
    elem[1] = val2;
    elem[2] = val3;
    elem[3] = val4;
  ...
  // Copy constructor
  Vector4d(const Vector4d &vec) : BaseObject(vec) {
    elem[0] = vec.elem[0];
    elem[1] = vec.elem[1];
    elem[2] = vec.elem[2];
    elem[3] = vec.elem[3];
  ...
  Vector4d(const Vector2d &vec) : BaseObject() { copyFrom(vec); }

  // Construct from a Vector3d - fourth element is set to 0
  Vector4d(const Vector3d &vec) : BaseObject() { copyFrom(vec); }
  ...
  void set(double val) { elem[0] = elem[1] = elem[2] = elem[3] = val; }
  ...
  const double *getCArray(void) const { return (const double *)elem; }

  double *getCArray(void) { return (double *)elem; }
  ...
  operator double *(void) { return (double *)elem; }
#endif

  // Element access operator - doesn't check for valid indices
  double &operator[](uint index) { return elem[index]; }

  double operator[](uint index) const { return elem[index]; }
  ...
  friend Vector4d operator*(double scalar,
                            const Vector4d &vec) // Pre-multiply by scalar
  {
    Vector4d prod(vec);
  ...
  friend Vector4d operator*(const Vector4d &vec,
                            double scalar) // Post-multiply by scalar
  {
    Vector4d prod(vec);
  ...
  friend Vector4d operator/(const Vector4d &vec,
                            double scalar) // Divide by scalar
  {
    Vector4d prod(vec);
  ...
  friend Vector4d product(const Vector4d &vec1,
                          const Vector4d &vec2) // Elem-by-elem product
  {
    Vector4d prod(vec1[0] * vec2[0], vec1[1] * vec2[1], vec1[2] * vec2[2],
                  vec1[3] * vec2[3]);
  ...
  {
    double n = norm(vec);
    if (!areEqual(n, 1.0) && (isNonZero(n) == true))
      vec /= n;
    return n;
  }
  ...
  friend void swap(Vector4d &vec1,
                   Vector4d &vec2) // Swap elements of 2 Vector4ds
  {
  ...
  friend ostream &operator<<(ostream &o,
                             const Vector4d &vec) // Insertion operator
  {
    ...
    o << "[" << vec.elem[0] << " " << vec.elem[1] << " " << vec.elem[2] << " "
      << vec.elem[3] << "]";
