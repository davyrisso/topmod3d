...
// in the source file, Vector3d.cc
...
public:
  ...
  Vector3d(double val1, double val2, double val3 = 0.0) : BaseObject() {
    elem[0] = val1;
    elem[1] = val2;
    elem[2] = val3;
  }
  ...
  // Copy constructor
  Vector3d(const Vector3d &vec) : BaseObject(vec) {
    elem[0] = vec.elem[0];
    elem[1] = vec.elem[1];
    elem[2] = vec.elem[2];
  }
  ...
  Vector3d(const Vector2d &vec) : BaseObject() { copyFrom(vec); }

  // Construct from a Vector4d - copies first 3 elements
  Vector3d(const Vector4d &vec) : BaseObject() { copyFrom(vec); }
  ...
  void set(double val) { elem[0] = elem[1] = elem[2] = val; }
  ...
  Vector3d
  operator%(const Vector3d &vec) const // Multiplication - cross product
  {
    Vector3d crossp;
  ...
  friend Vector3d operator*(double scalar,
                            const Vector3d &vec) // Pre-multiply by scalar
  {
    Vector3d prod(vec);
  ...
  friend Vector3d product(const Vector3d &vec1,
                          const Vector3d &vec2) // Elem-by-elem product
  {
  ...
  friend void swap(Vector3d &vec1,
                   Vector3d &vec2) // Swap elements of 2 Vector3ds
  {
  ...
  friend ostream &operator<<(ostream &o,
                             const Vector3d &vec) // Insertion operator
  {
