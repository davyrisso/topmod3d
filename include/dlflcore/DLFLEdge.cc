...
void DLFLEdge::updateMidPoint(void) {
  if (fvpV1 != NULL && fvpV2 != NULL)
    midpoint = 0.5 * (fvpV1->getVertexCoords() + fvpV2->getVertexCoords());
}

// Update the edge normal - average of normals at the 4 corners adjacent to this
// edge
...
DLFLFaceVertexPtr DLFLEdge::getFaceVertexPtr(DLFLFacePtr fptr) {
  ...
  fp1 = fvpV1->getFacePtr();
  fp2 = fvpV2->getFacePtr();
  if (fp1 == fptr)
    fvp = fvpV1;
  else if (fp2 == fptr)
    fvp = fvpV2;
...
bool coFacial(DLFLEdgePtr ep1, DLFLEdgePtr ep2) {
  ...
  bool cofacial = false;
  if (fp11 == fp21 || fp11 == fp22 || fp12 == fp21 || fp12 == fp22)
    cofacial = true;
  return cofacial;
}

void DLFLEdge::getEndPoints(Vector3d &p1, Vector3d &p2) const {
  p1 = fvpV1->getVertexCoords();
  p2 = fvpV2->getVertexCoords();
}
...
void DLFLEdge::getFacePointers(DLFLFacePtr &fptr1, DLFLFacePtr &fptr2) {
  fptr1 = fvpV1->getFacePtr();
  fptr2 = fvpV2->getFacePtr();
}
...
DLFLFacePtr DLFLEdge::getOtherFacePointer(DLFLFacePtr fptr) {
  ...
  if (fvpV1->getFacePtr() == fptr)
    return fvpV2->getFacePtr();
  else if (fvpV2->getFacePtr() == fptr)
    return fvpV1->getFacePtr();
  return NULL;
}

void DLFLEdge::getVertexPointers(DLFLVertexPtr &vp1, DLFLVertexPtr &vp2) {
  vp1 = fvpV1->getVertexPtr();
  vp2 = fvpV2->getVertexPtr();
}

DLFLVertexPtr DLFLEdge::getOtherVertexPointer(DLFLVertexPtr vptr) {
  if (fvpV1->getVertexPtr() == vptr)
    return fvpV2->getVertexPtr();
  else if (fvpV2->getVertexPtr() == vptr)
    return fvpV1->getVertexPtr();
  return NULL;
}
...
bool operator==(const DLFLEdge &e1, const DLFLEdge &e2) {
  ...
  if ((e11 == e21) && (e12 == e22))
    same = true;
  else if ((e11 == e22) && (e12 == e21))
    same = true;
  return same;
}

bool DLFLEdge::isValid(void) const {
  if (fvpV1->getFacePtr() == fvpV2->getFacePtr())
    return false;
  return true;
}

bool DLFLEdge::isSelfLoop(void) const {
  if (fvpV1->getVertexPtr() == fvpV2->getVertexPtr())
    return true;
  return false;
}
...
bool checkIntersection(DLFLEdgePtr ep1, DLFLEdgePtr ep2) // friend function
{
  ...
  // If one end point of one edge lies on the other edge, it IS NOT considered
  // an intersection If the edges are co-linear, it IS NOT considered an
  // intersection If either of the edges are self loops, there is NO
  // intersection
  ...
  Vector3d n1 = (ep1v2 - ep1v1) % (ep2v1 - ep1v1);
  normalize(n1);
  Vector3d n2 = (ep1v2 - ep1v1) % (ep2v2 - ep1v1);
  normalize(n2);
  ...
  // on either side of ep1. Then check if point of intersection is within the
  // edge
  if (normsqr(n1 + n2) < 1.0e-4) {
    ...
    n1 = (ep2v2 - ep2v1) % (ep1v1 - ep2v1);
    normalize(n1);
    n2 = (ep2v2 - ep2v1) % (ep1v2 - ep2v1);
    normalize(n2);
    if (normsqr(n1 + n2) < 1.0e-4) {
      intersects = true;
    }
...
ostream &operator<<(ostream &o, const DLFLEdge &e) {
  o << "Edge " << e.getID() << " : " << e.fvpV1->getVertexID() << "<-->"
    << e.fvpV2->getVertexID();
  return o;
}

void DLFLEdge::printFaceIDs(void) const {
  cout << "Edge " << uID << " : " << fvpV1->getFaceID() << " ("
       << fvpV1->getVertexID() << ") "
       << "<--> " << fvpV2->getFaceID() << " (" << fvpV2->getVertexID() << ") "
       << endl;
}

void DLFLEdge::printPointers(void) const { cout << fvpV1 << " <-> " << fvpV2; }

// Write out the edge in DLFL format
void DLFLEdge::writeDLFL(ostream &o) const {
  // o << "e {" << getID() << "} " << fvpV1->getIndex() << ' ' <<
  // fvpV2->getIndex() << endl;
...
void DLFLEdge::writeDLFLReverse(ostream &o) const {
  // o << "e {" << getID() << "} " << (fvpV1->next())->getIndex() << ' ' <<
  // (fvpV2->next())->getIndex() << endl;
  o << "e " << (fvpV1->next())->getIndex() << ' ' << (fvpV2->next())->getIndex()
    << endl;
}
...
double DLFLEdge::distFromXY(double x, double y) const {
  ...
  p1.set(p1temp[0], p1temp[1]);
  p2.set(p2temp[0], p2temp[1]);
...
double DLFLEdge::distFromYZ(double y, double z) const {
  ...
  p1.set(p1temp[1], p1temp[2]);
  p2.set(p2temp[1], p2temp[2]);
...
double DLFLEdge::distFromZX(double z, double x) const {
  ...
  p1.set(p1temp[2], p1temp[0]);
  p2.set(p2temp[2], p2temp[0]);
...
void resetEdgeType(DLFLEdgePtr dep) { dep->resetType(); }

void makeEdgeUnique(DLFLEdgePtr dep) { dep->makeUnique(); }
...
/*Vector3d DLFLEdge::getEdgeNormal(DLFLFaceVertexPtr fvptr) const
  ...
  Vector3d edge_vector = normalized(fvpV2->getVertexCoords() -
  fvpV1->getVertexCoords()); // Edge vector
