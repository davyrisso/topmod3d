...
public:
  /// Constructor
  DLFLObject()
      : position(), scale_factor(1), rotation(), vertex_list(), edge_list(),
        face_list(), /* patch_list(), patchsize(4)*/
...
public:
  ...
  DLFLFaceVertexPtrArray
      sel_fvptr_array; // List of selected DLFLFaceVertex pointers
  ...
  inline void removeVertex(DLFLVertexPtr vp) {
    if (vertex_idx.find(vp) != vertex_idx.end())
      vertex_list.erase(vertex_idx[vp]);
    else
      vertex_list.remove(vp);
    vertex_idx.erase(vp);
  };
  inline void removeEdge(DLFLEdgePtr ep) {
    edgeMap.erase(ep->getID());
    if (edge_idx.find(ep) != edge_idx.end())
      edge_list.erase(edge_idx[ep]);
    else
      edge_list.remove(ep);
    edge_idx.erase(ep);
  };
  inline void removeFace(DLFLFacePtr fp) {
    ...
    if (face_idx.find(fp) != face_idx.end())
      face_list.erase(face_idx[fp]);
    else
      face_list.remove(fp);
    face_idx.erase(fp);
  };
...
protected:
  ...
  /*void destroyPatches( ) {
    TMPatchFacePtrList::iterator first = patch_list.begin(), last =
    patch_list.end(); TMPatchFacePtr pfp = NULL; while ( first != last ) { pfp =
    (*first); ++first; delete pfp;
    }
    patch_list.clear();
    };*/
...
private:
  ...
  DLFLObject(const DLFLObject &dlfl)
      : position(dlfl.position), scale_factor(dlfl.scale_factor),
        rotation(dlfl.rotation), vertex_list(dlfl.vertex_list),
        edge_list(dlfl.edge_list), face_list(dlfl.face_list),
...
public:
  ...
  void getVertices(DLFLVertexPtrArray &vparray) {
    vparray.clear();
    vparray.reserve(vertex_list.size());
    DLFLVertexPtrList::const_iterator first = vertex_list.begin(),
  ...
  void getEdges(DLFLEdgePtrArray &eparray) {
    eparray.clear();
    eparray.reserve(edge_list.size());
  ...
  void makeFacesUnique() {
    // Make faces unique
    DLFLFacePtrList::iterator ffirst = face_list.begin(),
                              flast = face_list.end();
    while (ffirst != flast) {
  ...
  // Lists of second object are cleared. Otherwise when that object is
  // destroyed, pointers in this object will become invalid.
  ...
  // Check if a given edge exists in the edge list. If it does pointer is set to
  // that edge
  ...
  void addEdges(DLFLEdge *edges, int num_edges) {
    ...
    for (int i = 0; i < num_edges; ++i) {
      if (edgeExists(edges[i], eptr) == false) {
        ...
        // If Edge already exists, then the second FaceVertexPtr in the Edge
        // must be changed to that from the new Edge with the same ID as the
        // second one in this Edge
  ...
  DLFLFaceVertexPtr createPointSphere(const Vector3d &v,
                                      DLFLMaterialPtr matl = NULL);
  ...
  void fromData(Vector3d *vertices, int vertex_count, uint *face_index,
                int face_count);
  ...
  void writeLG3d(ostream &o,
                 bool select = false); //!< added by dave - for LiveGraphics3D
                                       //!< support to embed 3d models into html
...
/*void createPatches() {
  ...
  DLFLVertexPtrList::iterator vfirst = vertex_list.begin(), vlast =
vertex_list.end(); DLFLVertexPtr vp = NULL; while ( vfirst != vlast ) { vp =
(*vfirst); ++vfirst; if ( vp->valence() == 4 ) { DLFLFaceVertexPtrArray
vcorners; vp->getOrderedCorners(vcorners); pp1 = vcorners[0]->getPatchPtr(); pp2
= vcorners[2]->getPatchPtr();
