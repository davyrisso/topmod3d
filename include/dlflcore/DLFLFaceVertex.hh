...
// Contains a pointer to a Vertex and a pointer to the Edge which *starts* at
// this Face Vertex NOTE : The same physical point may be the start of many
// edges, but for a given face only 1 Edge can start at a physical point 'start'
// implies existence of a rotation order for the face Each FaceVertex also
// contains a pointer to the Face to which it belongs
...
public:
  ...
  friend void average(const DLFLFaceVertex &dfv1, const DLFLFaceVertex &dfv2,
                      DLFLFaceVertex &ave);
  friend void average(DLFLFaceVertexPtr dfvp1, DLFLFaceVertexPtr dfvp2,
  ...
  void getDS2Coords(Vector3d &dsc0, Vector3d &dsc1, Vector3d &dsc2,
                    Vector3d &dsc3) const {
    dsc0 = ds2coords[0];
    dsc1 = ds2coords[1];
    dsc2 = ds2coords[2];
  ...
  void reset() {
    vertex = NULL;
    normal.reset(); // color = 1.0; texcoord.reset();
    epEPtr = NULL;
    fpFPtr = NULL;
    fvpNext = this;
    fvpPrev = this;
   ...
   * that face. Will not work properly for highly skewed geometry */
  ...
  void setDS2Coords(const Vector3d &dsc0, const Vector3d &dsc1,
                    const Vector3d &dsc2, const Vector3d &dsc3);
  ...
  // Update the DLFLFaceVertexPtr of the DLFLEdge starting at this
  // DLFLFaceVertex
  void addSelfToEdge();
...
private:
  ...
  // It should be used with extreme care, ideally as part of a face reversal
  // only. To prevent unintended misuse, this is made private and classes which
  // need access to this function are made friends
...
public:
  ...
  */
  ...
  /* Read normal, texcoord and color info for this face vertex from an input
   * stream in DLFL format. Returns the vertex index */
  uint readDLFL(istream &i);

  /*
  // "Render" this vertex. Simple calls the render() function for the vertex
  pointer
  ...
  // Do a glVertex on this face vertex, without using the normal and without
  color
