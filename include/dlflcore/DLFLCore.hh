...
int insertEdge(DLFLObjectPtr obj, uint &faceId1, uint &vertId1, uint &faceId2,
               ...
               uint &vertId2b, bool check, bool set_type = false);

/**
 * The general case insertEdge subroutine. Calls one of the insertEdge
 * implementations, depending on whether the corners are cofacial or not. If the
 * 2 corners are cofacial checks to see if the 2 pointers refer to the same
 * corner, if so doesn't do insert.
 */
...
std::vector<int> deleteEdgeID(DLFLObjectPtr obj, uint edgeId,
                              bool cleanup = true);
...
DLFLVertexPtrArray subdivideEdge(DLFLObjectPtr obj, int num_divs,
                                 DLFLEdgePtr edgeptr, bool set_type = false);
...
void subdivideAllEdges(DLFLObjectPtr obj, int num_divs = 2,
                       bool set_type = false);

// Subdivide all edges in the object which are not self loops
void subdivideNonLoopingEdges(DLFLObjectPtr obj, int num_divs = 2,
                              bool set_type = false);
