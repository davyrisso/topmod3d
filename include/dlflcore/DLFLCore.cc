...
int insertEdge(DLFLObjectPtr obj, uint &faceId1, uint &vertId1, uint &faceId2,
               uint &vertId2, uint &faceId1b, uint &vertId1b, uint &faceId2b,
               uint &vertId2b, bool check, bool set_type) {
  int id = -1;
...
DLFLFacePtrArray deleteEdge(DLFLObjectPtr obj, DLFLEdgePtr edgeptr,
  ...
  if (f1 != f2) {
    ...
    // Go through vertices of f2 starting at the beginning and add all of them
    // except fvpV2
    ...
    //  Free the pointer also since the edge_list owns the DLFLEdge pointed to
    //  by edgeptr
    ...
    // Start at vertex after fvpV1, remove from face and add to new face till we
    // reach fvpV2
    //  Don 't add fvpV2 unless fvpV2 is right after fvpV1, in which case we
    //  will have a point sphere
    fvpTemp = fvpV1->next();
    if (fvpTemp == fvpV2) {
      // Add fvpV2.nfp will be a point sphere.Add a copy since fvpV2 will be
      // deleted later on
      //  Since nfp will be a point sphere adding a copy will not affect any
      //  other pointers
    ...
    // sphere
    //  Don 't delete fvpV1 from f1. Otherwise delete fvpV1 from f1 and free the
    //  pointer
    if (fvpV1->next() != fvpV1) {
    ...
    // The Edge can now be removed from the EdgeList
    //  Free the pointer also, since edge_list owns the object pointed to by
    //  edgeptr
    obj->removeEdge(edgeptr);
    delete edgeptr;
    edgeptr = NULL;
    ...
    // If f1 ends up being a point sphere, reset the EdgePtr field of it 's only
    // corner
    if (f1->size() == 1) {
...
DLFLVertexPtr collapseEdge(DLFLObjectPtr obj, DLFLEdgePtr edgeptr,
                           bool cleanup) {
  // Collapse an edge - merge two vertices into one after removing in - between
  // edge
  if (edgeptr == NULL)
    return NULL;
  ...
  // First adjust edge pointer of efvp1 and correspondingly adjust the face -
  // vertex pointer of the edge to which efvp1 will point after removal of
  // vertex 2 Also remove the FVP after efvp1 from the face
  ...
  //  Removal from Edge is not necessary since the Edge itself is going to be
  //  deleted For the same reason no edge pointers have to be adjusted
  efvp2->deleteSelfFromVertex();
  efvp2->deleteSelfFromFace();
  delete efvp2;

  // Go through all remaining face - vertices referring to vp2 and make them
  // point
  //  to vp1 instead.Since vp2 is going to be deleted anyway, these face -
  //  vertices need not be removed from vp2 's list.
  ...
  if (cleanup == true) {
    if (ep1next->getOtherVertexPointer(vp1) ==
        ep1prev->getOtherVertexPointer(vp1)) {
      // Remove ep1next
      deleteEdge(obj, ep1next, true);
    }
    if (ep2next->getOtherVertexPointer(vp1) ==
        ep2prev->getOtherVertexPointer(vp1)) {
      // Remove ep2prev
...
DLFLVertexPtr subdivideEdge(DLFLObjectPtr obj, DLFLEdgePtr edgeptr,
                            bool set_type) {
  // Sub - divide an Edge into 2 Edges.A new Vertex will be added at the
  // midpoint
  ...
  // Edge subdivision will work whether the two Edge sides belong to different
  // Faces
  //  or not.
...
DLFLVertexPtr subdivideEdge(DLFLObjectPtr obj, uint edge_index) {
  // Find the EdgePtr for the given edge_index from the EdgeList and sub -
  // divide it
  if (edge_index >= obj->num_edges())
    return NULL;
...
vector<int> subdivideEdgeID(DLFLObjectPtr obj, int num_divs, uint edgeId,
                            bool set_type) {
  DLFLEdgePtr ep = obj->findEdge(edgeId);
...
DLFLVertexPtrArray subdivideEdge(DLFLObjectPtr obj, int num_divs,
                                 DLFLEdgePtr edgeptr, bool set_type) {
...
void trisectEdge(DLFLObjectPtr obj, DLFLEdgePtr edgeptr, double num, bool scale,
  ...
  //  Boolean flag indicates if specified number is a scale factor or offset
  //  distance Offset distance is clamped to lie between 0.01 and edgelength / 2
  //  - 0.01 Scale factor is clamped to lie between 0.01 and 0.99(inclusive)
...
void subdivideNonLoopingEdges(DLFLObjectPtr obj, int num_divs, bool set_type) {
  // Go through edge list and sub - divide all the old edges which are not self
  // - loops
  DLFLEdgePtrList::iterator first;
  DLFLEdgePtr ep;
  int num_edges, num_old_edges;
...
void spliceCorners(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
                   DLFLFaceVertexPtr fvptr2) {
...
void edgeCleanup(DLFLObjectPtr obj) { edgeCleanup(obj, obj->getEdgeList()); }
...
void edgeCleanup(DLFLObjectPtr obj, const DLFLEdgePtrList &edges) {
  ...
  first = deletion_list.begin();
  last = deletion_list.end();
  while (first != last) {
    edge = (*first);
    ++first;
...
void cleanupWingedVertices(DLFLObjectPtr obj) {
  ...
  while (vfirst != vlast) {
    ...
    if (vp->valence() == 2) {
      ...
      if (pfvp == nfvp) {
        // We have a disconnected 2-gon (consisting of 2 faces, 2 vertices and 2
        // edges) This component will be left as is. Deleting this component is
        // tricky, since we are traversing the vertex list and can't remove
        // vertices arbitrarily We can keep track of such cases and handle them
        // outside the loop
      } else {
  ...
  for (DLFLFacePtrArray::iterator it = faces.begin(); it != faces.end(); ++it) {
...
void splitValence2Vertices(DLFLObjectPtr obj, double offset) {
  ...
  // If offset is negative the vertices are moved by a distance proportional to
  // the length of the edges. Intended for use primarily after a Doo-Sabin type
  // subdivision where the original object has a valence 1 vertex NOTE:
  // Splitting a valence 2 vertex produces 2 new valence 2 vertices They should
  // not be split further. So we first create a list of valence 2 vertices and
  // then go through that list and split them
  ...
  while (vl_first != vl_last) {
    ...
    double e1len, e2len;
    e1vec = e1v2->coords - e1v1->coords;
    e1len = normalize(e1vec);
    e2vec = e2v2->coords - e2v1->coords;
    e2len = normalize(e2vec);
    ...
    vp->setType(VTNewEdgePoint);
    newvp->setType(VTNewPoint);
  }
}

uint *createVertex(double x, double y, double z, DLFLObjectPtr &obj,
                   bool set_type) {
  Vector3d pos(x, y, z);
