...
// An edge appears twice in the system unless it is a boundary edge of a 2D
// surface Each of the pointers points to the beginning Vertex of the edge in
// each of the 2 cases So each of the pointers will be to Face Nodes in
// different faces Definition of class FaceVertexPtr is not required for this
// class since only the pointer is stored
...
private:
  ...
  static void expandEdgePool() {
    ...
    size_t size = (sizeof(DLFLEdge) > sizeof(NextOnFreeList))
                      ? sizeof(DLFLEdge)
...
protected:
  ...
  // Update the edge normal - average of normals at the 4 corners adjacent to
  // this edge
...
public:
  // Default constructor
  DLFLEdge()
      : fvpV1(NULL), fvpV2(NULL), etType(ETNormal), auxcoords(), auxnormal(),
  ...
  DLFLEdgeType getType(void) const { return etType; }

  Vector3d getAuxCoords(void) const { return auxcoords; }

  Vector3d getAuxNormal(void) const { return auxnormal; }

  void setType(DLFLEdgeType type) { etType = type; }

  void resetType(void) { etType = ETNormal; }
  ...
  */
  ...
  // Query functions
  DLFLFaceVertexPtr getFaceVertexPtr1(void) const { return fvpV1; }

  DLFLFaceVertexPtr getFaceVertexPtr2(void) const { return fvpV2; }
  ...
  void getFaceVertexPointers(DLFLFaceVertexPtr &fvptr1,
                             DLFLFaceVertexPtr &fvptr2) const {
    fvptr1 = fvpV1;
    fvptr2 = fvpV2;
  }
  ...
  // Specify the corner where the normal is to be calculated. This defines the
  // origin for the edge vector. Checks to see if given corner belongs to Edge.
  // If not returns a zero vector as the normal
...
public:
  ...
  // If any of the FaceVertexPtr fields are NULL set it to the given
  // FaceVertexPtr
  ...
  // If any of the FaceVertexPtr fields matches the given FaceVertexPtr, set it
  // to NULL
  ...
  // Check if the DLFLEdge is a 'valid' edge - both FaceVertexes shouldn't be
  // from the same Face
  ...
  // Write out the edge in DLFL format but in reverse order. Useful for crust
  // modeling
