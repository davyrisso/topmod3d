...
public:
  ...
  // Get all sums
  void getSums(Vector3d &sumg, Vector2d &sumtc, RGBColor &sumc,
               Vector3d &sumn) const;
  ...
  uint getID(void) const { return uID; }

  DLFLFaceType getType(void) const { return ftType; }

  DLFLMaterialPtr material(void) const { return matl_ptr; }

  Vector3d getAuxCoords(void) const { return auxcoords; }

  Vector3d getAuxNormal(void) const { return auxnormal; }
  ...
  // face-vertices Memory for given face-vertex is not freed. Given FaceVertex
  // must belong to this Face If given face-vertex is the only face-vertex in
  // this face, nothing is done (for now)
  void deleteVertexPtr(DLFLFaceVertexPtr dfvp);

  // Same as above, but more correct function name
  void deleteFaceVertexPtr(DLFLFaceVertexPtr dfvp) { deleteVertexPtr(dfvp); }

  // Get the coordinates of the vertices of the face
  void getVertexCoords(Vector3dArray &verts);

  // STL list type access to the Face
  DLFLFaceVertexPtr front(void) { return head; }
  ...
  DLFLFaceVertexPtr firstVertex(void) { return front(); }

  DLFLFaceVertexPtr lastVertex(void) { return back(); }

  // Set the material pointer
  void setMaterial(DLFLMaterialPtr mptr);

  void setType(DLFLFaceType type) { ftType = type; }

  void resetType(void) { ftType = FTNormal; }

  void setAuxCoords(const Vector3d &p) { auxcoords = p; }

  void setAuxNormal(const Vector3d &n) { auxnormal = n; }

  void addToAuxCoords(const Vector3d &p) { auxcoords += p; }

  void addToAuxNormal(const Vector3d &n) { auxnormal += n; }

  void resetAuxCoords(void) { auxcoords.reset(); }

  void resetAuxNormal(void) { auxnormal.reset(); }

  uint size(void) const; // No. of vertices in this face
  uint numFaceVertexes(void) const { return size(); }

  void
  resetTypeDeep(void); // Reset type of Face and all FaceVertexes in this Face
  ...
  // Create an array of DLFLEdges for this DLFLFace. Number of DLFLEdges in the
  // array is returned. Memory will be allocated inside this function, which
  // should be freed by the caller. Pass a pointer to the array (DLFLEdge **)
  ...
  // Get the corners in the face. Stores the existing DLFLFaceVertexPtr's in the
  // face into the array
  ...
  void getCornersAndCoords(DLFLFaceVertexPtrArray &corners,
  ...
  static void findClosestCorners(DLFLFacePtr fp1, DLFLFacePtr fp2,
                                 DLFLFaceVertexPtr &fvp1,
                                 DLFLFaceVertexPtr &fvp2);

  // Add a new DLFLFaceVertex after/before a given DLFLFaceVertex in this
  // DLFLFace
  DLFLFaceVertexPtr insertAfter(DLFLFaceVertexPtr fvptr,
                                DLFLFaceVertexPtr new_fvp, bool copy = true);
  DLFLFaceVertexPtr insertBefore(DLFLFaceVertexPtr fvptr,
                                 DLFLFaceVertexPtr new_fvp, bool copy = true);
...
public:
  ...
  void print(void) const { boundaryWalk(); }
  ...
  */
  ...
  void objWriteWithNormals(ostream &o, uint min_id,
                           uint &normal_id_start) const;
  ...
  /*
    ...
    void render_FVCN(void) const                      // Render using FV colors
    and normals
    {
    DLFLFace :: glBeginP(size());
    ...
    void render_FV(void) const                        // Render without using FV
    colors or normals
    {
    DLFLFace :: glBeginP(size());
    ...
    void render(void) const                           // Default render -
    render_FVN
    {
    render_FVN();
    }
    ...
    void outline_FVC(void) const                      // Render outline using FV
    colors
    {
    ...
    void outline(void) const                          // Default outline -
    outline_FV
    {
    outline_FV();
    }
    ...
    // because of conflicts with other functions with similar names
    (DLFLMaterial)
