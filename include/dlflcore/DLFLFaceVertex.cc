...
void average(const DLFLFaceVertex &dfv1, const DLFLFaceVertex &dfv2,
  ...
  // color are averaged Assumes that all 3 face vertexes have valid vertex
  // pointers
...
bool DLFLFaceVertex::isConcaveCorner() const {
  ...
  Vector3d pvec = ppos - pos;
  normalize(pvec);
  Vector3d nvec = npos - pos;
  normalize(nvec);
  ...
  // Compute a new point by adding the 2 edge vectors, scaled to a very small
  // length. The choice of the length of the vector is arbitrary and must be
  // chosen to avoid miscalculations because of rounding errors in floating
  // point computations. For a concave corner this point will end up lying
  // outside the polygon For a convex corner this point will end up lying inside
  // the polygon
...
void DLFLFaceVertex::getEdgeVectors(Vector3d &pvec, Vector3d &nvec) {
  nvec = getEdgeVector();
  pvec = -(fvpPrev->getEdgeVector()); // Reverse direction of edgevector from
                                      // previous corner
}
...
void DLFLFaceVertex::addSelfToVertex() {
  if (vertex)
    vertex->addToFaceVertexList(this);
}

void DLFLFaceVertex::deleteSelfFromVertex() {
  if (vertex)
    vertex->deleteFromFaceVertexList(this);
}

void DLFLFaceVertex::addSelfToEdge() {
  // Add self to Edge if there is a NULL DLFLFaceVertex pointer in the Edge
  if (epEPtr)
    epEPtr->setNullFaceVertexPtr(this);
}

void DLFLFaceVertex::deleteSelfFromEdge() {
  // Set the DLFLFaceVertexPtr of Edge which refers to this DLFLFaceVertex to
  // NULL
  if (epEPtr)
    epEPtr->resetFaceVertexPtr(this);
}
...
void DLFLFaceVertex::writeDLFL(ostream &o, uint newindex) {
  o << "fv " << vertex->getIndex() << ' ' << normal << ' ' << texcoord << ' '
    << endl;
  index = newindex;
}
...
uint DLFLFaceVertex::readDLFL(istream &i) {
  ...
  // Read the vertex index and return it, so the caller can set the vertex
  // pointer using the index to reference the vertex list
  uint vindex;
...
void advance(DLFLFaceVertexPtr &fvp) { fvp = fvp->fvpNext; }

void backward(DLFLFaceVertexPtr &fvp) { fvp = fvp->fvpPrev; }

void DLFLFaceVertex::setDS2Coords(const Vector3d &dsc0, const Vector3d &dsc1,
