...
private:
  ...
  static void expandVertexPool() {
    ...
    size_t size = (sizeof(DLFLVertex) > sizeof(NextOnFreeList))
                      ? sizeof(DLFLVertex)
...
public:
  ...
  DLFLVertex(const DLFLVertex &dv)
      : coords(dv.coords), flags(dv.flags), uID(dv.uID), index(dv.index),
        fvpList(dv.fvpList), vtType(dv.vtType), auxcoords(dv.auxcoords),
        auxnormal(dv.auxnormal), normal(dv.normal) {}
  ...
  */
  ...
  DLFLVertexType getType(void) const { return vtType; }

  Vector3d getCoords(void) const { return coords; }

  DLFLFaceVertexPtrList getFaceVertexList(void) const { return fvpList; }

  // Number of Edges incident on this Vertex = no. of Faces adjacent to this
  // Vertex = size of the FaceVertex list = valence of Vertex
  uint numEdges(void) const { return fvpList.size(); }

  uint numFaces(void) const { return fvpList.size(); }

  uint valence(void) const { return fvpList.size(); }

  uint getID(void) const { return uID; }

  uint getIndex(void) const { return index; }
  ...
  void setType(DLFLVertexType type) { vtType = type; }

  void resetType(void) { vtType = VTNormal; }
  ...
  */
  ...
  void setFaceVertexList(const DLFLFaceVertexPtrList &list) { fvpList = list; }

  void setCoords(const Vector3d &p) { coords = p; }
  ...
  // If flag is true recompute normals for all FaceVertexes referring to this
  // vertex, Update the vertex normal. Returns the vertex normal
  ...
  Vector3d averageNormal(void) { return updateNormal(false); }

  // Get normals of all corners referring to this vertex. Returns average normal
  Vector3d getNormals(Vector3dArray &normals);

  // Set tex coordinates, color and normal info for all FaceVertexes referring
  // to this vertex
  void setFaceVertexProps(const Vector2d &texcoord, const RGBColor &color,
  ...
  */
  ...
  // If there are more than 1 and none of them satisfies the condition, returns
  // NULL
  ...
  // The 3 coordinates should be specified separated by spaces (as in OBJ
  // format)
