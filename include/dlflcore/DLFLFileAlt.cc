...
public:
  ...
  Corner(int i, int p, int n) : index(i), prev(p), next(n) {}

  Corner(int i = -1) : index(i), prev(-1), next(-1) {}

  Corner(const Corner &c) : index(c.index), prev(c.prev), next(c.next) {}
...
struct Edge {
  ...
  Edge(Corner s, Corner e) : start(s), end(e) {}
...
static DLFLVertexPtrArray
    vertex_array; // Array containing vertices from each point-sphere
...
static void printFace(const Face &face) {
  ...
  for (int i = 0; i < face.corners.size(); ++i)
    cout << "<" << face.corners[i].prev << "," << face.corners[i].index << ","
         << face.corners[i].next << ">"
         << "  ";
  cout << endl;
}

// Alternate versions of insertEdge subroutines which try to make sure original
// corners are always together and are in the "back" face
DLFLEdgePtr insertEdgeAltCoFacial(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
...
DLFLEdgePtr insertEdgeAltNonCoFacial(DLFLObjectPtr obj,
  ...
  // If a material is specified use it, otherwise stick with material of the
  // newface (fp2)
  if (matl)
    newfaceptr->setMaterial(matl);

  // Go through all vertices in fp1 and add them to the new face and delete them
  // from the old face
  fvp = fp1->front();
  do {
    temp = fvp->next();
    fp1->deleteVertexPtr(fvp);
    newfaceptr->addVertexPtr(fvp);
    fvp = temp;
  } while (fp1->front());
...
DLFLEdgePtr insertEdgeAlt(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
...
void DLFLObject::readObjectAlt(istream &i) {
  ...
  while (i) {
    ...
    if (c == 'c' && c2 == ' ') {
      ...
      if (c2 == ' ') {
        ...
        // Create a point sphere using this vertex and add the vertex to the
        // vertex array. NOTE: Vertex is automatically created and added to
        // vertex list A new face will also be created and added to face list
        ...
        // TEXTURE COORDS ARE NOT USED IN THIS METHOD, BUT WE MUST STILL READ
        // THE LINE
        i >> uv;
      }
    } else if (c == 'f' && c2 == ' ') {
      ...
      if (c != '\n') {
        ...
        // We have to update the prev and next fields for all corners in the
        // face
  ...
  // Go through the array of Faces and create the object using insertEdge
  // operations to create each edge in the Face If an edge already exists
  // between 2 vertices it wont be created
  DLFLFaceVertexPtr sfvp, efvp; // Edge start and end corners
  DLFLVertexPtr svp, evp;       // Edge start and end vertices
  DLFLVertexPtr svp_prev, svp_next, evp_prev,
      evp_next;            // previous and next vertices for each corner
  Corner scorner, ecorner; // Starting and ending corners
  for (int j = 0; j < faces.size(); ++j) {
    face = faces[j];
    int size = face.corners.size();
    for (int k = 0; k < size; ++k) {
      scorner = face.corners[k];
      ecorner = face.corners[(k + 1) % size];
      svp = vertex_array[scorner.index];
      evp = vertex_array[ecorner.index];
      if (edgeExists(svp, evp) == false) {
        ...
        svp_prev = vertex_array[scorner.prev];
        svp_next = vertex_array[scorner.next];
        evp_prev = vertex_array[ecorner.prev];
        evp_next = vertex_array[ecorner.next];

        sfvp = svp->getFaceVertexWithPrev(svp_prev);
        if (sfvp == NULL)
          sfvp = svp->getFaceVertexWithNext(svp_next);

        efvp = evp->getFaceVertexWithPrev(evp_prev);
        if (efvp == NULL)
          efvp = evp->getFaceVertexWithNext(evp_next);
        ...
        if (sfvp != NULL && efvp != NULL) {
          ...
          // cout << "Could not find correct corner for vertex. Postponing edge
          // insertion (1)" << endl;
  ...
  Edge edge;
  first = open_edges.begin();
  last = open_edges.end();
  int depth = 2, num = 0;
  int size = open_edges.size();
  int psize = size;
  bool looping = false;
  // Check for an infinite loop has to be included here since we add elements to
  // the list within this loop
  while (!open_edges.empty()) {
    edge = open_edges.front();
    open_edges.pop_front();
    scorner = edge.start;
    ecorner = edge.end;
    svp = vertex_array[scorner.index];
    evp = vertex_array[ecorner.index];
    if (edgeExists(svp, evp) == false) {
      ...
      if (sfvp == NULL && looping) {
        //                 cout << "Couldn't find correct corner. Trying
        //                 backfacing corner (" << depth << ")" << endl;
    ...
    if (num >= size) {
      ...
      depth++;
      size = open_edges.size();
      num = 0;
      // If we are getting into an infinte loop, set looping flag
      // so backfacing corners will be used next time
      // NOTE: That will always break the deadlocks and thus prevent infinite
      // loops We want to do that only as a last resort
      if (size == psize)
        looping = true;
      psize = size;
    }
