...
Vector3d DLFLVertex::getNormals(Vector3dArray &normals) {
  ...
  if (numnormals > 0) {
    ...
    while (first != last) {
      fvp = (*first);
      ++first;
      normals.push_back(fvp->normal);
      avenormal += fvp->normal;
    }
...
// Set tex coordinates, color and normal info for all FaceVertexes referring to
// this vertex
void DLFLVertex::setFaceVertexProps(const Vector2d &texcoord,
                                    const RGBColor &color, const Vector3d &n) {
  DLFLFaceVertexPtrList::iterator first, last;
...
int DLFLVertex::getEdges(DLFLEdgePtr **edges) const {
  // Create an array of DLFLEdge's incident on this DLFLVertex. The number of
  // DLFLEdges is returned. Memory will be allocated inside this function, which
  // should be freed by the caller. Pass a pointer to the array (DLFLEdge ** or
  // DLFLEdgePtr *) Equivalent to a vertex trace In case of an error -1 is
  // returned, but memory is not freed
...
void DLFLVertex::getOrderedFaceVertices(DLFLFaceVertexPtrArray &fvparray) {
  // Get the face vertices associated with this vertex in the clockwise rotation
  // order
  fvparray.clear();
...
void makeVertexUnique(DLFLVertexPtr dvp) { dvp->assignID(); }

void resetVertexType(DLFLVertexPtr dvp) { dvp->resetType(); }

void vertexTrace(DLFLVertexPtr vertexptr) { vertexptr->vertexTrace(); }

void vertexTrace(const DLFLVertex &vertex) { vertex.vertexTrace(); }
