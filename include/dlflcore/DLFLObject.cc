...
void DLFLObject::dump(ostream &o) const {
  ...
  DLFLVertexPtrList::const_iterator vfirst = vertex_list.begin(),
...
void DLFLObject::splice(DLFLObject &object) {
  ...
  // Entities must be removed from the second object to prevent dangling
  // pointers when it is destroyed.
...
void DLFLObject::assignTileTexEdgeFlags(int n) {
  ...
  // Flags are stored in the first component of the texture coordinate in each
  // face vertex
  int randomvariable1, randomvariable2;
  DLFLEdgePtrList ::const_iterator first = edge_list.begin(),
                                   last = edge_list.end();
  while (first != last) {
...
void DLFLObject::assignTileTexCoords(int n) {
  ...
  DLFLFacePtrList ::const_iterator first = face_list.begin(),
                                   last = face_list.end();
  while (first != last) {
    ...
    //      std::cout<<fromPattern<<std::endl;
    //      ss>>fromPattern;
    //      ss<<"";ss.clear();

    //      ss<<TileMapBase[fromPattern];
    //      std::cout<<TileMapBase[fromPattern]<<std::endl;
    //      for (int i=0; i < corners.size(); ++i)
    //          ss>>flag[i];
    ...
    for (int i = 0; i < corners.size(); ++i) {
      ...
      // Subtract from 1.0 since image origin is at top-left, instead of
      // bottom-left
      // v = 1.0 - (float)( n*flag[2] + flag[3]  + (int)(i/2) ) / (n*n);
      //  Already corrected texture is loaded using Qt opengl method, so no need
      //  to invert here
      v = (float)(n * flag[2] + flag[3] + (int)(i / 2)) / (n * n);
...
void DLFLObject::randomAssignTexCoords() {
  // FOR QUADS ONLY - randomly assign texture coordinates from a unit square to
  // the 4 corners
  DLFLFacePtrList ::iterator first, last;
  first = face_list.begin();
  last = face_list.end();
  while (first != last) {
...
DLFLFacePtrArray DLFLObject::createFace(const Vector3dArray &verts,
                                        DLFLMaterialPtr matl, bool set_type) {
  // Create 2 new faces with the given vertex coordinates. The 2 faces will have
  // the same vertices and share the same edges, but will have opposite rotation
  // orders. This essentially creates a 2 manifold with 2 faces with no volume.
  ...
  newface1 = new DLFLFace;
  newface2 = new DLFLFace;

  if (set_type) {
    newface1->setType(FTNew);
    newface2->setType(FTNew);
  }

  for (int i = 0; i < numverts; ++i) {
    vptr = new DLFLVertex(verts[i]);
    if (i == 0)
      tempvptr = vptr;
    addVertexPtr(vptr);
    fv.setVertexPtr(vptr);
    newface1->addVertex(fv);
    newface2->addVertex(fv);
  }
  ...
  newface1->addFaceVerticesToVertices();
  newface2->addFaceVerticesToVertices();

  // Reorder the second new face so that the first vertex is the first vertex in
  // the array
  DLFLFaceVertexPtrList fvplist = tempvptr->getFaceVertexList();
  fvptr = fvplist.front();
  if (fvptr->getFacePtr() != newface2)
    fvptr = fvplist.back();
  ...
  addFacePtr(newface1);
  addFacePtr(newface2);
  newface1->setMaterial(matl);
  newface2->setMaterial(matl);

  newface1->computeNormal();
  newface2->computeNormal();
...
DLFLFaceVertexPtr DLFLObject::createPointSphere(const Vector3d &v,
...
void DLFLObject::boundaryWalk(uint face_index) {
  // Find the Face with the given face_index from the FaceList and do a boundary
  // walk on it
  uint i = 0;
...
void DLFLObject::vertexTrace(uint vertex_index) {
  // Find the Vertex with the given index in the VertexList and do a vertex
  // trace on it
  uint i = 0;
...
DLFLEdgePtr DLFLObject::findEdge(const uint eid) {
  ...
  // DLFLEdgePtrList::iterator first = edge_list.begin(), last =
  // edge_list.end();
...
DLFLFacePtr DLFLObject::findFace(const uint fid) {
  ...
  // DLFLFacePtrList::iterator first = face_list.begin(), last =
  // face_list.end();
...
DLFLFaceVertexPtr DLFLObject::findFaceVertex(const uint fvid) {
  // Find a face vertex with the given face vertex id. Return NULL if none
  // exists
...
void DLFLObject::addEdge(const DLFLEdge &edge) { addEdgePtr(edge.copy()); }

void DLFLObject::addEdge(DLFLEdgePtr edgeptr) { addEdgePtr(edgeptr->copy()); }

void DLFLObject::addFace(const DLFLFace &face) { addFacePtr(face.copy()); }

void DLFLObject::addFace(DLFLFacePtr faceptr) { addFacePtr(faceptr->copy()); }
...
/*
  ...
  // Find the VertexPtr for the given vertex_index from the VertexList and
  delete it if ( vertex_index > vertex_list.size() ) return;
