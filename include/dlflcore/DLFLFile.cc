...
void DLFLObject::fromData(Vector3d *vertices, int vertex_count,
                          uint *face_index, int face_count) {
  /*
  ...
  for (int i = 0; i < face_count; ++i) {
    ...
    for (int j = 0; j < 4; ++j) {
      ...
      int v =
          face_index[4 * i + j] - 1; // minus 1 is because the face indices
                                     // start with 1 based on OBJ file format
  ...
  while (i) {
    ...
    if (c == 'm' && c2 == 't') {
      ...
      while (c != '\n') {
        ...
        if (vt > 0)
          newfvptr->texcoord = texcoords[vt - 1];
        if (vn > 0)
          newfvptr->normal = normals[vn - 1];
      ...
      // Add the edges from the new face
      addEdges(edges, num_edges);
      // Delete the Edge array allocated by getEdges, since addEdges makes a
      // copy
      delete[] edges;
      edges = NULL;
    }
...
void DLFLObject::writeObject(ostream &o, ostream &omtl, bool with_normals,
  ...
  DLFLVertexPtrList::const_iterator vf = vertex_list.begin(),
                                    vl = vertex_list.end();
  while (vf != vl) {
    o << *(*vf);
    ++vf;
  ...
  if (with_normals) {
    ...
    if (with_tex_coords) {
      ...
      while (ff != fl) {
        ...
        (*ff)->objWriteWithNormalsAndTexCoords(o, min_id, normal_id_start,
                                               tex_id_start);
        ++ff;
      }
    } else {
...
void DLFLObject::readDLFL(istream &i, istream &imtl, bool clearold) {
  ...
  // Otherwise new vertices,faces and edges will be appended to the existing
  // lists
  if (clearold)
    reset();
  ...
  // Read the vertices first. Stop when we get to a '#' sign at the beginning of
  // a line
  while (i && c != '#') {
    i.get(c);
    i.get(c2);
  ...
  // Read the face vertices next. Stop when we get to a '#' sign at the
  // beginning of a line
  if (c2 != '\n')
    readTillEOL(i);
  c = ' ';
  ...
  // Read the edges next. Stop when we get to a '#' sign at the beginning of a
  // line
  if (c2 != '\n')
    readTillEOL(i);
  ...
  while (i && c != '#') {
    ...
    if (c == 'e' && c2 == ' ') {
      ...
      neweptr->setFaceVertexPointers(face_vertex_array[fvindex1],
                                     face_vertex_array[fvindex2], false);
  ...
  // Read the faces next. Stop when we get to a '#' sign at the beginning of a
  // line
  if (c2 != '\n')
    readTillEOL(i);
  c = ' ';
...
bool DLFLObject::writeMTL(ostream &o) {
  ...
  if (o) {
    ...
    DLFLMaterialPtrList::const_iterator mf = matl_list.begin(),
                                        ml = matl_list.end();
    while (mf != ml) {
      o << "newmtl " << (*mf)->name << "\n"
        << "illum 4\n"
        << "Kd " << (*mf)->color.r << " " << (*mf)->color.g << " "
        << (*mf)->color.b << "\n"
        << "Ka 0.00 0.00 0.00\n"
        << "Tf 1.00 1.00 1.00\n"
        << "Ni 1.00\n"
...
void DLFLObject::writeLG3d(ostream &o, bool selected) {
  ...
  if (selected) {
    ...
    // Write the object in LG3d (*.m) format for use with the LiveGraphics3D
    // live.jar java archive from Mathworld.com
    o << "Graphics3D[{";
...
void DLFLObject::writeSTL(ostream &o) {
  ...
  // {
  // 	// Write Header
  // 	char *header="VCG "; 	if(objectname)	strncpy(header,objectname,80);
  ...
  // set to zero 		p.Import(vcg::NormalizedNormal(*fi));
  ...
  o << "solid ascii\n";
  while (ff != fl) {
    o << "  facet normal " << (*ff)->getNormal(true)[0] << " "
      << (*ff)->getNormal()[1] << " " << (*ff)->getNormal()[2] << "\n";
    o << "    outer loop\n";
    (*ff)->getVertexCoords(coords);
    for (i = 0; i < coords.size(); i++) {
      o << "      vertex  " << coords[i][0] << " " << coords[i][1] << " "
        << coords[i][2] << "\n";
    }
    o << "    endloop\n";
    o << "  endfacet\n";
