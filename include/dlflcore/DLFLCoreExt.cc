...
Vector4d Hv(double t) {
  ...
  return Vector4d(2.0 * t3 - 3.0 * t2 + 1.0, -2.0 * t3 + 3.0 * t2,
                  t3 - 2.0 * t2 + t, t3 - t2);
}
...
Vector4d dHv(double t) {
  double t2 = sqr(t);
  return Vector4d(6.0 * t2 - 6.0 * t, -6.0 * t2 + 6.0 * t,
                  3.0 * t2 - 4.0 * t + 1, 3.0 * t2 - 2.0 * t);
}

// Derivative of dHv - second derivative of Hv
Vector4d ddHv(double t) {
  return Vector4d(12.0 * t - 6.0, -12.0 * t + 6.0, 6.0 * t - 4.0,
                  6.0 * t - 2.0);
}
...
bool pointInPolygon(Vector3d p, Vector3dArray poly, const Vector3d &projn) {
  ...
  // The 2 coordinates that will be used in the calculation are (pp+1)%3 and
  // (pp+2)%3 That is X will correspond to (pp+1)%3 and Y will correspond to
  // (pp+2)%3
  int c = 0; // Count of no. of intersections of ray
  Vector3d prevp =
      poly[poly.size() -
           1];   // Previous point to current point. Initialized to last point
  ...
  for (int i = 0; i < (int)poly.size(); i++) {
    ...
    if (((p[y] >= curp[y]) && (p[y] < prevp[y])) ||
      ...
      if (isNonZero(curp[y] - prevp[y])) { // Edge not along ray
        double interx = prevp[x] + (curp[x] - prevp[x]) * (p[y] - prevp[y]) /
                                       (curp[y] - prevp[y]);
        if (p[x] < interx)
          c ^= 1;
      }
...
int findAxialProjectionPlane(const Vector3d &n) {
  ...
  if (x > y) {
    if (y > z)
      ppn = 0; // x is largest, pick YZ plane
    else if (x > z)
      ppn = 0; // x is largest, pick YZ plane
    else
      ppn = 2; // z is largest, pick XY plane
  } else {
    if (x > z)
      ppn = 1; // y is largest, pick ZX plane
    else if (y > z)
      ppn = 1; // y is largest, pick ZX plane
    else
      ppn = 2; // z is largest, pick XY plane
...
Vector3d findProjectionPlane(const Vector3d &n1, const Vector3d &n2) {
  ...
  if (Abs(n1 * n2 + 1.0) < ZERO)
    nproj = n1; // If 2 planes are opposite, choose 1 of them arbitrarily
  normalize(nproj);
  return nproj;
}

// Project a polygon onto a plane defined by point and normal
void planarProject(Vector3dArray &poly, const Vector3d &p,
                   const Vector3d &nproj) {
...
void planarProject(Vector3d &p1, Vector3d &p2, const Vector3d &point,
...
// Rotate the given polygon about it's centroid such that normal goes to new
// direction
void rotate(Vector3dArray &poly, const Vector3d &normal,
...
// Resolve a planar polygon into distance and angle parameters for each vertex,
// based on the centroid. The angles are w.r.t to the given xref vector (assumed
// to be normalized). The yref vector is used to determine quadrant for correct
// angles. xref and yref should be mutually perpendicular and lie on the plane
// of the polygon The distances are from the centroid
void resolvePolygon(const Vector3dArray &poly, const Vector3d &xref,
                    const Vector3d &yref, DoubleArray &angle,
...
void reconstructPolygon(Vector3dArray &poly, const Vector3d &xref,
                        const Vector3d &yref, const DoubleArray &angle,
                        const DoubleArray &distance) {
  int n = angle.size();
  poly.resize(n);
  for (int i = 0; i < n; ++i) {
...
void linearInterpolate(const DoubleArray &start, const DoubleArray &end,
                       DoubleArray &inter, double t) {
  int n = start.size();
  if (end.size() < n)
    n = end.size();
...
// Will do interpolation only upto smaller of 2 given arrays
void linearInterpolate(const Vector3dArray &start, const Vector3dArray &end,
                       Vector3dArray &inter, double t) {
  int n = start.size();
  if (end.size() < n)
    n = end.size();
...
// Do hermite interpolation given 2 end points and 2 end tangents. Compute point
// and tangent at point for given parameter
void hermiteInterpolate(const Vector3d &p1, const Vector3d &v1,
                        const Vector3d &p2, const Vector3d &v2, Vector3d &p,
                        Vector3d &v, Vector3d &dvdt, double t) {
  Vector4d hv, dhv, ddhv, G;
  hv = Hv(t);
  dhv = dHv(t);
  ddhv = ddHv(t);

  G.set(p1[0], p2[0], v1[0], v2[0]);
  p[0] = hv * G;
  v[0] = dhv * G; // dvdt[0] = ddhv * G;
  G.set(p1[1], p2[1], v1[1], v2[1]);
  p[1] = hv * G;
  v[1] = dhv * G; // dvdt[1] = ddhv * G;
  G.set(p1[2], p2[2], v1[2], v2[2]);
  p[2] = hv * G;
  v[2] = dhv * G; // dvdt[2] = ddhv * G;
}

// Compute length of a Hermite curve using chord-length summation.
// Curve is split into specified number of segments
double hermiteCurveLength(const Vector3d &p1, const Vector3d &v1,
                          const Vector3d &p2, const Vector3d &v2, int numsegs) {
  double length = 0.0, t;
  Vector3d p, p0;
...
// Compute the centroid and average normal for a given array of points, based on
// vertex averaging scheme assuming they form a polygon and are specified in the
// correct order (RHS)
void computeCentroidAndNormal(const Vector3dArray &p, Vector3d &centroid,
                              Vector3d &normal) {
  centroid.reset();
  normal.reset();
  int num = p.size();
...
void angleBisectorIntersections(Vector3d const (&pnts)[3],
                                Vector3d (&output)[3]) {
...
// A triangle is transformed to a new triangle whose verticies are the
// intersection of its angle bisectors to their front edges when this process is
// repeated n times the resulting triangle will become regularized. N must be an
// even number.
...
double getAngleBetween(const Vector3d &v1, const Vector3d &v2) {
  ...
  // clamping the dot value to take care of the possible numerical inaccuracy of
  // operations on double
  if (dot > 1.0)
    return 0.0;
  if (dot < -1.0)
    return M_PI;
  // the documentation suggests that acos function return a value in the
  // interval [0,pi] radians, therefore angle is always a positive number
  double angle = acos(dot);
  return angle;
}

// uses Rodrigues' rotation formula to do the vector rotation operation
void rotateAroundVector(const Vector3d &axis, const double &angle,
                        const bool &ckeckAxisIsUnit, Vector3d &vec) {
  if (ckeckAxisIsUnit) {
...
void rotateAroundVector(const Vector3d &axis, const double &angle,
                        const bool &ckeckAxisIsUnit,
                        vector<Vector3d> &vectors) {
  if (ckeckAxisIsUnit) {
    // trust the axis is unit vector
    double b = cos(angle), a = sin(angle);
    for (int i = 0; i < (int)vectors.size(); ++i) {
      vectors[i] = vectors[i] * b + (axis % vectors[i]) * a +
                   axis * (axis * vectors[i]) * (1 - b);
    }
  } else {
    // check if the axis is unit vector
    double length_squared = axis.lengthsqr();
    ...
    n /= sqrt(length_squared);
    for (int i = 0; i < (int)vectors.size(); ++i) {
      vectors[i] = vectors[i] * b + (n % vectors[i]) * a +
                   n * (n * vectors[i]) * (1 - b);
    }
  }
}
...
// Compute the centroid and average normal for a given array of points, based on
// Edge averaging scheme assuming they form a polygon and are specified in the
// correct order (RHS)
void computeEdgeBasedCentroidAndNormal(const Vector3dArray &p,
  ...
  for (int i = 0; i < numpts; ++i) {
    ...
    for (int j = 0; j < numpts; ++j) {
      ...
      if (i == j)
        ...
        coef =
            (3.0 + 2.0 * cos(2.0 * (i - j) * M_PI / numpts)) / (4.0 * numpts);
      p += coef * op[j];
    }
    points[i] = p;
  }
  ...
  for (int i = 0; i < numpts; ++i) {
    ...
    for (int j = 0; j < numpts; ++j) {
      ...
      if (i == j)
        ...
        coef = (1.0 - alpha) *
               (3.0 + 2.0 * cos(2.0 * (i - j) * M_PI / numpts)) /
               (3.0 * numpts - 5.0);
      p += coef * op[j];
    }
    points[i] = p;
...
Vector3d projectPointOnLine(const Vector3d &p, const Vector3d &p0,
                            const Vector3d &p1) {
...
double distPointLine(const Vector3d &p, const Vector3d &p0,
                     const Vector3d &p1) {
...
// Returns a color
RGBColor computeLighting(const Vector3d p, const Vector3d n, const RGBColor&
basecolor, double Ka, double Kd, double Ks, LightPtr lightptr) { RGBColor color;
...
*/
// Find the intersection point between two coplanar lines specified by their end
// points
