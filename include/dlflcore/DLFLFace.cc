...
static Vector2d unittexcoords[] = {Vector2d(0, 0), Vector2d(1, 0),
...
/*
  ...
  The list of DLFLFaceVertexPtr s is circular. So we have to ensure that every
  node get's traversed exactly once. head -> points to beginning of Face.
...
DLFLFace::~DLFLFace() {
  ...
  // associated material
  if (matl_ptr)
    matl_ptr->deleteFace(this);
  destroy();
}

// Copy face from another face. Doesn't destroy existing face. So it's more of
// an insert
...
RGBColor &DLFLFace::fv_color(uint index) { return facevertexptr(index)->color; }
...
void DLFLFace::getSums(Vector3d &sumg, Vector2d &sumtc, RGBColor &sumc,
                       Vector3d &sumn) const {
  if (head) {
    DLFLFaceVertexPtr current = head;
    sumg = current->vertex->coords;
...
void DLFLFace::updateCentroid(void) {
  ...
  if (head) {
    int num = 0;
    DLFLFaceVertexPtr current = head;
    centroid = current->vertex->coords;
    ++num;
    current = current->next();
    while (current != head) {
      centroid += current->vertex->coords;
...
void DLFLFace::getCentroids(Vector3d &ceng, Vector2d &centc, RGBColor &cenc,
                            Vector3d &cenn) const {
  if (head) {
    int num = 0;
    DLFLFaceVertexPtr current = head;
...
// any new edges It simply gets the existing edges. Puts the edge pointers into
// an array
...
void DLFLFace::getCornersAndCoords(DLFLFaceVertexPtrArray &corners,
...
DLFLFaceVertexPtr DLFLFace::insertAfter(DLFLFaceVertexPtr fvptr,
                                        DLFLFaceVertexPtr new_fvp, bool copy) {
...
DLFLFaceVertexPtr DLFLFace::insertBefore(DLFLFaceVertexPtr fvptr,
                                         DLFLFaceVertexPtr new_fvp, bool copy) {
...
void DLFLFace::objWriteWithNormals(ostream &o, uint min_id,
...
float DLFLFace::getArea() {
  ...
  for (eit = eptrarray.begin(); eit != eptrarray.end(); eit++) {
    // get the 2 faces for the current edge, select them if they aren't already
    // selected
...
void boundaryWalk(DLFLFacePtr faceptr) { faceptr->boundaryWalk(); }

void boundaryWalk(const DLFLFace &face) { face.boundaryWalk(); }
