...
public:
  ...
  Camera2(Vector3d P, Vector3d A,
          Vector3d U); //!< constructor setting up camera orientation

  Camera2(Vector3d P, Vector3d A, Vector3d U, float Near, float Far,
          float ViewAngle); //!< Constructor setting up camera orientation and
                            //!< view volume

  void
  SetClippingPlanes(float Near,
                    float Far); //!< sets the clipping planes of the view volume

  void
  SetFOV(float ViewAngle); //!< sets the FOV, ViewAngle should be in degrees
  ...
  void SetCenterOfFocus(
      Vector3d NewAim); //!< focus camera to some input aim position

  void PerspectiveDisplay(int W, int H);

  void HandleMouseEvent(Qt::MouseButton button, QEvent::Type state, int x,
                        int y, int delta = 0);
  ...
  void enterSelectionMode(double x, double y, double w, double h,
                          GLint *vp); //!< \brief for OpenGL selection
