...
void createCrustWithScaling(DLFLObjectPtr obj, double scale_factor) {
  ...
  // Clamp the scale factor to lie between -1 and 1. If negative use inverse of
  // scale factor to get scale factor > 1
  if (scale_factor < -1.0)
    scale_factor = -1.0;
...
void cmMakeHole(DLFLObjectPtr obj, DLFLFacePtr fp, bool cleanup) {
  ...
  if (index > crust_num_old_faces)
    index -= crust_num_old_faces;
  ...
  if (fp1 != NULL && fp2 != NULL) {
    ...
    if (cleanup) {
      ...
      fp1->getEdges(eparray1);
      fp2->getEdges(eparray2);
    }
    connectFaces(obj, fvp1, fvp2);
    crustfp1[index] = crustfp2[index] =
        NULL; // These face pointers are no longer valid
...
void createCrustForWireframe(DLFLObjectPtr obj, double thickness) {
  ...
  if (thickness > 0.0) {
    ...
    while (num_verts < num_old_verts) {
      ...
      DLFLFaceVertexPtr fvp1, fvp2, fvp0, fvp3, fvp4, fvptemp, fvptemp1,
          fvptemp2;
      ...
      DLFLFaceVertexPtrList::iterator first = fvplist.begin(),
                                      last = fvplist.end();
      while (first != last) {
        fvptemp = (*first);
      ...
      normal = normalized(
          n2 % n4); // normal is the direction in which the new_pos will lie.
    ...
    // 		// If uniform thickness is required, adjust thickness
    // appropriately 		if ( uniform ) { 		  avenormal = vp->getNormals(normals);
    // 		  mod_thickness = 0.0;
    // 		  for (int i=0; i < normals.size(); ++i)
    // 		    mod_thickness += thickness/(avenormal*normals[i]);
    // 		  mod_thickness /= normals.size();
    ...
    // 		  mod_thickness = thickness;
    // 		}
    // 		// Positive sign because thickness is positive and normal is
    // inward
    // 		// Use the modified thickness
    // 		newpos = vp->coords + mod_thickness*avenormal;
    // 		vp->coords = newpos;
...
void createCrustForWireframe2(DLFLObjectPtr obj, double scale_factor) {
  ...
  if (scale_factor > 0.0) {
    ...
    while (num_verts < num_old_verts) {
      ...
      //****by dave - experimental - find the centroid of the faces that contain
      //the current vertex
      Vector3d vpcen;
      vpcen.reset();
      // We need to find the centroid of the object
...
void createWireframeWithSegments(DLFLObjectPtr obj, double thickness,
                                 int numSides) {
  ...
  Vector3d edgeVector, xyz, dir, edgeVec1, edgeVec2, axis, start_point,
      faceNormal, P0, P1, P2, normal;
  Vector3dArray vertlist, edge_connect_normals, temp_vertlist,
      temp_chface_vertlist;

  int count = 0, numFvps = 0, edgeID, min_edge_id, index;
  float angle, cosT1, cosT2, T1, T2, L, L1, L2;
  int numVerts, num_original_edges;
  Vector3d zeroVec(0, 0, 0), Xaxis(1, 0, 0), Yaxis(0, 1, 0), vec, PonAxs, vec1,
      Ndir1, Ndir2, VListVec;
  ...
  while (num < numVerts) {
      ...
      for (int i = 0; i < fvparray.size(); i++) {
        ...
        for (int ii = 0; ii < fvparray.size(); ii++) {
          ...
          if (eptr_temp1->getID() != eptr_temp2->getID()) {
            edgeVec1 = (eptr_temp1->getOtherVertexPointer(vp))->getCoords() -
                       vp->getCoords();
            edgeVec2 = (eptr_temp2->getOtherVertexPointer(vp))->getCoords() -
                       vp->getCoords();
            ...
            if (L > length)
              length = L + fabs(cosT1 * thickness) + 0.001 * thickness;
          } // if
        }   // for
    ...
    for (int i = 0; i < fvparray.size(); i++) {
      ...
      edgeVector = normalized((eptr->getOtherVertexPointer(vp))->getCoords() -
                              vp->getCoords());
      axis = edgeVector;
      if (fvparray.size() == 1) {
        vec = Xaxis;
      ...
      length += 0.0001;
      start_point =
          length * axis + normalized(dir) * thickness + vp->getCoords();
    ...
    if (fvparray.size() == 1) {
      ...
      if (!convexhull.createHull(vertlist))
        jointCreated = false;
      else
        jointCreated = true;
      if (jointCreated) {
        ...
        for (int i = 0; i < num_ch_faces; ++i) {
          ...
          if (chfp->size() == numSides) {
            if (vparrayOrdered.size() == fvparray.size() * numSides) {
              ...
              ii = j;
              VListPtr =
                  vparrayOrdered[ii * numSides]; // first vertex of the face
    ...
    if (vertListSizeMatch && jointCreated) {
      ...
      cout << "vertListSizeMatch not matched for vertex " << vp->getID()
           << endl;
    ...
    if (allFacesMatched && vertListSizeMatch && jointCreated) {
      ...
      if (fvparray.size() != 1) {
        ...
        Cindex1 = vp1->CHullIndex;
        Cindex2 = vp2->CHullIndex;
        if (((Cindex1 % 2 == 0) && (Cindex2 % 2 == 0)) ||
            ((Cindex1 % 2 != 0) && (Cindex2 % 2 != 0)))
          doSimilar = true;
        if (((Cindex1 % 2 == 0) && (Cindex2 % 2 != 0)) ||
            ((Cindex1 % 2 != 0) && (Cindex2 % 2 == 0)))
          doSimilar = false;
      ...
      cout << "something not matched for vertex " << vp->getID()
           << endl; // if 0/1
    ...
    if (fvparray.size() != 1 && jointCreated)
  ...
  while (el_first != el_last) {
    ...
    if (eptr->getType() == ETChull) {
      if (edge_connect_fparray[count] != NULL &&
          edge_connect_fparray[count + 1] != NULL) {
        connectFaces(obj, edge_connect_fparray[count],
                     edge_connect_fparray[count + 1],
                     1); // makes the connections
        ++count;
        ++count;
      } else
...
// selects all faces with same number of vertices as the face passed to the
// function initializes them in fparray by reference
void selectMatchingFaces(DLFLObjectPtr obj, DLFLFacePtr fptr,
  ...
  while (fl_first != fl_last) {
    if ((*fl_first)->size() == fptr->size())
      fparray.push_back(*fl_first);
    fl_first++;
  }
}

void selectMatchingEdges(DLFLObjectPtr obj, DLFLEdgePtr eptr,
                         DLFLEdgePtrArray &eparray) {
  eparray.clear();
  ...
  while (el_first != el_last) {
    ...
    if ((vp1->numEdges() == v1->numEdges() &&
         vp2->numEdges() == v2->numEdges()) ||
        (vp1->numEdges() == v2->numEdges() &&
         vp2->numEdges() == v1->numEdges())) {
...
void selectMatchingVertices(DLFLObjectPtr obj, DLFLVertexPtr vptr,
                            DLFLVertexPtrArray &vparray) {
  vparray.clear();
  DLFLVertexPtrList::iterator vl_first = obj->beginVertex(),
                              vl_last = obj->endVertex();

  while (vl_first != vl_last) {
    if ((*vl_first)->numEdges() == vptr->numEdges())
      vparray.push_back(*vl_first);
    vl_first++;
  }
}

void selectFacesByArea(DLFLObjectPtr obj, DLFLFacePtr fptr,
                       DLFLFacePtrArray &fparray, float delta) {
  fparray.clear();
  DLFLFacePtrList::iterator fl_first = obj->beginFace(),
                            fl_last = obj->endFace();
  float area = fptr->getArea();
  while (fl_first != fl_last) {
    if ((*fl_first)->getArea() <= (area + delta) &&
        (*fl_first)->getArea() >= (area - delta))
      fparray.push_back(*fl_first);
    fl_first++;
  }
}

void selectFacesByColor(DLFLObjectPtr obj, DLFLFacePtr fptr,
                        DLFLFacePtrArray &fparray, float delta) {
  // use delta later...
  fparray.clear();
  DLFLFacePtrList::iterator fl_first = obj->beginFace(),
...
void punchHoles(DLFLObjectPtr obj) {
  ...
  fl_first = hole_faces.begin();
  fl_last = hole_faces.end();
  while (fl_first != fl_last) {
    fp = (*fl_first);
    ++fl_first;
    fp->resetType();
...
void makeWireframeWithColumns(DLFLObjectPtr obj, double wireframe_thickness,
                              int wireframe_segments) {
