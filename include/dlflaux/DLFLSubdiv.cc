...
void loopSubdivide(DLFLObjectPtr obj) {
  ...
  while (efirst != elast) {
    ...
    fvp1p1 = fvp1->prev();
    fvp1n2 = (fvp1->next())->next();
    fvp2p1 = fvp2->prev();
    fvp2n2 = (fvp2->next())->next();
    p1 = fvp1->getVertexCoords();
    p2 = fvp2->getVertexCoords();
    p1p1 = fvp1p1->getVertexCoords();
    p2p1 = fvp2p1->getVertexCoords();
    p1n2 = fvp1n2->getVertexCoords();
    p2n2 = fvp2n2->getVertexCoords();
    newpt = (p1 + p2) * 3.0 / 8.0 + (p1p1 + p1n2 + p2p1 + p2n2) * 1.0 / 16.0;
    ep->setAuxCoords(newpt);
  }

  // For every vertex compute the new point coordinates and adjust the
  // coordinate
  ...
  while (vfirst != vlast) {
    ...
    if (valence > 0) {
      ...
      beta = (0.625 - sqr(0.375 + 0.25 * cos(2.0 * M_PI / double(valence)))) /
             double(valence);
  ...
  while (efirst != elast && count < num_old_edges) {
    ...
    ++efirst;
    ++count;
    newpt = ep->getAuxCoords();
    ep->resetAuxCoords();
    vp = subdivideEdge(obj, ep);
    vp->coords = newpt;
  }

  // Connect newly created midpoints in each face
  // Go through all old vertices (before edge subdivision) and
  // go through all corners for each vertex and connect previous and next
  // corners
  vfirst = obj->beginVertex();
  vlast = obj->endVertex();
  count = 0;
...
void checkerBoardRemeshing(DLFLObjectPtr obj, double thickness) {
  ...
  // The code is similar to that for creating the remeshing faces for the Menger
  // Sponge Thickness is a fraction of the edge length for each edge It should
  // be between 0.0 and 0.5. Values outside this range will produce weird
  // results Boundary values produce coincident vertices. If thickness is
  // negative a default value of 0.25 is assumed If thickness is > 0.5 it is
  // clamped to 0.5
  if (thickness < 0.0)
    thickness = 0.25;
  if (thickness > 0.5)
  ...
  // Use the extrudeFacePlanarOffset method to create the central faces for each
  // original face
  ...
  while (count < num_old_verts) {
    ...
    for (int i = 0; i < eparray.size(); ++i) {
      ...
      if (ovp->getType() != VTNewPoint)
        deleteEdge(obj, ep);
      else
        ovp->resetType(); // Reset the type to avoid complications in subsequent
                          // operations
    }
  }
}
...
void simplestSubdivide(DLFLObjectPtr obj) {
  ...
  while (vl_first != vl_last && num_verts < num_old_verts) {
    fvplist = (*vl_first)->getFaceVertexList();
    ++vl_first;
    ++num_verts;

    fvp_first = fvplist.begin();
    fvp_last = fvplist.end();
...
void vertexCuttingSubdivide(DLFLObjectPtr obj, double offset) {
  ...
  while (num_edges < num_old_edges) {
    ...
    vp = subdivideEdge(obj, ep);
    vp->coords = p2 + offset * (p1 - p2);
  }

  // Go through vertex list. For all old vertices (before edge subdivision)
  // go through the face-vertex list and insert edges between previous and
  // next corners to each face-vertex.
  // All old corner edges will have to be deleted after new edges have been
  // inserted
...
void pentagonalSubdivide2(DLFLObjectPtr obj, double scale_factor) {
  ...
  DLFLFaceVertexPtrArray fvplist1,
      fvplist2; // List of face-vertices which have to be connected
  int num_faces, num_old_faces, face_valence;
  Vector3dArray vertex_coords;

  // Reserve space in the two arrays for making connections. The size will be
  // twice the number of original edges. i.e. number of edges after subdividing
  // all edges
  int num_edges = obj->num_edges();
  fvplist1.reserve(num_edges);
  fvplist2.reserve(num_edges);
  ...
  while (fl_first != fl_last && num_faces < num_old_faces) {
    ...
    if (face_valence >= 6) {
      ...
      fvp = head;
      ifvp = ifp->front();
      fvplist1.push_back(fvp);
      fvplist2.push_back(ifvp);
      fvp->resetType();
      fvp = fvp->next();
      fvp = fvp->next();
      ifvp = ifvp->prev();
  ...
  // Go through fvplist1 and fvplist2 and insert edges between corresponding
  // face-vertices
  for (int i = 0; i < fvplist1.size(); ++i) {
    if (fvplist1[i] != NULL && fvplist2[i] != NULL)
      insertEdge(obj, fvplist1[i], fvplist2[i]);
    else
      cout << "NULL pointers found! i = " << i << " " << fvplist1[i] << " "
           << fvplist2[i] << endl;
  }
}
...
void pentagonalSubdivide(DLFLObjectPtr obj, double offset) {
  ...
  // First compute the coordinates of the new points and store them in the
  // aux-coords fields of each entity (vertex, edge, face)

  // Go through each face and compute the centroid and store it in the
  // aux-coords field Do the makeUnique also to make sure Face IDs are
  // consecutive
  Vector3d cen;
  num_faces = 0;
  fl_first = obj->beginFace();
  ...
  // Create point-spheres for each old Face. Insert edges between the
  // point-sphere and every other new vertex (created by edge subdivision) in
  // each old face. Also adjust the coordinates of the new vertices based on the
  // offset.
  ...
  while (fl_first != fl_last && num_faces < num_old_faces) {
    ...
    while (!done) {
      ...
      // in which the next edge has to be inserted because the point-sphere
      // corner is always specified first in the insertEdge call
...
void honeycombSubdivide(DLFLObjectPtr obj) {
  ...
  // Apply make-unique on the obj->num_edges to make sure all Edge IDs are
  // consecutive
  obj->makeEdgesUnique();
  ...
  // For each old face create new faces and update the 2 temporary lists using
  // the edge IDs as index in the lists
  fl_first = obj->beginFace();
  fl_last = obj->endFace();
  num_faces = 0;
  while (fl_first != fl_last && num_faces < num_old_faces) {
    ...
    for (int i = 0; i < num_verts; ++i) {
      ...
      for (int j = 0; j < num_verts; ++j) {
        ...
        if (i == j)
          ...
          coef = (1.0 - alpha) *
                 (3.0 + 2.0 * cos(2.0 * (i - j) * M_PI / num_verts)) /
                 (3.0 * num_verts - 5.0);
        p += coef * midpoints[j];
      }
    ...
    fvp1 = head1 = fp->front();
    fvp2 = head2 = ifp->front();

    ep = fvp1->getEdgePtr();
    edgeindex = ep->getID() - eistart;
    if (fvplist1[edgeindex] == NULL)
      fvplist1[edgeindex] = fvp2;
    else
      fvplist2[edgeindex] = fvp2;
    fvp1 = fvp1->next();
    fvp2 = fvp2->prev();
...
bool dooSabinSubdivide(DLFLObjectPtr obj,
                       bool check /*, QProgressDialog *progress*/) {
  ...
  // testing qprogressbar - need to figure out a way to do this without putting
  // qt into the dlfl libraries
  //  if (progress) {
  ...
  // Reserve and create num_old_edges entries in the 2 temporary lists
  eplist1.resize(num_old_edges, NULL);
  eplist2.resize(num_old_edges, NULL);
  ...
  while (fl_first != fl_last && num_faces < num_old_faces) {
    ...
    for (int i = 0; i < num_verts; ++i) {
      ...
      for (int j = 0; j < num_verts; ++j) {
        if (i == j)
          coef = 0.25 + 5.0 / (4.0 * num_verts);
        else
          coef = (3.0 + 2.0 * cos(2.0 * (i - j) * M_PI / num_verts)) /
                 (4.0 * num_verts);
    ...
    iep = fvp2->getEdgePtr();
    edgeindex = ep->getID() - eistart;
    // cout << "DLFLSubdiv::dooSabinSubdivide: edgeindex = " << edgeindex <<
    // endl;
  ...
  cout << "DooSabin:" << std::clock() / (double)CLOCKS_PER_SEC << endl;
  // Go through eplist1,fplist1 and eplist2,fplist2 and connect corresponding
  // half-edges
  ...
  for (int i = 0; i < num_old_edges; ++i) {
    ...
    if (eplist1[i] != NULL && eplist2[i] != NULL) {
      ...
      if (tfp1->getType() == FTNew)
        ...
        cout << i << " : "
             << "Face not found for half-edge!" << endl;
      ...
      cout << "NULL pointers found! i = " << i << " " << eplist1[i] << " -- "
           << eplist2[i] << endl;
  }
  // //progress bar stuff - dave
  // if (progress)
...
void cornerCuttingSubdivide(DLFLObjectPtr obj, float alpha) {
  ...
  // Go through the face_list,obj->num_edges and vertex_list and destroy all the
  // old faces, edges and vertices
  num_faces = 0;
  fl_first = obj->beginFace();
  fl_last = obj->endFace();
  while (fl_first != fl_last && num_faces < num_old_faces) {
...
void root4Subdivide(DLFLObjectPtr obj, double a, double twist) {
  ...
  int numoldedges =
      obj->num_edges(); // Keep track of no. of edges in un-sub-divided object
  // All old edges have to be deleted in the end
  int numoldverts =
      obj->num_vertices(); // No. of old vertices which have to be moved
  ...
  while (fl_first != fl_last && numfaces < numoldfaces) {
    ...
    oldfaces.push_back(faceptr);
    newfaces.push_back(obj->lastFace());
    ++fl_first;
    ++numfaces;
  }
  ...
  for (int i = 0; i < numconn; ++i) {
    ...
    DLFLFaceVertexPtr fvp1, fvp2;
    fvp1 = oldface->firstVertex();
    fvp2 = newface->firstVertex();
  ...
  while (vl_first != vl_last) {
    ...
    if (count >= numoldverts)
      break; // Done with old vertices
  }
}
...
void catmullClarkSubdivide(DLFLObjectPtr obj) {
  ...
  // Go through each face and compute the centroid and store it in the
  // aux-coords field Do the makeUnique also to make sure Face IDs are
  // consecutive Send the contribution to all vertexes belonging to this face
  Vector3d cen;
  num_faces = 0;
  fl_first = obj->beginFace();
  fl_last = obj->endFace();
  while (fl_first != fl_last) {
    ...
    current->vertex->addToAuxCoords(cen);
  ...
  while (el_first != el_last) {
    ...
    mp = ep->getMidPoint(true);
    afp = (efp1->getAuxCoords() + efp2->getAuxCoords()) / 2.0;
    edgept = (mp + afp) / 2.0;
    ...
    efvp1->vertex->addToAuxCoords(2.0 * mp);
    efvp2->vertex->addToAuxCoords(2.0 * mp);
  }
  ...
  // Now create point-spheres for each Face. Store a pointer to the FaceVertex
  // in a temporary array indexed using the Face ID. For this makeUnique has to
  // be applied to the face list to make the Face IDs consecutive.
  DLFLVertexPtrArray psarray; // Array of point spheres (vertices)
  int fistart, faceindex;

  psarray.reserve(num_faces);
  psarray.resize(num_faces, NULL);
  fl_first = obj->beginFace();
  fl_last = obj->endFace();
  ...
  while (fl_first != fl_last && num_faces < num_old_faces) {
    ...
    fvp->color = fp->colorCentroid();
    fvp->texcoord = fp->textureCentroid();
    fp->resetAuxCoords();
    psarray[faceindex] = fvp->vertex;
  }

  // Now subdivide all the edges. Get the newly created face-vertices
  // and add them to the table along with the vertex from above list of
  // pointspheres
  ...
  num_old_edges = obj->num_edges();
  connindex = 0;
  numconn = 2 * num_old_edges;

  fvplist.resize(numconn, NULL);
  vplist.resize(numconn, NULL);
  ...
  while (el_first != el_last && num_edges < num_old_edges) {
    ...
    for (int i = 0; i < fvparray.size(); ++i) {
      ...
      fvplist[connindex] = fvp;
      vplist[connindex] = psarray[faceindex];
      ++connindex;
    }
    fvparray.clear();
  }
...
void fractalSubdivide(DLFLObjectPtr obj, double offset) { // Doug
  ...
  while (num_faces < num_old_faces) {
    ...
    // This should be the distance btwn two adjacent new vertices, but it is too
    // big
...
void stellateSubdivide(DLFLObjectPtr obj) { // Eric
  ...
  while (el_first != el_last) {
    ...
    ++count;
    deleteEdge(obj, eptr);
    if (count >= num_old_edges)
      break; // Done with old edges
  }
}

void twostellateSubdivide(DLFLObjectPtr obj, double offset,
                          double curve) { // Eric
  DLFLFacePtrList::iterator fl_first, fl_last;
  DLFLFacePtr fp;
  int num_faces, num_old_faces;
...
void dual1264Subdivide(DLFLObjectPtr obj, double sf) {
  ...
  // Reserve and create num_old_edges entries in the 2 temporary lists
  eplist1.reserve(num_old_edges);
  eplist2.reserve(num_old_edges);
  eplist1.resize(num_old_edges, NULL);
  eplist2.resize(num_old_edges, NULL);
  ...
  while (fl_first != fl_last && num_faces < num_old_faces) {
    ...
    if (head) {
      ...
      // Scale the new vertices about their centroid if scale factor is not 1.0
      // or 0.0
      sf = Abs(sf);
...
void dooSabinSubdivideBCNew(DLFLObjectPtr obj, double sf,
                            double length) { // bei dec 11
  DLFLFacePtrList::iterator fl_first, fl_last;
  DLFLEdgePtrList::iterator el_first, el_last;
  DLFLVertexPtrList::iterator vl_first, vl_last;
  DLFLFaceVertexPtrArray fvparray;
...
void subdivideFace(DLFLObjectPtr obj, DLFLFacePtr faceptr, bool usequads) {
  ...
  // If quads are used, edges are subdivided, otherwise existing corners are
  // used
  ...
  if (usequads) {
    // Subdivide all edges in this face and store the corresponding newly
    // created corners which will be used as the end points of the new edges
  ...
  DLFLFaceVertexPtr cenfvp =
      obj->createPointSphere(geomcen, faceptr->material());
  cenfvp->normal = normalcen;
  cenfvp->color = colorcen;
  cenfvp->texcoord = texcen;
...
void subdivideFaces(DLFLObjectPtr obj, DLFLFacePtrArray fparray,
                    bool usequads) {
  ...
  if (usequads) {
    ...
    DLFLFaceVertexPtrList
        fvptypereset; // List of corners whose type has to be reset
    for (int i = 0; i < fparray.size(); ++i) {
      faceptr = fparray[i];

      // Subdivide all edges in this face, set type of new vertex when
      // subdividing. If an edge is a new edge, then we will not subdivide that
      // edge since it was created as a result of a previous edge subdivision
      faceptr->getEdges(edges);
      for (int i = 0; i < edges.size(); ++i) {
        if (edges[i]->getType() != ETNew) {
          ...
          if (vp != NULL) {
            ...
            vp->getEdges(vedges);
            vp->getFaceVertices(vcorners);
            eptypereset.push_back(vedges[0]);
            eptypereset.push_back(vedges[1]);
            fvptypereset.push_back(vcorners[0]);
            fvptypereset.push_back(vcorners[1]);
          }
        }
      }
      ...
      // NOTE: Make sure centroid corner is specified first in the insertEdge
      // call
    ...
    DLFLEdgePtrList::iterator epfirst = eptypereset.begin(),
                              eplast = eptypereset.end();
    ...
    DLFLFaceVertexPtrList::iterator fvpfirst = fvptypereset.begin(),
                                    fvplast = fvptypereset.end();
...
void subdivideFaces(DLFLObjectPtr obj, DLFLFacePtrList fplist, bool usequads) {
  ...
  if (usequads) {
    ...
    while (fpfirst != fplast) {
      ...
      for (int i = 0; i < corners.size(); ++i) {
        if (corners[i]->getType() == FVTNew) {
          ...
          corners[i]->getVertexPtr()->resetType();
          // skip self loop
          if (cenfvp->getVertexPtr() == corners[i]->getVertexPtr())
            continue;
    ...
    fplast = fplist.end();
    while (fpfirst != fplast) {
      subdivideFace(obj, *fpfirst, false);
      ++fpfirst;
    }
  }
}
...
void subdivideAllFaces(DLFLObjectPtr obj, bool usequads) {
  ...
  // will add new faces. Parameter has to passed by value to ensure copy will be
  // made
...
void triangulateAllFaces(DLFLObjectPtr obj) {
  // NOTE: A copy of the list needs to be made since subdivision
  // will add new faces. Parameter has to passed by value to ensure copy will be
  // made this function will only triangulate faces which are not already
  // triangles this will help prepare files for STL export
