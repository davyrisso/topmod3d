...
void planarize(DLFLObjectPtr obj) {
  ...
  Vector3dArray p_array,
      n_array; // Arrays of point and normals for each face adjacent to a vertex
  int num_faces;
  ...
  while (vl_first != vl_last) {
    ...
    */
    fvparray.clear(); // The local array must be cleared before calling
                      // getFaceVertices
    ...
    */
    p_array.clear();
    n_array.clear(); // The local arrays must be cleared for each Vertex

    // If we resize the array before filling it, we can avoid unnecessary memory
    // allocation thus speeding up the process a little bit.
    // p_array.resize(num_faces,Vector3d(0));
    // n_array.resize(num_faces,Vector3d(0));
    p_array.resize(num_faces, Vector3d());
    n_array.resize(num_faces, Vector3d());
    for (int i = 0; i < num_faces; ++i) {
      ...
      // cout << "After p = " << p_array[i] << " n = " << n_array[i] << endl <<
      // endl;
    }
    ...
    */
    ...
    /*
      Use the 2 arrays (n_array and p_array) to compute the new position for
      this vertex
    */
