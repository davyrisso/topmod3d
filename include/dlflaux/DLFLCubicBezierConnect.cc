...
CubicBezierCurve::CubicBezierCurve(const Vector3d (&cp)[4]) { set(cp); }
...
void CubicBezierCurve::point_at(const double &t, Vector3d &point) const {
  ...
  double w3 = t * t * t;
  // set x
  point[0] = w0 * cntrl_pnts[0][0] + w1 * cntrl_pnts[1][0] +
             w2 * cntrl_pnts[2][0] + w3 * cntrl_pnts[3][0];
  // set y
  point[1] = w0 * cntrl_pnts[0][1] + w1 * cntrl_pnts[1][1] +
             w2 * cntrl_pnts[2][1] + w3 * cntrl_pnts[3][1];
  // set z
  point[2] = w0 * cntrl_pnts[0][2] + w1 * cntrl_pnts[1][2] +
             w2 * cntrl_pnts[2][2] + w3 * cntrl_pnts[3][2];
}

void CubicBezierCurve::differentiate_at(const double &t,
                                        Vector3d &derivative) const {
  double u = 1 - t;
  double w0 = 3 * u * u;
  double w1 = 6 * u * t;
  double w2 = 3 * t * t;
  // set x
  derivative[0] = w0 * diff_cntrl_pnts[0][0] + w1 * diff_cntrl_pnts[1][0] +
                  w2 * diff_cntrl_pnts[2][0];
  // set y
  derivative[1] = w0 * diff_cntrl_pnts[0][1] + w1 * diff_cntrl_pnts[1][1] +
                  w2 * diff_cntrl_pnts[2][1];
  // set z
  derivative[2] = w0 * diff_cntrl_pnts[0][2] + w1 * diff_cntrl_pnts[1][2] +
...
CubicBezierBuilder::CubicBezierBuilder(const Vector3d &start,
                                       const Vector3d &normal_s,
                                       const Vector3d &end,
                                       const Vector3d &normal_e,
                                       const double &rotation_angle) {
  set(start, normal_s, end, normal_e, rotation_angle);
}

void CubicBezierBuilder::set(const Vector3d &start, const Vector3d &normal_s,
  ...
  std::cout << "vector 1: " << STRING(normal_s[0]) << " " << STRING(normal_s[1])
            << " " << STRING(normal_s[2]) << '\n';
  std::cout << "vector 2: " << STRING(normal_e[0]) << " " << STRING(normal_e[1])
            << " " << STRING(normal_e[2]) << '\n';
...
bool CubicBezierBuilder::isValid() const { return is_valid && instantiated; }
...
void CubicBezierBuilder::rotate(const double &rotation) {
  ...
  Vector3d v_start =
      cosine * normal_at_start + sine * (rotation_axis % normal_at_start);
  Vector3d v_end =
      cosine * normal_at_end - sine * (rotation_axis % normal_at_end);
  // calculate the control points
  Vector3d p = point_at_end - point_at_start, n = v_end - v_start;
  std::cout << "n: " << STRING(n[0]) << " " << STRING(n[1]) << " "
            << STRING(n[2]) << '\n';
  std::cout << "p: " << STRING(p[0]) << " " << STRING(p[1]) << " "
            << STRING(p[2]) << '\n';
  double a = n * n - 1.0;
  std::cout << "a: " << STRING(a) << '\n';
  double b = 2 * (p * n);
