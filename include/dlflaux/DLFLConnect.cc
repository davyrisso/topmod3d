...
void connectEdgesWithoutLoopCheck(DLFLObjectPtr obj, DLFLEdgePtr eptr1,
                                  DLFLFacePtr fptr1, DLFLEdgePtr eptr2,
                                  DLFLFacePtr fptr2, bool check) {
  // Connect 2 half-edges
  ...
  if (check) {
    ...
    if ((c1->next() == c2n) || (c1->prev() == c2n)) {
      ...
      insertEdgeWithoutCheck(
          obj, c1, c2n); // Use version which doesn't check for self loops
    }
    if ((c1n->next() == c2) || (c1n->prev() == c2)) {
      // Edge already exists. Don't insert a new edge
    } else {
      insertEdgeWithoutCheck(
          obj, c1n, c2); // Use version which doesn't check for self loops
    }
  } else {
...
DLFLEdgePtrArray connectFaces(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
...
void dualConnectFaces(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
                      DLFLFaceVertexPtr fvptr2) {
  // Connect two faces using repeated insertEdge operations
  // All newly created faces will be triangles.
  // Every vertex in the new face 1 will be connected to 2 adjacent vertices in
  // face 2 and every vertex in face 2 will be connected to 2 adjacent vertices
  // in face 1. The 2 corners have to belong to different faces
  if (coFacial(fvptr1, fvptr2))
    return;
...
void connectFaces(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
                  DLFLFaceVertexPtr fvptr2, int numsegs, int maxconn,
  ...
  if (abs(pinch) < 0.0001 && pinchCenter == 0.5)
    pinch = 0.0001;
  ...
  for (int i = 1; i < numsegs; ++i) {
    ...
    if (t <= pinchCenter) // use the bezier equation for the first portion of
                          // the handle
    {
      t2 = (t / pinchCenter);  // t2 goes from 0 to 1
      t2 = pow(t2, bubbleExp); // the bubble factor affects the speed we travel
                               // on the bezier curve
    ...
    nfvp2 = nf2->firstVertex();
    nfvp3 = nf3->firstVertex();
...
void connectFaces(DLFLObjectPtr obj, DLFLFacePtr fp1, DLFLFacePtr fp2,
                  int numsegs) {
  ...
  if (fvp1 && fvp2)
    connectFaces(obj, fvp1, fvp2, numsegs);
}

void ScherkCollinsHandle(DLFLObjectPtr obj, DLFLEdgePtrArray new_edges,
                         double pinch, double pinch_center, double pinch_width,
                         int twist /*=1*/, int init_skip /*=1*/,
  ...
  for (int i = 0; i < new_edges.size(); ++i) {
    ...
    if (face_1 == face_3)
      new_faces.push_back(face_1);
    else if (face_1 == face_4)
      new_faces.push_back(face_1);
    else if (face_2 == face_3)
      new_faces.push_back(face_2);
  ...
  if (new_faces.size() < CS_VertCount)
    return;
  for (int i = init_skip; i < new_faces.size() / 2; i += (skip + 1)) {
    DLFLFacePtr face_1 = new_faces[(i + twist) % CS_VertCount];
    DLFLFacePtr face_2 =
        new_faces[(CS_VertCount - 1 - i + init_skip + twist) % CS_VertCount];
    DLFLFaceVertexPtr fvp1, fvp2;
    DLFLFace::findClosestCorners(face_1, face_2, fvp1, fvp2);

    connectFaces(obj, fvp1, fvp2, pinch_num_segs, -1, pinch, pinch_center,
                 pinch_width);
  }
}
...
void hermiteConnectFaces(
    ...
    int numsegs, double wt1, double wt2, int maxconn, int numtwists,
    double pinch, double pinchCenter, double bubble,
    bool triangulate_newface /*=false*/, bool scherk_collins /*=false*/,
    double sc_pinch /*=1.0*/, double sc_pinch_center /*=0.5*/,
    double sc_pinch_width /*=1.0*/, int hole_twist /*=1*/, int init_skip /*=1*/,
    int skip /*=0*/, int hole_seg_skip /*=0*/, int pinch_num_segs /*=2*/) {
  if (numsegs == 1) {
    DLFLEdgePtrArray new_edges = connectFaces(obj, fvptr1, fvptr2);
    ScherkCollinsHandle(obj, new_edges, sc_pinch, sc_pinch_center,
                        sc_pinch_width, hole_twist, init_skip, skip,
                        pinch_num_segs);
    return;
  }

  // Do hermite interpolated handle. Centroid is interpolated using hermite
  // interpolation Vertices are computed in relation to the centroid with linear
  // interpolation from first face to second face
  ...
  // x-axis : vector from centroid (origin) to the midpoint of the last edge in
  // the first polygon y-axis : use x-axis and normal to the plane (rotplane) to
  // find y axis Note that both polygons are already centered at the origin and
  ...
  // If either of the 2 polygons has a normal pointing in exactly the opposite
  // direction as the normal to the rotation plane, it can't be rotated to the
  // rotation plane and hence the handle can't be created without additional
  // information
  ...
  nfvp1 = fvptr1;
  verts.resize(numverts);
  angle.resize(numverts);
  distance.resize(numverts);
  ...
  p = cen1;
  v = n1;
  dvdt = 6.0 * (cen2 - cen1) - 4.0 * n1 - 2.0 * n2;
  ...
  for (int i = 1; i < numsegs; ++i) {
    ...
    // Do hermite interpolation on centroids and normals to find interpolated
    // point and normal
    ...
    // Rotate reconstructed polygon from rotation plane (XY plane) to plane with
    // v as normal Rotation is done around centroid of polygon
    ...
    if (numconnected < maxconn) {
      ++numconnected;
      if (triangulate_newface)
        dualConnectFaces(obj, nfvp1, nfvp2);
      else {
        DLFLEdgePtrArray new_edges = connectFaces(obj, nfvp1, nfvp2);
        if (scherk_collins &&
            num_segments_processed % (hole_seg_skip + 1) == 0) {
          ScherkCollinsHandle(obj, new_edges, sc_pinch, sc_pinch_center,
  ...
  if (numconnected < maxconn) {
    if (triangulate_newface)
      ...
      if (scherk_collins) {
        ScherkCollinsHandle(obj, new_edges, sc_pinch, sc_pinch_center,
                            sc_pinch_width, offset);
      }
    }
  }
}

void bezierConnectFaces(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
                        DLFLFaceVertexPtr fvptr2, int numsegs, double wt1,
                        double wt2) {

  // Do hermite interpolated handle. All vertices are interpolated using hermite
  // interpolation
  ...
  // Get the normals to the 2 faces. Second face is already reversed so we don't
  // have to fix the normal
  Vector3d n1, n2;
  n1 = wt1 * f1->computeNormal();
  n2 = wt2 * f2->computeNormal();
...
void hermiteConnectFaces2(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
                          DLFLFaceVertexPtr fvptr2, int numsegs, double wt1,
                          double wt2, int maxconn, int numtwists, double pinch,
                          double pinchCenter, double bubble,
                          bool triangulate_newface, bool scherk_collins,
                          double sc_pinch, double sc_pinch_center,
                          double sc_pinch_width, int hole_twist, int init_skip,
                          int skip, int hole_seg_skip, int pinch_num_segs,
  ...
  Vector3d bezier_control_point[] = {cen1, cen1 + n1 * wt1 / 3,
                                     cen2 - n2 * wt2 / 3, cen2};
  ...
  for (int i = 1; i < numsegs; ++i) {
    ...
    std::cout << "        "
              << "p: " << STRING(p[0]) << " " << STRING(p[1]) << " "
              << STRING(p[2]) << '\n';
    std::cout << "        "
              << "v: " << STRING(v[0]) << " " << STRING(v[1]) << " "
              << STRING(v[2]) << '\n';
    std::cout << "Bezier interpolation: " << '\n';
    std::cout << "        "
              << "p: " << STRING(pt[0]) << " " << STRING(pt[1]) << " "
              << STRING(pt[2]) << '\n';
    std::cout << "        "
              << "v: " << STRING(vt[0]) << " " << STRING(vt[1]) << " "
              << STRING(vt[2]) << '\n';
    std::cout << "-------------------------------------------------------------"
                 "-------------"
              << '\n';
