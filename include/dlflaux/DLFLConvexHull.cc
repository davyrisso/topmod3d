...
bool DLFLConvexHull::colinear(const Vector3d &p1, const Vector3d &p2,
                              const Vector3d &p3) {
  ...
  v2 = p3 - p1;
  area = v1 % v2;
  if (!isNonZero(area[0]) && !isNonZero(area[1]) && !isNonZero(area[2]))
    return true;
  return false;
}
...
int DLFLConvexHull::volumeSign(DLFLFacePtr face, const Vector3d &p) {
  if (face) {
    ...
    // Find determinant of above matrix. 1/6th of this the volume of the
    // tetrahedron
...
bool DLFLConvexHull::doubleTriangle() {
  ...
  triangle.resize(3);
  triangle[0] = iv1.point;
  triangle[1] = iv2.point;
  triangle[2] = iv3.point;

  // Create two "twin" faces
  createFace(triangle);

  DLFLFacePtr tfp =
      firstFace(); // Face which is in the given order (iv1,iv2,iv3)
  ...
  for (i = 0; i < numverts; ++i) {
    ...
    if (vertices[i].processed == false && volsign != 0) {
      ...
      // Even if the first vertex in the array is part of the initial triangle,
      // it doesn't affect anything, since it's already been processed and will
      // not be considered again
      InputVertex temp = vertices[0];
      temp.index = vertices[0].index; // Esan
      vertices[0] = vertices[i];
      vertices[i] = temp;
      vertices[0].index = vertices[i].index;
      vertices[i].index = temp.index; // Esan
      break;
    }
  }
  if (i == numverts)
...
bool DLFLConvexHull::addOne(InputVertex &iv) {
  ...
  efirst = edge_list.begin();
  elast = edge_list.end();
  while (efirst != elast) {
    ep = (*efirst);
    ++efirst;
    ep->getFacePointers(fp1, fp2);
    if (fp1->flags == DLFLConvexHull::visible &&
        fp2->flags == DLFLConvexHull::visible) {
      // Interior edge : add edge to remove_list
      remove_list.push_back(ep);
    } else if (fp1->flags == DLFLConvexHull::visible ||
               fp2->flags == DLFLConvexHull::visible) {
  ...
  if (fp1->flags == DLFLConvexHull::invisible)
    conebase = fp2;
  else if (fp2->flags == DLFLConvexHull::invisible)
    conebase = fp1;
  else {
  ...
  // Adjust coordinates of the apex to be the new point we are adding to the
  // hull
  apex->coords = p;
...
bool DLFLConvexHull::constructHull() {
  ...
  // Go through list of InputVertices and add ones which have not been processed
  // yet
  InputVertex iv;
