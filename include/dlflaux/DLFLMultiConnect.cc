...
void tripleConnectFaces(DLFLObjectPtr obj, DLFLFacePtr fp1, DLFLFacePtr fp2,
                        DLFLFacePtr fp3) {
  // Connect 3 faces. Connects closest edges between each pair of faces
  // Only edges which make an angle of 45 deg or less with the normal to the
  // plane containing the centroids

  // Find centroids of the 3 faces
  Vector3d cen1, cen2, cen3, cen_cen;

  cen1 = fp1->geomCentroid();
  cen2 = fp2->geomCentroid();
  cen3 = fp3->geomCentroid();
  ...
  DLFLEdgePtrList
      cedges; // Use linked list, not array, to simplify erase operations
  DLFLFacePtrList
      cfaces; // Faces for candidate edges (we need half-edges to connect)

  //--- NOTE ---//
  // In the list of faces, the other face for the edge is stored, instead of the
  // face which defines the half-edge to be connected. This is because once two
  // half-edges are connected, that face pointer might no longer be valid, but
  // the other side will still be valid. This assumes that both sides of the
  // edge are not the same face
  ...
  */
  ...
  while (!cedges.empty()) {
    ...
    while (epfirst != eplast) {
      ...
      if (ofp != temp_ofp) { // Different faces
        ...
        double cosangle =
            cos(5.0 * M_PI / 180.0); // Tolerance for parallel planes check
        double min_planarity = cosangle;
        fpcen = ofp->geomCentroid();
        temp_fpcen = temp_ofp->geomCentroid();
        n = normalized((fpcen - cen_cen) % (temp_fpcen - cen_cen));

        if ((hep.planarity > min_planarity) &&
  ...
  while (!heparray.empty()) {
    ...
    connectEdges(obj, hep.ep1, (hep.ep1)->getOtherFacePointer(hep.fp1), hep.ep2,
                 (hep.ep2)->getOtherFacePointer(hep.fp2));
    ...
    heparray.erase(
        remove_if(heparray.begin(), heparray.end(), contains_connected),
        heparray.end());
  }
}
  ...
  while (!cedges.empty()) {
    ...
    while (epfirst != eplast) {
      ...
      if (ofp != temp_ofp) { // Different faces
        ...
        if (isNonZero(normsqr(n)) && (hep.planarity > min_planarity) &&
            (Abs(n * hep.normal) < cosangle)) {
          // Add this half-edge pair to the array
          heparray.push_back(hep);
        } else {
...
void multiConnectFaces(DLFLObjectPtr obj, DLFLFacePtrArray fparray,
                       double scale_factor, double extrude_dist,
                       bool use_max_offsets) {
  // Connect multiple faces using convex hull method
  // scale specifies scale factor for vertices before convex hull is created
  // extrude specified distance faces should be extruded before convex hull is
  // created

  // First get vertices from all selected faces.
  // If two vertices are geometrically concident, only one of them is used for
  // convex hull
  ...
  for (int i = 0; i < num_sel_faces; ++i) {
    ...
    // Add points from face_vertices to vertices checking for coincident
    // vertices maxdist is the square of max distance between 2 points for them
    // to be considered coincident. Also extrude the points if extrude_dist is
    // non-zero
  ...
  // Scale the points for convex hull with centroid as origin using given scale
  // factor Scaling is done only if scale_factor is non-zero and not equal to 1
  ...
  // If convex hull is interior to the original object, it has to be reversed
  // before making further connections. This can be tested by checking normals
  // of original faces If the normals point towards the centroid of the convex
  // hull, no reversal is needed. Only one face needs to be checked
  Vector3d v1 = fparray[0]->normalCentroid();
  Vector3d v2 = fparray[0]->geomCentroid() - centroid(vertices);
  normalize(v2);
  if (v1 * v2 > 0.0)
    convexhull.reverse();

  // Go through selected faces and faces of convex hull and find matching ones
  // to make connections. A match is when a selected face and a face in the
  // convex hull are parallel to each other AND are facing each other.
  DLFLFacePtrArray chfparray, fp1array, fp2array;
  DLFLFacePtr sfp, chfp;
  Vector3d sfpn, chfpn;

  fp1array.reserve(num_sel_faces);
  fp2array.reserve(num_sel_faces);
  ...
  for (int i = 0; i < num_ch_faces; ++i) {
    ...
    for (int j = 0; j < num_sel_faces; ++j) {
      ...
      if (!isNonZero(1.0 + chfpn * sfpn)) {
        ...
        fp1array.push_back(sfp);
        fp2array.push_back(chfp);
        break;
      }
    }
  }
...
void multiConnectFaces(DLFLObjectPtr obj, DLFLFacePtrArray fparray,
                       double min_factor, bool make_connections) {
  ...
  // hull in the current iteration we stop. Adjustment factor will never go
  // below the min_factor.
  ...
  // centroid of each face to the centroid of all the face-centroids and
  // continuing till we reach the stop criterion described above.

  vector<Vector3dArray>
      vertices; // 2D Array of vertices in their original positions
  Vector3dArray
      chvertices; // Translated/extruded vertices for computing convex hull
  Vector3dArray centroids;  // Overall centroid
  Vector3d global_centroid; // Centroid of each face
  Vector3dArray adj_vect;   // Normalized vectors from centroid of each face to
                            // global centroid
  DoubleArray
      adj_dist; // Distance from centroid of each face to global centroid
  DLFLFacePtr fp;
  int num_sel_faces = fparray.size();
  ...
  // If convex hull of initial vertices excludes any point, we don't proceed any
  // further
  DLFLConvexHull convexhull;
  ...
  bool ptinside =
      false; // Was any point inside convex hull in current iteration?
  ...
  double
      efactor_diff;   // Change in efactor from this iteration to next iteration
  ...
  // If we haven't converged within max. no. of iterations, print out error
  // message and return.
  if (converged == false) {
    cout << "Could not find suitable convex hull within " << max_iters
         << " iterations." << endl;
  ...
  // of original faces If the normals point towards the centroid of the convex
  // hull, no reversal is needed. Only one face needs to be checked
  if (fparray[0]->normalCentroid() * adj_vect[0] > 0.0)
    convexhull.reverse();
...
Find the maximum distance by which each of the given
  ...
  */
  ...
  for (int i = 0; i < numfaces; ++i) {
    ...
    fparray[i]->getEdges(edges);
    facenormal = fparray[i]->computeNormal();
    numedges = edges.size();
    ...
    for (int j = 0; j < numedges; ++j) {
      ...
      edgevec = edges[j]->getEdgeVector();
      normalize(edgevec);
      normal = edgevec % facenormal;
      normalize(normal);
  ...
  for (int i = 0; i < numfaces; ++i) {
    facenormal =
        fparray[i]->normal; // Normal would have been computed in previous loop
...
void multiConnectMidpoints(DLFLObjectPtr obj) {
  ...
  while (vl_first != vl_last && num_verts < num_old_verts) {
    ...
    while (fvp_first != fvp_last) {
      ...
      if (fvp1->getVertexPtr() == fvp1->next()->getVertexPtr())
        fvp1 = (fvp1->next())->next();
      else
        fvp1 = fvp1->next();
      if (fvp2->getVertexPtr() == fvp2->prev()->getVertexPtr())
        fvp2 = (fvp2->prev())->prev();
      else
        fvp2 = fvp2->prev();
  ...
  DLFLFaceVertexPtr fvp,
      fvplink[2];   // Corners which are to be connected for each midpoint
...
void multiConnectCrust(DLFLObjectPtr obj, double scale_factor) {
  ...
  for (int i = 0; i < num_holes; ++i) {
    ...
    fvp1 = exfp1->firstVertex();
    fvp2 = fp2->firstVertex();
...
void modifiedMultiConnectCrust(DLFLObjectPtr obj, double scale_factor) {
  ...
  while (count < num_old_verts) {
    ...
    for (int i = 0; i < (int)eparray.size(); ++i) {
      ...
      if (ovp->getType() != VTNewPoint)
        ...
        ovp->resetType(); // Reset the type to allow proper recursive operation
    }
  }
  ...
  for (int i = 0; i < num_holes; ++i) {
    fp1 = crustfp1[i];
    fp2 = crustfp2[i];
    fvp1 = fp1->firstVertex();
    fvp2 = fp2->firstVertex();
    connectFaces(obj, fvp1, fvp2, 1);
  }
}

void createSponge(DLFLObjectPtr obj, double thickness,
                  double collapse_threshold_factor) {
  ...
  DLFLEdgePtrArray eplist1,
      eplist2; // List of edges to be connected (for inner shell)
  DLFLFacePtrArray fplist1,
      fplist2; // List of faces to be connected (between outer and inner shell)
  ...
  fplist1.resize(num_old_faces, NULL);
  fplist2.resize(num_old_faces, NULL);
  ...
  while (fl_first != fl_last && num_faces < num_old_faces) {
    ...
    duplicateFacePlanarOffset(obj, fp, -thickness, 0.0, thickness,
    ...
    // With respect to the outer shell, newfp1 faces inwards and newfp2 faces
    // outwards When creating the inner shell we want to reverse the surface The
    // edge connections will be made keeping this in mind
    ...
    fp->getEdges(eparray1);
    newfp2->getEdges(eparray2);
    ...
    for (int i = 0; i < (int)eparray1.size(); ++i) {
      ...
      if (eplist1[edgeindex] == NULL)
        eplist1[edgeindex] = ep2;
      else
        eplist2[edgeindex] = ep2;
    }
    ...
    newfp2 = extrudeFacePlanarOffset(obj, fp, 0.0, 0.0, thickness,
                                     fractional_thickness);
  ...
  // The correct half-edge is determined by the type tag which was set
  // previously
          ...
          for (int j = 0; j < (int)eparr2.size(); ++j) {
            ...
            if (tep2 != ep && tep2 != tep1) {
              ...
              if (checkIntersection(tep1, tep2)) {
                ...
                colledges.push_back(ep);
                i = eparr1
                        .size(); // To make sure we break out of outer loop also
                break;
              }
            }
          }
  ...
  el_first = colledges.begin();
  el_last = colledges.end();
  while (el_first != el_last) {
    ep = (*el_first);
    ++el_first;
...
bool contains_connected(const HalfEdgePair &hep) {
  if (hep.ep1 == HalfEdgePair::last_connected1 ||
      ...
      hep.ep2 == HalfEdgePair::last_connected2)
    return true;
  return false;
}
