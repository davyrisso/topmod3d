...
void createConvexHull(DLFLObjectPtr obj) {
  ...
  DLFLVertexPtrList::const_iterator first = obj->beginVertex(),
                                    last = obj->endVertex();
  int i = 0;
  while (first != last) {
    ...
    for (int iy = 0; iy < SLICENUM[1]; iy++) {
      ...
      for (int iz = 0; iz < SLICENUM[2]; iz++) {
        ...
        for (int i = 0; i < ovarray.size(); i++)
          if ((ovarray.at(i)[0] > x) &&
              (ovarray.at(i)[0] < x + step[0] + EPS) &&
              (ovarray.at(i)[1] > y) &&
              (ovarray.at(i)[1] < y + step[1] + EPS) &&
...
DLFLObjectPtr createDualConvexHull(DLFLObjectPtr obj,
                                   const Vector3dArray &ovarray) {

  DLFLVertexPtrArray cverts;

  if (convexhull == NULL)
    convexhull = new DLFLConvexHull();
  ...
  // Create top face
  p.set(cvmax[0], cvmax[1], cvmax[2]);
  verts[0] = p;
  p.set(cvmax[0], cvmax[1], cvmin[2]);
  verts[1] = p;
  p.set(cvmin[0], cvmax[1], cvmin[2]);
  verts[2] = p;
  p.set(cvmin[0], cvmax[1], cvmax[2]);
  verts[3] = p;
  cube->createFace(verts);
  DLFLFacePtr fp1 = cube->lastFace();

  // Create bottom face
  p.set(cvmax[0], cvmin[1], cvmax[2]);
  verts[0] = p;
  p.set(cvmin[0], cvmin[1], cvmax[2]);
  verts[1] = p;
  p.set(cvmin[0], cvmin[1], cvmin[2]);
  verts[2] = p;
  p.set(cvmax[0], cvmin[1], cvmin[2]);
  verts[3] = p;
  cube->createFace(verts);
...
void visitVertex(DLFLObjectPtr obj, DLFLVertexPtr vp, DLFLEdgePtr from,
                 Vector3d normal, Vector3d P0) {
...
void localCut(DLFLObjectPtr obj, DLFLVertexPtr vp, Vector3d normal,
              Vector3d P0) {
...
// void localCut( DLFLObjectPtr obj, DLFLVertexPtr vp,Vector3d normal,Vector3d
// P0 ){
//
// 	DLFLVertexPtrArray verts;
//
...
void performCutting(DLFLObjectPtr obj, int type, float offsetE, float offsetV,
                    bool global, bool selected) {
  ...
  for (int i = 0; i < esize; i++) {
    ...
    if (((selected) && (!e->ismarked)) || (type == 201))
      continue;
    ...
    for (int j = 0; j < 2; j++) {
      ...
      for (int k = 0; k < vedges.size(); k++) {
        ...
        vlocs[vnum++] =
            (ev2->getCoords() - ev1->getCoords()) * offsetE + ev1->getCoords();
      }
    }
  ...
  for (int i = 0; i < vsize; i++) {
    ...
    for (int k = 0; k < vedges.size(); k++) {
      ...
      vlocs[vnum++] =
          (ev2->getCoords() - ev1->getCoords()) * offsetV + ev1->getCoords();
    }
    ...
    for (int i = 0; i < fsize; i++) {
      ...
      for (int j = 0; j < fverts.size(); j++) {
        ...
        for (int k = 0; k < vedges.size(); k++) {
          ...
          vlocs[vnum++] = (ev2->getCoords() - ev1->getCoords()) * offsetV +
                          ev1->getCoords();
        }
      }
...
void cutSelectedVertices(DLFLObjectPtr obj, float offsetE, float offsetV,
  ...
  for (vit = svptrarr.begin(); vit != svptrarr.end(); vit++, cutcount++) {
    ...
    for (eit = vedges.begin(); eit != vedges.end(); eit++) {
      ...
      vlocs[vnum++] =
          (vp2->getCoords() - vp1->getCoords()) * offsetV + vp1->getCoords();
    }
