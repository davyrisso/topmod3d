...
void createDual(DLFLObjectPtr obj, bool accurate) {
  ...
  if (accurate) {
    ...
    obj->getVertexPtrID(16)->getFaceVertexList(); for
    (DLFLFaceVertexPtrList::iterator it = fvpList.begin(); it != fvpList.end();
    ++it){ cout << (*it)->next()->getVertexPtr()->getID() << ",";
    }
    cout << endl;
    ...
    */
    ...
    vpfirst = obj->beginVertex();
    vplast = obj->endVertex();
