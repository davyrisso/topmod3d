...
public:
  ...
  // Default constructor
  HalfEdgePair()
      : ep1(NULL), ep2(NULL), fp1(NULL), fp2(NULL), mpdist(0.0), planarity(0.0),
        normal() {}

  // Constructor
  HalfEdgePair(DLFLEdgePtr eptr1, DLFLEdgePtr eptr2, DLFLFacePtr fptr1,
               DLFLFacePtr fptr2)
      : ep1(eptr1), ep2(eptr2), fp1(fptr1), fp2(fptr2), mpdist(0.0),
        planarity(0.0), normal() {
    computeMetrics();
  }
  ...
  void computeMetrics(void) {
    if (ep1 && ep2) {
      ...
      // Takes average of normals at any two opposite corners formed with
      // adjacent corners. Rotation order is defined by associated face and not
      // order of vertices in edge
      DLFLFaceVertexPtr e1fvp1, e1fvp2, e2fvp1, e2fvp2;

      e1fvp1 = ep1->getFaceVertexPtr(ep1->getOtherFacePointer(fp1));
      e1fvp2 = e1fvp1->next();
      e2fvp1 = ep2->getFaceVertexPtr(ep2->getOtherFacePointer(fp2));
      e2fvp2 = e2fvp1->next();

      Vector3d v11, v12, v21, v22; // Corners
      Vector3d av11, av12, av21,
          av22;          // Adjusted coordinates of corners - see below
      Vector3d n11, n21; // Normals at 2 opposite corners

      v11 = e1fvp1->getVertexCoords();
      v12 = e1fvp2->getVertexCoords();
      v21 = e2fvp1->getVertexCoords();
      v22 = e2fvp2->getVertexCoords();
      ...
      av11 = 0.75 * v11 + 0.25 * v12;
      av12 = 0.25 * v11 + 0.75 * v12;
      av21 = 0.75 * v21 + 0.25 * v22;
      av22 = 0.25 * v21 + 0.75 * v22;
  ...
  // Comparison between two HalfEdgePairs. Uses distance to determine relative
  // ordering
  friend bool less_than(const HalfEdgePair &hep1, const HalfEdgePair &hep2);
  /*
      friend bool less_than(const HalfEdgePair& hep1, const HalfEdgePair& hep2)
     { if ( hep1.mpdist < hep2.mpdist ) return true; return false;
      }
  */
  // Comparison between two HalfEdgePairs. Uses distance to determine relative
  // ordering
  ...
  /*
      friend bool greater_than(const HalfEdgePair& hep1, const HalfEdgePair&
     hep2) { if ( hep1.mpdist > hep2.mpdist ) return true; return false;
      }
  */
  ...
  */
  ...
  void print(void) const {
    if (ep1 && ep2)
      cout << "{" << ep1->getID() << ","
           << (ep1->getOtherFacePointer(fp1))->getID() << "}"
           << " <--> "
           << "{" << ep2->getID() << ","
           << (ep2->getOtherFacePointer(fp2))->getID() << "}"
...
public:
  ...
  Plane(const Plane &plane) : origin(plane.origin), normal(plane.normal) {}
  ...
  // Computes parametric distance along v with p as origin and returns it as
  // parameter Returns false if there is no intersection, true otherwise
...
void multiConnectFaces(DLFLObjectPtr obj, DLFLFacePtrArray fparray,
                       ...
                       bool use_max_offsets = false);
void multiConnectFaces(DLFLObjectPtr obj, DLFLFacePtrArray fparray,
                       double min_factor, bool make_connections = true);
...
void createSponge(DLFLObjectPtr obj, double thickness,
                  double collapse_threshold_factor = 0.0);
