...
DLFLFacePtrArray duplicateFace(DLFLObjectPtr obj, DLFLFacePtr fptr,
...
DLFLFacePtrArray duplicateFace(DLFLObjectPtr obj, DLFLFacePtr fptr,
                               const Vector3d &dir, double offset, double rot,
                               double sf) {
  ...
  if (head) {
    ...
    // Scale the new vertices about their centroid if scale factor is not 1.0 or
    // 0.0
    sf = Abs(sf);
...
DLFLFacePtr duplicateFacePlanarOffset(DLFLObjectPtr obj, DLFLFacePtr fptr,
  ...
  Vector3d dir = fptr->computeNormal();
  return duplicateFacePlanarOffset(obj, fptr, dir, offset, rot, thickness,
                                   fractionalthickness);
}

DLFLFacePtr duplicateFacePlanarOffset(DLFLObjectPtr obj, DLFLFacePtr fptr,
                                      const Vector3d &dir, double offset,
  ...
  if (head) {
    ...
    // Offset the corners by thickness using the edge vectors to determine
    // direction If boolean flag is set thickness is assumed to be a fraction of
    // edge length. Traverse the face and the coordinate array simultaneously
    ...
    if (fractionalthickness) {
      ...
      do {
        ...
        // If fvp is a winged corner we need to find the new coordinates
        // differently
...
/*DLFLFacePtr extrudeFacesTogether(DLFLObjectPtr obj, DLFLFacePtrArray fparray,
double d) {

        //need an array of old vertices, new vertices, and which ones are
outside, and which ones need to be connected DLFLVertexArray old_verts;
        DLFLVertexArray new_verts;

        //also need to delete edges inside the regions that will be kept
together
...
DLFLFacePtr extrudeFaceScherkCollins(
    DLFLObjectPtr obj, DLFLFacePtr fptr, double d, int num, double rot /*=0.0*/,
    double sf /*=1.0*/, bool triangulate_new_faces /*=false*/,
    bool scherk_collins /*=false*/, double pinch /*=1.0*/,
    double pinch_center /*=0.5*/, double pinch_width /*=1.0*/) {
    ...
    if (theface != NULL) {
      ...
      if (triangulate_new_faces)
        ...
        if (scherk_collins && endface->size() % 2 == 0)
          ScherkCollinsHandle(obj, new_edges, pinch, pinch_center, pinch_width,
...
DLFLFacePtr extrudeFace(DLFLObjectPtr obj, DLFLFacePtr fptr, double d,
                        const Vector3d &dir, double rot, double sf) {
...
DLFLFacePtr extrudeFace(DLFLObjectPtr obj, DLFLFacePtr fptr, double d,
                        const Vector3d &dir, int num, double rot, double sf) {
  DLFLFacePtr exface = fptr;
  for (int i = 0; i < num; ++i) {
...
DLFLFacePtr extrudeFacePlanarOffset(DLFLObjectPtr obj, DLFLFacePtr fptr,
  ...
  return extrudeFacePlanarOffset(obj, fptr, d, dir, rot, thickness,
                                 fractionalthickness);
}

DLFLFacePtr extrudeFacePlanarOffset(DLFLObjectPtr obj, DLFLFacePtr fptr,
                                    double d, const Vector3d &dir, double rot,
                                    double thickness,
                                    bool fractionalthickness) {
  DLFLFacePtr endface = duplicateFacePlanarOffset(
      obj, fptr, dir, d, rot, thickness, fractionalthickness);
  if (endface != NULL) {
    // Get the pointer to the second newly inserted face
    DLFLFacePtr nfp = obj->lastFace();
...
DLFLFacePtr extrudeFaceDS(DLFLObjectPtr obj, DLFLFacePtr fptr, double d,
                          const Vector3d &dir, double twist, double sf) {
  ...
  if (head) {
    ...
    newverts[numverts - 1] +=
        (1.0 - twist) * oldverts[numverts - 1] + twist * oldverts[0];
    ...
    for (int i = 0; i < numverts; ++i) {
      ...
      for (int j = 0; j < numverts; ++j) {
        if (i == j)
          ...
          coef = (3.0 + 2.0 * cos(2.0 * (i - j) * M_PI / numverts)) /
                 (4.0 * numverts);
        p += coef * newverts[j];
      }
      oldverts[i] = p;
...
DLFLFacePtr extrudeFaceDS(DLFLObjectPtr obj, DLFLFacePtr fptr, double d,
                          const Vector3d &dir, int num, double twist,
                          double sf) {
  DLFLFacePtr exface = fptr;
  for (int i = 0; i < num; ++i) {
...
DLFLFacePtr extrudeDualFace(DLFLObjectPtr obj, DLFLFacePtr fptr, double d,
...
DLFLFacePtr extrudeDualFace(DLFLObjectPtr obj, DLFLFacePtr fptr, double d,
  ...
  if (head) {
    ...
    if (mesh)
      fptr->getEdges(ep_arr);

    // New face will contain the midpoints of edges of old face, appropriately
    // transformed
...
void doubleStellateFace(DLFLObjectPtr obj, DLFLFacePtr fptr, double d) {
  ...
  // of the new faces. Midpoint is then moved by specified distance along
  // normal.
  Vector3d dir = fptr->computeNormal();
...
//--- Additions by Eric ---//
DLFLFacePtr extrudeFaceDodeca(DLFLObjectPtr obj, DLFLFacePtr fptr, double d,
...
DLFLFacePtr extrudeHexTileFace(DLFLObjectPtr obj, DLFLFacePtr fptr, double d,
                               double rot, double sf) {
...
DLFLFacePtr extrudeFaceDodeca(DLFLObjectPtr obj, DLFLFacePtr fptr, double d,
                              double rot, double sf) {
  // Dodecahedral extrusion
  // Sequence of extrude, dual extrude and extrude followed by deletion of
  // lateral edges
  DLFLFacePtr exface = fptr;
  DLFLEdgePtrArray ep_arr;
...
DLFLFacePtr extrudeFaceDodeca(DLFLObjectPtr obj, DLFLFacePtr fptr, double angle,
                              int num, double ex_dist1, double ex_dist2,
                              double ex_dist3, bool hexagonalize) {
  // Dodecahedral extrusion
  DLFLFacePtr exface = fptr;
  DLFLEdgePtrArray ep_arr;
  if (hexagonalize) {
    ...
    exface =
        extrudeFaceDodeca(obj, exface, angle, ex_dist1, ex_dist2, ex_dist3);
...
DLFLFacePtr extrudeDodeca_Symmetric(DLFLObjectPtr obj, DLFLFacePtr fptr,
                                    double d, double rot, double sf,
                                    int modfactor) {
  // int num_faces;
  // 		int num_old_faces;
  // 		int face_counter;
  ...
  // 		DLFLFacePtr fp;
  // 		DLFLFacePtrList :: iterator fl_first, fl_last, fl_exfirst,
  // fl_exlast; 		DLFLFacePtr retface; 		Vector3d dir;
  //
  ...
  // 				normalize(dir);
  // 				//FIND AV. LENGTH OF BASE
  // EDGE************************************ 				av_edge_length = 0; 				face_sides =
  // fp->numFaceVertexes(); 				fp->getVertexCoords(base_verts);
  //
  ...
  // 				//DLFLFacePtr endface = duplicateFace(fp,dir,  av_edge_length *
  // d,rot,sf); 				DLFLFacePtr endface = extrudeFaceDodeca(obj, fp, 45.0, 1, 1, 1,
  // 1, false);
  //
  // 				retface = endface;
  ...
  // 					endface->symm_group = current_max_symm_group +
  // modoffset; 					MakeNewMaterial(endface, current_max_symm_group + modoffset);
  ...
  // 					count_counter++;
  // 					modoffset = count_counter % modulus + 2;
  // 					fp->symm_group = current_max_symm_group +
  // modoffset; 					MakeNewMaterial(fp, current_max_symm_group + modoffset);
  // 					if(max_symm_group < fp->symm_group)
  // 						max_symm_group = fp->symm_group;
  // 						/*
  // 									// Get the pointer to the second newly inserted
  // face 					DLFLFacePtr nfp = face_list.back();
  //
  // 									// The last face will be the one facing the old
  // face
  // 									// Find the first face-vertices in the 2 faces
  // to be connected 					DLFLFaceVertexPtr fvp1, fvp2; 					fvp1 = fp->firstVertex();
  // fvp2 = nfp->firstVertex(); 					connectFaces(fvp1,fvp2);
  // 						*/
  ...
  //
  // 					count_counter = 3;
  // 						//count_counter += num_extrudefaces -
  // num_oldexfaces; 					num_excounter = 0; 					while(num_excounter < num_extrudefaces
  // && fl_exfirst != fl_exlast) { 						if(num_excounter >= num_oldexfaces) { 							fp =
  // (*fl_exfirst);
  // 								//use the modulus to determine new face
  // group 							modoffset = count_counter % modulus + 2;
  //
  // 							if( count_counter == 3)
  // { 								fp->symm_group = current_max_symm_group + 1; 								MakeNewMaterial(fp,
  // current_max_symm_group + 1);
  // 							}
  // 							else {
  // 								fp->symm_group = current_max_symm_group +
  // modoffset; 								MakeNewMaterial(fp, current_max_symm_group + modoffset);
  ...
  //
  // 							if(max_symm_group <
  // fp->symm_group) 								max_symm_group = fp->symm_group;
  // 								//fp->symm_group = (symmetry_id-1) *
  // (num_extrudefaces - num_oldexfaces) + count_counter + 1;
...
DLFLFacePtr extrudeFaceDodeca(DLFLObjectPtr obj, DLFLFacePtr fptr, double angle,
                              double ex_dist1, double ex_dist2,
                              double ex_dist3) {
  // Dodecahedral extrusion
  // Sequence of extrude, dual extrude and extrude followed by deletion of
  // lateral edges
  DLFLFacePtr exface = fptr;
  DLFLEdgePtrArray ep_arr;
  DLFLVertexPtrList ::iterator vl_first, vl_last, vl_current, vl_ring_1,
      vl_ring_2, vl_ring_3;
  DLFLVertexPtr vp;
  ...
  for (i = 0; i < face_sides; i++) {
    ...
    l1 = l_base;
    // l1 *= (ex_dist1/cos(alpha));  //defines the height based on the dihedral
    // angle
    l1 *=
        ex_dist1; // changes size for first set of lines, ignores dihedral angle
  ...
  for (i = 0; i < face_sides; i++) {
    ...
    // find the length of one side
    l1 = norm(enext);
    /*l1 = (l1*0.5)*(1./cos(M_PI*54./180.)+tan(M_PI*54./180.) ) * ex_dist2;
     * //PI/5  (36 degrees)*/
...
DLFLFacePtr extrudeFaceIcosa(DLFLObjectPtr obj, DLFLFacePtr fptr, double angle,
                             int num, double ex_dist1, double ex_dist2,
                             double ex_dist3) {
...
DLFLFacePtr extrudeFaceIcosa(DLFLObjectPtr obj, DLFLFacePtr fptr, double angle,
  ...
  while (el_first != el_last) {
    ...
    //      stellate_amount = center_to_vert - normsqr(first_face_centroid -
    //      center_sphere2); started = true;
    //    }
  ...
  // RINGS ARE SET UP*****************************************
  Vector3d vnext, vprev, finaldir, facenext, faceprev, facenext_2, faceprev_2,
      vnext_2, vprev_2;
  Vector3d halfv;
  ...
  for (i = 0; i < face_sides; i++) {
    ...
    // l1 *= ex_dist1; //changes size for first set of lines, ignores dihedral
    // angle
    normalize(vnext);
    // facenext
  ...
  vl_current = vl_ring_3;
  for (i = 0; i < face_sides; i++) {
    // Vector3d GetRingPosition(DLFLVertexPtrList :: iterator vl_ring, int
    // position, int numfaces)
...
DLFLFacePtr extrudeFaceCubOcta(DLFLObjectPtr obj, DLFLFacePtr fptr,
                               double angle, int num, double ex_dist1,
                               double ex_dist2, double ex_dist3) {
  // Cuboctahedral extrusion
  DLFLFacePtr exface = fptr;
  // double sf3 = pow(sf, 1.0/2.0);
  for (int i = 0; i < num; ++i)
    exface =
        extrudeFaceCubOcta(obj, exface, angle, ex_dist1, ex_dist2, ex_dist3);
  return exface;
}

DLFLFacePtr extrudeFaceSmallRhombiCubOcta(DLFLObjectPtr obj, DLFLFacePtr fptr,
  ...
  // double sf3 = pow(sf, 1.0/2.0);
  for (int i = 0; i < num; ++i)
    exface = extrudeFaceSmallRhombiCubOcta(obj, exface, angle, ex_dist1,
                                           ex_dist2, ex_dist3);
  return exface;
}

DLFLFacePtr extrudeFaceCubOcta(DLFLObjectPtr obj, DLFLFacePtr fptr,
                               double angle, double ex_dist1, double ex_dist2,
                               double ex_dist3) {
  // Cuboctahedral extrusion
  DLFLFacePtr exface = fptr;
  DLFLFacePtr otherface;
...
DLFLFacePtr extrudeFaceSmallRhombiCubOcta(DLFLObjectPtr obj, DLFLFacePtr fptr,
  ...
  DLFLEdgePtrArray ::iterator el_first, el_last;
  DLFLEdgePtr ep;
  el_first = ep_arr_1.begin();
  el_last = ep_arr_1.end();
  while (el_first != el_last) {
    ep = (*el_first);
    ++el_first;
    subdivideEdge(obj, ep);
  }
  el_first = ep_arr_2.begin();
  el_last = ep_arr_2.end();
  while (el_first != el_last) {
    ep = (*el_first);
    ++el_first;
    subdivideEdge(obj, ep);
  ...
  // RINGS ARE SET UP*****************************************
  Vector3d vnext, vprev, finaldir, finaldir_2, facenext, faceprev, facenext_2,
      faceprev_2, vnext_2, vprev_2;
  Vector3d halfv, midpoint_tier2;
  ...
  for (i = 0; i < face_sides; i++) {
    vp2 = base_vps[i];
    vp = (*vl_current);
    insertEdge(obj, vp->getFaceVertexInFace(exface),
               vp2->getFaceVertexInFace(exface));
    vl_current++;
  }
  ...
  for (i = 0; i < face_sides; i++) {
    ...
    tempv *= 0.5;
    // got the average position, so that weird angles don't twist the
    // rhombicuboctahedron
    vp->setCoords(tempv);
    vl_current++;
  }
...
Vector3d GetRingPosition(DLFLVertexPtrList ::iterator vl_ring, int position,
                         int numfaces) {
  int count = position;
  int j;
  DLFLVertexPtrList ::iterator vl_temp;
...
DLFLVertexPtr GetRingVertex(DLFLVertexPtrList ::iterator vl_ring, int position,
                            int numfaces) {
  int count = position;
  int j;
...
// DLFLFacePtr extrudeFaceIcosa(DLFLObjectPtr obj, DLFLFacePtr fptr, double d,
// int num, double rot, double sf) {
// 		// Icosahedral extrusion
// 		DLFLFacePtr exface = fptr;
//
...
// dave
void extrudeFaceDome(DLFLObjectPtr obj, DLFLFacePtr fptr, double length,
                     double rot, double sf) {
...
/* API
         uint extrudeFace(DLFLObjectPtr obj, uint faceID, double d, int num,
   double rot, double sf ) { DLFLFacePtr fp = extrudeFace( obj,
   obj->findFace(faceID), d, num, rot, sf ); return fp->getID();
         }

         uint extrudeFaceDS(DLFLObjectPtr obj, uint faceID, double d, int num,
   double twist, double sf ) { }

         uint extrudeDualFace(DLFLObjectPtr obj, uint faceID, double d, int num,
   double rot, double sf, bool mesh ) { }

         uint extrudeFaceDodeca(DLFLObjectPtr obj, uint faceID, double d, int
   num, double rot, double sf, bool hexagonalize ) { }

         uint extrudeFaceIcosa(DLFLObjectPtr obj, uint faceID, double d, int
   num, double rot, double sf ) { }
