 ...
 *
 * \class Camera2
 *	\brief Camera class from Chris Root used as an alternative to the
 *original TopMod camera class
 *
 * has an error when the aim and position vectors are not at the same y-value.
 *\todo  fix the camera2 class up vector problem
 *
 */
 ...
 */
void ArbitraryRotate(Vector3d U, Vector3d V, Vector3d W, float degreeX,
                     float degreeY, Vector3d &point, Vector3d aim) {
  ...
  trans.set(Vector4d(1.f, 0.f, 0.f, -aim[0]), Vector4d(0.f, 1.f, 0.f, -aim[1]),
            Vector4d(0.f, 0.f, 1.f, -aim[2]), Vector4d(0.f, 0.f, 0.f, 1.f));

  Matrix4x4 mat;
  mat.set(Vector4d(U[0], U[1], U[2], 0.f), Vector4d(V[0], V[1], V[2], 0.f),
          Vector4d(W[0], W[1], W[2], 0.f), Vector4d(0.f, 0.f, 0.f, 1.f));
  ...
  rot.set(Vector4d(1, 0, 0, 0), Vector4d(0, cx, sx, 0), Vector4d(0, -sx, cx, 0),
          Vector4d(0, 0, 0, 1));

  pos = rot * pos;

  rot.set(Vector4d(cy, 0, sy, 0), Vector4d(0, 1, 0, 0), Vector4d(-sy, 0, cy, 0),
          Vector4d(0, 0, 0, 1));
...
/*!
 * \brief default constructor... sets position to 0, 0, 5, aimed at the origin
 * with the up vector set to the y axis
 */
Camera2::Camera2()
    : mousex(0), mousey(0), pickw(0), pickh(0), viewport(NULL),
      pickmode(false) {
 ...
 *
 */
Camera2::Camera2(Vector3d P, Vector3d A, Vector3d U, float Near, float Far,
                 float ViewAngle)
    : mousex(0), mousey(0), pickw(0), pickh(0), viewport(NULL),
      pickmode(false) {
 ...
 *
 * \note be careful with these because if you set either of them which causes
 * the orientation of the camera to be un-orthogonal, then you'll see
 * problems.just remember that (Aim - Pos).normalize() % Up == 0, or you'll see
 * problems
 *
 */
void Camera2::SetPos(Vector3d P) { Pos = P; }
 ...
 *
 */
void Camera2::SetAim(Vector3d A) { Aim = A; }
 ...
 *
 */
void Camera2::SetUp(Vector3d U) { Up = U; }
 ...
 *
 */
void Camera2::SetFOV(float ViewAngle) { Fov = ViewAngle; }
 ...
 *
 */
void Camera2::enterSelectionMode(double x, double y, double w, double h,
                                 GLint *vp) {
  mousex = x;
  mousey = y;
  pickw = w;
...
void Camera2::PerspectiveDisplay(int W, int H) {
  ...
  glLoadIdentity();
  gluLookAt(Pos[0], Pos[1], Pos[2], Aim[0], Aim[1], Aim[2], Up[0], Up[1],
            Up[2]);
}
 ...
 *
 */
void Camera2::HandleMouseEvent(Qt::MouseButton button, QEvent::Type state,
  ...
  if (state == QEvent::MouseButtonRelease && CameraMode != INACTIVE) {
    ...
    case Qt::MidButton:
      if (QApplication::keyboardModifiers() ==
          Qt::ShiftModifier) { // Stupid little hack since Mac makes alt+LMB ==
                               // MMB
        CameraMode = ROTATE;
        break;
      }
      ...
      gluProject(Aim[0], Aim[1], Aim[2], MvMatrix, ProjMatrix, ViewPort, &wx,
                 &wy, &wz);
      ...
      gluUnProject((GLfloat)x, (GLfloat)realy, wz, MvMatrix, ProjMatrix,
                   ViewPort, &mp0, &mp1, &mp2);
...
void Camera2::HandleMouseMotion(int x, int y) {
  ...
  if (CameraMode != INACTIVE) {
    ...
    case ROTATE:
      ...
      ArbitraryRotate(WindowX, WindowY, WindowZ, LocalDeltaElev, 0, Pos, Aim);

      ArbitraryRotate(Vector3d(1, 0, 0), Vector3d(0, 1, 0), Vector3d(0, 0, 1),
                      0, -LocalDeltaAzim, Pos, Aim);
