...
/* ******************* Camera ************************
        ...
        This can be set in the constructor as well, or with the SetZoom()
   function
...
public:
  ...
  virtual void enterSelectionMode(double x, double y, double w, double h,
                                  GLint *vp) = 0;
...
public:
  ...
  OrthoCamera(Vector3d eye, Vector3d center, Vector3d up)
      : zoom(5), Camera(eye, center, up){};
  OrthoCamera(Vector3d eye, Vector3d center, Vector3d up, float z)
      : zoom(z), Camera(eye, center, up){};
  ~OrthoCamera(){};
...
public:
  ...
  PerspCamera(Vector3d eye, Vector3d center, Vector3d up)
      : FOV(60), nearplane(0.1), farplane(500), Camera(eye, center, up){};
  PerspCamera(Vector3d eye, Vector3d center, Vector3d up, float fov, float n,
              float f)
      : FOV(fov), nearplane(n), farplane(f), Camera(eye, center, up){};
  ~PerspCamera(){};
