  ...
  void createList(void) {
    ...
    glNewList(disp_list_id, GL_COMPILE);
    {
      ...
      case XY:
        ...
        for (i = 0; i <= subdiv; ++i) {
          delta = float(i) * dd;
          glVertex3f(sx + delta, sy, 0.0);
          glVertex3f(sx + delta, ey, 0.0);
          glVertex3f(sx, sy + delta, 0.0);
          glVertex3f(ex, sy + delta, 0.0);
        }
        break;
      case YZ:
        sy = sz = -size / 2.0;
        ...
        for (i = 0; i <= subdiv; ++i) {
          delta = i * dd;
          glVertex3f(0.0, sy + delta, sz);
          glVertex3f(0.0, sy + delta, ez);
          glVertex3f(0.0, sy, sz + delta);
          glVertex3f(0.0, ey, sz + delta);
        }
        break;
      case ZX:
        sz = sx = -size / 2.0;
        ...
        for (i = 0; i <= subdiv; ++i) {
          delta = i * dd;
          glVertex3f(sx, 0.0, sz + delta);
          glVertex3f(ex, 0.0, sz + delta);
          glVertex3f(sx + delta, 0.0, sz);
          glVertex3f(sx + delta, 0.0, ez);
        }
        break;
      }
      glEnd();
