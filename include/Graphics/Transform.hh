...
public:
  ...
  // Compute the matrix equivalent to gluLookAt for given eye, center and
  // upvector
  static Matrix4x4 lookat(const Vector3d &eye, const Vector3d &center,
                          const Vector3d &up) {
    Matrix4x4 lmat;
  ...
  void operator*=(const Transformation &tr) { transform *= tr.transform; }

  void operator*=(const Matrix4x4 &mat) { transform *= mat; }
  ...
  void operator/=(const Matrix4x4 &mat) { transform = mat * transform; }
  ...
  friend Transformation operator*(const Matrix4x4 &mat,
  ...
  Vector3d applyTo(const Vector3d &vec) { return transform * vec; }

  // Apply transformations - pre-multiply by corresponding transformation
  // matrices
  ...
  Matrix4x4 matrix(void) { return transform; }

  // Reset the transformation matrix
  void reset(void) { transform.reset(); }

  // Set the transformation matrix
  void set(const Matrix4x4 mat) { transform = mat; }
};
