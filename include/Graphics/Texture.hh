  ...
  void readTexture(const char *imagefile) {
    if (imagefile) {
      ...
      if (status == iflOKAY) {
        ...
        iflConfig config(iflUChar, iflInterleaved, 0, NULL, 0,
                         iflLowerLeftOrigin);
...
public:
  Texture() : pixels(NULL), dims() {}

  Texture(const char *imagefile) : pixels(NULL), dims() {
    readTexture(imagefile);
  }

  Texture(const Texture &tex) : pixels(NULL), dims(tex.dims) {
    if (tex.pixels) {
      pixels = new unsigned char[dims.y * dims.x * dims.c];
  ...
  friend void glTexture(const Texture &tex) {
    ...
    if (tex.pixels) {
      if (tex.dims.c == 1)
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE_ALPHA, tex.dims.x,
                     tex.dims.y, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE,
                     tex.pixels);
