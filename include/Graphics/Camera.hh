...
public:
  Camera(double aspratio = 1.0)
      : center(0, 0, 0), eye(0, 0, 50), up(0, 1, 0), nearplane(1),
        farplane(1000), umin(-1), umax(1), vmin(-1), vmax(1), fovy(45.0),
        aspect(aspratio), projtype(Perspective), dist(50.0), mousex(0),
        mousey(0), pickw(0), pickh(0), viewport(NULL), pickmode(false) {}

  Camera(const Camera &cam)
      : center(cam.center), eye(cam.eye), up(cam.up), nearplane(cam.nearplane),
        farplane(cam.farplane), umin(cam.umin), umax(cam.umax), vmin(cam.vmin),
        vmax(cam.vmax), fovy(cam.fovy), aspect(cam.aspect),
        projtype(cam.projtype), dist(cam.dist), mousex(0), mousey(0), pickw(0),
        pickh(0), viewport(NULL), pickmode(false) {}
  ...
  void toggleProjectionType(void) {
    ...
    if (projtype == Perspective) {
      ...
      vmax = dist * tan(deg2rad(fovy / 2.0));
      vmin = -vmax;
      umax = aspect * vmax;
      umin = -umax;
    } else {
  ...
  double distance(void) const { return dist; }

  void setEye(const Vector3d &v) {
    eye = v;
    dist = norm(eye - center);
  }
  ...
  void setUpVector(const Vector3d &v) { up = v; }

  void setUpVector(double x, double y, double z) { up.set(x, y, z); }
  ...
  void setOrthographicViewVolume(double uwidth, double vheight,
                                 bool adjust = true) {
    umin = -Abs(uwidth) / 2.0;
    umax = -umin;
    vmin = -Abs(vheight) / 2.0;
    vmax = -vmin;
    if (adjust)
      aspect = umax / vmax;
  }
  ...
  void setAspect(double asp) { aspect = Abs(asp); }

  void setFieldOfViewY(double angle) { fovy = Abs(angle); }
  ...
  void applyTransform(void) const {
    ...
    if (projtype == Perspective)
      ...
      glOrtho(umin, umax, vmin, vmax, nearplane, farplane);
