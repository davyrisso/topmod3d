...
public:
  // Default constructor
  RGBColor() : BaseObject(), color(), r(color[0]), g(color[1]), b(color[2]) {}
  ...
  RGBColor(double red, double green, double blue)
      : BaseObject(), color(red, green, blue), r(color[0]), g(color[1]),
        b(color[2]) {}

  // Copy constructor
  RGBColor(const RGBColor &col)
      : BaseObject(col), color(col.color), r(color[0]), g(color[1]),
        b(color[2]) {}
  ...
  bool operator==(const RGBColor &c) const {
    if (Abs(r - c.r) < 1.0e-3 && Abs(g - c.g) < 1.0e-3 && Abs(b - c.b) < 1.0e-3)
      return true;
    return false;
  }
  ...
  void operator+=(const RGBColor &c) { color += c.color; }

  void operator-=(const RGBColor &c) { color -= c.color; }

  void operator*=(double scalar) { color *= scalar; }
  ...
  friend double luminance(const RGBColor &c) { return (c.r + c.g + c.b); }
  ...
  friend void glColor(const RGBColor &c) { glColor3d(c.r, c.g, c.b); }
};
