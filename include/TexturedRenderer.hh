...
/*
  ...
  Renders with face-vertex normals, without material colors or fave-vertex
  colors. Renders with texture, specified through an OpenGL texture ID
*/
...
protected:
  ...
  void readTexture(const char *imagefile) {
    ...
    if (filename.indexOf(".jpg", filename.length() - 5) ==
            filename.length() - 4 ||
        filename.indexOf(".jpeg", filename.length() - 5) ==
            filename.length() - 4 ||
        filename.indexOf(".JPG", filename.length() - 5) ==
            filename.length() - 4 ||
        filename.indexOf(".JPEG", filename.length() - 5) ==
            filename.length() - 4 ||
        filename.indexOf(".png", filename.length() - 5) ==
            filename.length() - 4 ||
        filename.indexOf(".PNG", filename.length() - 5) ==
            filename.length() - 4) {
      QTransform myTransform;
      myTransform.rotate(180);
      QImage input = QGLWidget::convertToGLFormat(QImage(imagefile));
      image = new QImage(input.transformed(myTransform));
    }
...
public:
  /* Default constructor */
  TexturedRenderer()
      : DLFLRenderer(), image(NULL), width(0), height(0), depth(0),
        texture_id(0) {}

  TexturedRenderer(QColor wc, double wt, QColor sc, double st, QColor vc,
                   double vt, QColor fc, double ft, QColor nc, double nt)
  ...
  TexturedRenderer(const TexturedRenderer &tr)
    ...
    image =
        new QImage(tr.image->bits(), tr.width, tr.height, tr.image->format());
    //(*image) = tr.image->copy();
  }
  ...
  /* Destructor */
  virtual ~TexturedRenderer() { delete image; }
