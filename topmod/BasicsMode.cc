...
GeometricTool::GeometricTool(QWidget *parent) {
  ...
  translation_x_spin_ =
      createDoubleSpinBox(layout_, translation_x_label_, tr("X-translate"),
  ...
  translation_y_spin_ =
      createDoubleSpinBox(layout_, translation_y_label_, tr("Y-translate"),
  ...
  translation_z_spin_ =
      createDoubleSpinBox(layout_, translation_z_label_, tr("Z-translate"),
  ...
  rotation_x_spin_ = createDoubleSpinBox(
      layout_, rotation_x_label_, tr("X-Rotate"), 0, 360, 10, 0.0, 0, 4, 0);
  ...
  rotation_y_spin_ = createDoubleSpinBox(
      layout_, rotation_y_label_, tr("Y-Rotate"), 0, 360, 10, 0.0, 0, 5, 0);
  ...
  rotation_z_spin_ = createDoubleSpinBox(
      layout_, rotation_z_label_, tr("Z-Rotate"), 0, 360, 10, 0.0, 0, 6, 0);
  ...
  scaling_x_spin_ = createDoubleSpinBox(
      layout_, scaling_x_label_, tr("X-Scale"), 0.1, 10.0, 0.1, 1.0, 3, 7, 0);
  ...
  scaling_y_spin_ = createDoubleSpinBox(
      layout_, scaling_y_label_, tr("Y-Scale"), 0.1, 10.0, 0.1, 1.0, 3, 8, 0);
  ...
  scaling_z_spin_ = createDoubleSpinBox(
      layout_, scaling_z_label_, tr("Z-Scale"), 0.1, 10.0, 0.1, 1.0, 3, 9, 0);
...
void GeometricTool::DoAction(double) {
  ...
  for (DLFLFacePtrArray::iterator it = selected_faces.begin();
    ...
    if (std::find(selected_vertices.begin(), selected_vertices.end(),
                  (*it)->front()->getVertexPtr()) == selected_vertices.end())
  ...
  for (DLFLVertexPtrArray::iterator it = selected_vertices.begin();
    ...
    p = p * cos(rotate_x) +
        (1 - cos(rotate_x)) * (p * Vector3d(1, 0, 0)) * Vector3d(1, 0, 0) +
        sin(rotate_x) * (p % Vector3d(1, 0, 0));
    p = p * cos(rotate_y) +
        (1 - cos(rotate_y)) * (p * Vector3d(0, 1, 0)) * Vector3d(0, 1, 0) +
        sin(rotate_y) * (p % Vector3d(0, 1, 0));
    p = p * cos(rotate_z) +
        (1 - cos(rotate_z)) * (p * Vector3d(0, 0, 1)) * Vector3d(0, 0, 1) +
...
void GeometricTool::Apply() {
  ...
  while (!Q.empty()) {
    ...
    for (DLFLEdgePtrArray::iterator eit = edges.begin(); eit != edges.end();
         eit++) {
  ...
  */
  ...
  for (DLFLFacePtrSet::iterator it = old_faces.begin(); it != old_faces.end();
       ++it) {
    DLFLFacePtr face = *it;

    DLFLFaceVertexPtr face_vertex = face->front();
    if (face_vertex == NULL)
      continue; // Ignore empty faces.
    ...
    do {
      ...
      cout << "Looking at corner (" << u->getID() << "," << v->getID() << ")"
           << endl;
...
BasicsMode::BasicsMode(QWidget *parent, QShortcutManager *sm,
  ...
  connect(mInsertEdgeAction, SIGNAL(triggered()), this,
          SLOT(triggerInsertEdge()));
  connect(mInsertEdgeAction, SIGNAL(hovered()), ((MainWindow *)mParent),
          SLOT(setAnimatedHelpImage()));
  ...
  connect(mDeleteEdgeAction, SIGNAL(triggered()), this,
          SLOT(triggerDeleteEdge()));
  ...
  connect(mCollapseEdgeAction, SIGNAL(triggered()), this,
          SLOT(triggerCollapseEdge()));
  ...
  connect(mSubdivideEdgeAction, SIGNAL(triggered()), this,
          SLOT(triggerSubdivideEdge()));
  ...
  connect(mConnectEdgesAction, SIGNAL(triggered()), this,
          SLOT(triggerConnectEdges()));
  ...
  connect(mSpliceCornersAction, SIGNAL(triggered()), this,
          SLOT(triggerSpliceCorners()));
  ...
  /*
        ...
        connect(mTransformsAction, SIGNAL(triggered()), this,
     SLOT(triggerTransforms())); actionList->addAction(mTransformsAction);
  */
  ...
  connect(mSelectionOptionsAction, SIGNAL(triggered()), this,
          SLOT(triggerSelectionOptions()));
...
QDoubleSpinBox *BasicsMode::createDoubleSpinBox(
...
void BasicsMode::setupInsertEdge() {
  ...
  noOptionsInsertEdgeLabel = new QLabel(this);
  (tr("No Options for this tool."));
...
void BasicsMode::setupCollapseEdge() {
  ...
  noOptionsCollapseEdgeLabel =
      new QLabel(tr("No Options for this tool."), this);
...
void BasicsMode::setupSubdivideEdge() {
  ...
  numSubdivsSpinBox =
      createDoubleSpinBox(mSubdivideEdgeLayout, numSubdivsLabel,
                          tr("# Subdivisions"), 1, 10, 1, 1, 0, 0, 0);
  connect(numSubdivsSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(changeNumSubDivs(double)));
...
void BasicsMode::setupTransforms(){
        ...
        xPosSpinBox = createDoubleSpinBox(mTransformsLayout, xPosLabel,
tr("X-translate"), -100.0, 100.0, 0.5, 0.0, 3, 1,0); connect(xPosSpinBox,
SIGNAL(valueChanged(double)), ((MainWindow*)mParent), SLOT(translatex(double)));

        yPosLabel = new QLabel(this);
        yPosSpinBox = createDoubleSpinBox(mTransformsLayout, yPosLabel,
tr("Y-translate"), -100.0, 100.0, 0.5, 0.0, 3, 2,0); connect(yPosSpinBox,
SIGNAL(valueChanged(double)), ((MainWindow*)mParent), SLOT(translatey(double)));

        zPosLabel = new QLabel(this);
        zPosSpinBox = createDoubleSpinBox(mTransformsLayout, zPosLabel,
tr("Z-translate"), -100.0, 100.0, 0.5, 0.0, 3, 3,0); connect(zPosSpinBox,
SIGNAL(valueChanged(double)), ((MainWindow*)mParent), SLOT(translatez(double)));
        ...
        xScaleSpinBox = createDoubleSpinBox(mTransformsLayout, xScaleLabel,
tr("X-scale"), 0.1, 10.0, 0.1, 1.0, 3, 5,0); connect(xScaleSpinBox,
SIGNAL(valueChanged(double)), ((MainWindow*)mParent), SLOT(scalex(double)));

        yScaleLabel = new QLabel(this);
        yScaleSpinBox = createDoubleSpinBox(mTransformsLayout, yScaleLabel,
tr("Y-scale"), 0.1, 10.0, 0.1, 1.0, 3, 6,0); connect(yScaleSpinBox,
SIGNAL(valueChanged(double)), ((MainWindow*)mParent), SLOT(scaley(double)));

        zScaleLabel = new QLabel(this);
        zScaleSpinBox = createDoubleSpinBox(mTransformsLayout, zScaleLabel,
tr("Z-scale"), 0.1, 10.0, 0.1, 1.0, 3, 7,0); connect(zScaleSpinBox,
SIGNAL(valueChanged(double)), ((MainWindow*)mParent), SLOT(scalez(double)));

        freezeTransformsButton = new QPushButton(tr("&Freeze Transforms"));
        connect(freezeTransformsButton, SIGNAL(clicked()), this,
SLOT(freezeTransforms()));
...
void BasicsMode::setupSelectionOptions() {
  ...
  mFaceAreaToleranceSpinBox = createDoubleSpinBox(
      mSelectionOptionsLayout, mFaceAreaToleranceLabel,
      tr("Face Area Sel.\nTolerance"), 0.0, 5.0, 0.001, 0.05, 3, 0, 0);
  connect(mFaceAreaToleranceSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(changeFaceAreaTolerance(double)));

  mSplit2ValenceVertexOffsetLabel = new QLabel(this);
  mSplit2ValenceVertexOffsetSpinBox = createDoubleSpinBox(
      mSelectionOptionsLayout, mSplit2ValenceVertexOffsetLabel,
      tr("Valence-2 Split\nOffset"), -0.1, 5.0, 0.001, -0.1, 3, 1, 0);
  connect(mSplit2ValenceVertexOffsetSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(changeValence2SplitOffset(double)));

  // yPosSpinBox = createDoubleSpinBox(mSelectionOptionsLayout, yPosLabel,
  // tr("Y-translate"), -100.0, 100.0, 0.5, 0.0, 1, 2,0); connect(yPosSpinBox,
  // SIGNAL(valueChanged(double)), ((MainWindow*)mParent),
  // SLOT(translatey(double)));
  //
  // zPosSpinBox = createDoubleSpinBox(mSelectionOptionsLayout, zPosLabel,
  // tr("Z-translate"), -100.0, 100.0, 0.5, 0.0, 1, 3,0); connect(zPosSpinBox,
  // SIGNAL(valueChanged(double)), ((MainWindow*)mParent),
  // SLOT(translatez(double)));
  //
  // // scaleLabel = new QLabel(this);(tr("Scale:"));
  // // mSelectionOptionsLayout->addWidget(scaleLabel,4,0,1,2);
  //
  // //x scale
  // xScaleSpinBox = createDoubleSpinBox(mSelectionOptionsLayout, xScaleLabel,
  // tr("X-scale"), 0.1, 10.0, 0.1, 1.0, 1, 5,0); connect(xScaleSpinBox,
  // SIGNAL(valueChanged(double)), ((MainWindow*)mParent),
  // SLOT(scalex(double)));
  //
  // yScaleSpinBox = createDoubleSpinBox(mSelectionOptionsLayout, yScaleLabel,
  // tr("Y-scale"), 0.1, 10.0, 0.1, 1.0, 1, 6,0); connect(yScaleSpinBox,
  // SIGNAL(valueChanged(double)), ((MainWindow*)mParent),
  // SLOT(scaley(double)));
  //
  // zScaleSpinBox = createDoubleSpinBox(mSelectionOptionsLayout, zScaleLabel,
  // tr("Z-scale"), 0.1, 10.0, 0.1, 1.0, 1, 7,0); connect(zScaleSpinBox,
  // SIGNAL(valueChanged(double)), ((MainWindow*)mParent),
  // SLOT(scalez(double)));
  //
  // QPushButton *freezeTransformsButton = new QPushButton(tr("&Freeze
  // Transforms")); connect(freezeTransformsButton, SIGNAL(clicked()), this,
  // SLOT(freezeTransforms()));
