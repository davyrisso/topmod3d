...
void computeLighting(DLFLFacePtr fp, LightPtr lightptr, bool usegpu) {
  if (fp->front()) {
    ...
    while (current != fp->front()) {
...
#ifdef GPU_OK
      if (usegpu) {
        // cgSetParameter3f(CgData::instance()->Kd, basecolor.r, basecolor.g,
        // basecolor.b);
        fvcolor = RGBColor(0, 0, 0);
      } else {
        fvcolor = lightptr->illuminate(pos, normal) * Kd;
      ...
      // current->color = RGBColor(((double)rand() /
...
void computeLighting(DLFLObjectPtr obj, TMPatchObjectPtr po, LightPtr lightptr,
  ...
  //
  // QProgressDialog *progress = new QProgressDialog("Computing Lighting...",
  // "Cancel", 0, obj->num_faces() + patchsize);
  ...
  if (po) {
    ...
    TMPatchFacePtrList::iterator pfirst = patch_list.begin(),
                                 plast = patch_list.end();
