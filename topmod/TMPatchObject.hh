...
protected:
  ...
  void createPatches(const struct PatchParameters &parameters,
                     DLFLObjectPtr obj);

public:
  // Set the patch size
  void setPatchSize(const struct PatchParameters &parameters,
                    DLFLObjectPtr obj = NULL);

private:
  void updateForPatches(const struct PatchParameters &parameters,
                        DLFLObjectPtr obj);

public:
  // Update the patches
  void updatePatches(const struct PatchParameters &parameters,
                     DLFLObjectPtr obj = NULL) {
    if (!obj) {
      obj = mObj;
    }
...
public:
  ...
  void renderDooSabinPolygons(void) {
    glPushMatrix();
    transform();
    PolygonArray::iterator begin = dooSabinPolygons.begin(),
                           end = dooSabinPolygons.end();
    while (begin != end) {
      if (begin->size() > 0) {
  ...
  void renderModifiedDooSabinPolygons(void) {
    glPushMatrix();
    transform();
    PolygonArray::iterator begin = modifiedDooSabinPolygons.begin(),
                           end = modifiedDooSabinPolygons.end();
    while (begin != end) {
      if (begin->size() > 0) {
        glBegin(GL_LINE_LOOP);
        for (int i = 0; i < begin->size(); ++i) {
