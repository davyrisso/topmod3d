...
private:
  ...
  bool faceCornerIsFine; // whether or not this patch belongs to a face with
                         // four edges

  bool vertexCornerIsFine; // whether or not this patch belongs to a vertex
                           // whose valence is 4
...
public:
  // Default constructor
  TMPatch()
      : patchsize(0), ctrlpts(), ctrlptnormals(), ctrlptcolors(),
        glctrlpts(NULL), glctrlptcolors(NULL), childs(NULL), level(0),
        faceCornerIsFine(false), vertexCornerIsFine(false) {
    resizePatch(4);
  }

private:
  TMPatch(const TMPatch &patch)
      : patchsize(patch.patchsize), ctrlpts(patch.ctrlpts),
        ctrlptnormals(patch.ctrlptnormals), ctrlptcolors(patch.ctrlptcolors),
        glctrlpts(NULL), glctrlptcolors(NULL), childs(NULL), level(0),
...
public:
  ...
  ~TMPatch() {
    ...
    delete[] glctrlptcolors;
    if (childs != NULL) {
      delete[] childs; // this will recursively delete all of the childs down
                       // the nested tree.
      childs = NULL;
    }
  }
...
private:
  ...
  TMPatch &operator=(const TMPatch &patch) {
    ...
    ctrlptnormals = patch.ctrlptnormals;
...
public:
  ...
  void resizePatch(int size) {
    if (size != patchsize) {
      patchsize = size;
      ctrlpts.resize(patchsize);
      ctrlptnormals.resize(patchsize);
      ctrlptcolors.resize(patchsize);
...
private:
  void allocateChilds() {
    ...
    GetGrid(ctrlptnormals, FORWARD, FORWARD, true,
            childs[childCount].ctrlptnormals);
    ...
    GetGrid(ctrlptnormals, BACKWARD, FORWARD, true,
            childs[childCount].ctrlptnormals);
    childs[childCount].faceCornerIsFine = true;
    ...
    GetGrid(ctrlptnormals, FORWARD, BACKWARD, true,
            childs[childCount].ctrlptnormals);
    childs[childCount].faceCornerIsFine = true;
    childs[childCount].vertexCornerIsFine = true;
    childs[childCount].fillNestedPatchPntrs();
    ...
    GetGrid(ctrlptnormals, BACKWARD, BACKWARD, true,
            childs[childCount].ctrlptnormals);
...
public:
  // this function will recursively fill the child patches that belong to a
  // patch
  ...
  const Vector3d &getControlPoint(int i, int j) const {
    ...
    // Flips the i and j since it is flipped when control points are stored (see
    // calculatePatchPoints method)
    return ctrlpts[j][i];
  }
  ...
  // Calculate the points of the patch given the base grid points and base grid
  // normals
  void calculatePatchPoints(const Vector3d (&cp)[4][4],
                            const Vector3d (&cn)[4][4],
                            const bool &fourSidedFace,
                            const bool &fourValenceVertex) {
  ...
  void fillGLLightingBuffer(const RGBColor &basecolor, double Ka, double Kd,
                            double Ks, LightPtr lightptr) {
  ...
  void computeLighting(const RGBColor &basecolor, double Ka, double Kd,
                       double Ks, LightPtr lightptr) {
    if (this->childs == NULL) {
...
private:
  ...
  // void render(void)
  {
    glMap2d(GL_MAP2_VERTEX_3, 0, 1, 3, patchsize, 0, 1, patchsize * 3,
            patchsize, glctrlpts);
    glMap2d(GL_MAP2_COLOR_4, 0, 1, 4, patchsize, 0, 1, patchsize * 4, patchsize,
            glctrlptcolors);
    glEnable(GL_MAP2_VERTEX_3);
    glEnable(GL_MAP2_COLOR_4);
    glMapGrid2d(TMPatch::GridDimensions[this->level], 0.0, 1.0,
                TMPatch::GridDimensions[this->level], 0.0, 1.0);
    glEvalMesh2(GL_FILL, 0, TMPatch::GridDimensions[this->level], 0,
                TMPatch::GridDimensions[this->level]);
...
private:
  ...
  void renderPatchOutlinedPolygons(void) {
    ...
    glMapGrid2d(TMPatch::GridDimensions[this->level], 0.0, 1.0,
                TMPatch::GridDimensions[this->level], 0.0, 1.0);
    glEvalMesh2(GL_LINE, 0, TMPatch::GridDimensions[this->level], 0,
                TMPatch::GridDimensions[this->level]);
    glDisable(GL_MAP2_VERTEX_3);
  }
...
private:
  ...
  void renderPatchBoundary(void) {
    ...
    glMap1d(GL_MAP1_VERTEX_3, 0, 1, patchsize * 3, patchsize,
            glctrlpts + patchsize * 3 - 3);
    glEnable(GL_MAP1_VERTEX_3);
    glMapGrid1d(TMPatch::GridDimensions[this->level], 0.0, 1.0);
    glEvalMesh1(GL_LINE, 0, TMPatch::GridDimensions[this->level]);
    glDisable(GL_MAP1_VERTEX_3);
...
public:
  ...
  void printControlPoints(ostream &stream) {
    for (int i = 0; i < patchsize; i++) {
      for (int j = 0; j < patchsize; j++) {
        stream << "v " << ctrlpts[i][j][0] << " " << ctrlpts[i][j][1] << " "
               << ctrlpts[i][j][2] << std::endl;
      }
    }
  }
};
