/*
 * $Id: SourceFileTemplates.txt,v 1.2 2005/07/28 15:37:03 JiriHnidek Exp
 * www-data $
 *
...
private:
  ...
  QDoubleSpinBox *mCameraFovSpinBox, *mCameraNearPlaneSpinBox,
...
public:
  TopModPreferences(QSettings *settings, StyleSheetEditor *sse,
                    QShortcutManager *sm, QWidget *parent = 0);
  ~TopModPreferences();
...
protected:
  ...
  void setupColors();
  void setupMain();
  QPushButton *addColorPreference(QLabel *label, QString text, QColor color,
                                  QGridLayout *layout, QStyle *style, int row,
                                  int col);
  void setButtonColor(QColor color, QPushButton *button);
  QDoubleSpinBox *addSpinBoxPreference(QLabel *label, QString text, double low,
                                       double high, double step, double value,
                                       int decimals, QGridLayout *layout,
                                       int row, int col);
