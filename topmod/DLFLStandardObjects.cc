...
DLFLObjectPtr DLFLObject ::makeMengerSponge(double edgelength) {
  ...
  // Now translate this object to the correct position
  mengersponge->position.set(-1.5 * edgelength, -1.5 * edgelength,
                             -1.5 * edgelength);
  mengersponge->freezeTransformations();
  return mengersponge;
}
...
DLFLObjectPtr DLFLObject ::makeMengerSponge(int level) {
  ...
  double baselength = pow(
      3.0, level - 1); // Edge length of each sub-Menger sponge at this level
  ...
  for (int m = 0; m < 20; ++m) {
    ...
    j = msfilled[m].j;
    k = msfilled[m].k;
    opos.set(baselength_2 + i * baselength, baselength_2 + j * baselength,
             baselength_2 + k * baselength);
    submengersponge = DLFLObject::makeMengerSponge(level - 1);
    submengersponge->position = opos;
    submengersponge
        ->freezeTransformations(); // An optimized version should be used
    mengersponge->splice(*submengersponge);
    delete submengersponge;
  }

  // Now translate this object to the correct position
  mengersponge->position.set(-1.5 * baselength, -1.5 * baselength,
                             -1.5 * baselength);
  mengersponge->freezeTransformations();
  return mengersponge;
}

// Create a Sierpinski tetrahedron with smallest edge length = 1 and centered at
// origin
DLFLObjectPtr DLFLObject ::makeSierpinskiTetrahedron(int level) {
  if (level < 0)
    return NULL;
  if (level == 0)
    return makeUnitTetrahedron();
  ...
  double baselength =
      pow(2.0, level - 1); // Edge length of sub-Sierpinski tetra at this level
  DLFLObjectPtr substetra; // Next lower level Sierpinski tetra

  substetra = DLFLObject::makeSierpinskiTetrahedron(level - 1);
  substetra->position.set(baselength * rt3_6, baselength * rt6_12,
                          baselength * 0.5);
  ...
  substetra = DLFLObject::makeSierpinskiTetrahedron(level - 1);
  substetra->position.set(baselength * rt3_6 * 4.0, baselength * rt6_12,
                          baselength);
  substetra->freezeTransformations();
  stetra->splice(*substetra);
  delete substetra;
  ...
  substetra = DLFLObject::makeSierpinskiTetrahedron(level - 1);
  substetra->position.set(baselength * rt3_6 * 2.0, baselength * rt6_12 * 5.0,
                          baselength);
  substetra->freezeTransformations();
  stetra->splice(*substetra);
  delete substetra;

  stetra->position.set(-baselength * rt3_6 * 2.0, -baselength * rt6_12,
                       -baselength);
