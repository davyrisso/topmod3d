...
protected:
  ...
  void assignID(void) { uID = DLFLLocator ::newID(); }

public:
  // Default constructor
  DLFLLocator() : coords(), flags(0) {
    vtx = NULL;
    assignID();
    selectedAxis = -1;
  ...
  DLFLLocator(const Vector3d &vec) : coords(vec), flags(0) {
    vtx = NULL;
    assignID();
    selectedAxis = -1;
    renderSelection = false;
  ...
  DLFLLocator(double x, double y, double z) : coords(x, y, z), flags(0) {
    vtx = NULL;
    assignID();
    selectedAxis = -1;
    renderSelection = false;
  ...
  DLFLLocator(const DLFLLocator &dl)
      : vtx(dl.vtx), coords(dl.coords), flags(dl.flags), uID(dl.uID),
        selectedAxis(dl.selectedAxis), renderSelection(dl.renderSelection) {}
  ...
  //--- Mutative functions ---//
  void setRenderSelection(bool s) { renderSelection = s; }
  ...
  void setActiveVertex(DLFLVertexPtr vptr) { vtx = vptr; }

  DLFLVertexPtr getActiveVertex() { return vtx; }
