...
// Class for a generic Viewport. Supports rotation, panning, zooming and
// dollying using a transformation matrix and subroutines to handle
// corresponding events This is a modified version of the ViewWindow class, with
// all connection to FLTK or any other user interface/windowing system removed.
...
// Enumerations for the different views. The names imply looking from the
// corresponding side. ie Front -> looking *from* front, Right -> looking *from*
// right
enum VPView {
  VPPersp = 0,
  VPFront = 1,
...
public:
  // Made public to allow changing camera settings easily (view-volume,
  // near/far)
  ...
  void setPerspView(const Vector3d &eye, const Vector3d &center,
                    const Vector3d &up) {
    if (view == VPPersp) {
  ...
  void setPerspView(double eyex, double eyey, double eyez, double cenx,
                    double ceny, double cenz, double upx, double upy,
                    double upz) {
    setPerspView(Vector3d(eyex, eyey, eyez), Vector3d(cenx, ceny, cenz),
  ...
  Viewport(int w, int h)
      : width(w), height(h), arcball(), trcontrol(), zoomcontrol(),
        dollycontrol(10.0), transform(), currenttr(VPNone), view(VPPersp),
        camera() {
    switchTo(view);
  }
  ...
  Viewport(int w, int h, VPView v)
      : width(w), height(h), arcball(), trcontrol(), zoomcontrol(),
        dollycontrol(10.0), transform(), currenttr(VPNone), view(v), camera() {
    switchTo(view);
  }

  virtual ~Viewport() {}

  VPTransformType current(void) const { return currenttr; }
  ...
  // This function should be called when the window using this viewport is
  // resized
  ...
  virtual void handle_rotate(VPMouseEvent event, int event_x, int event_y) {
    ...
    if (currenttr != VPNone && currenttr != VPRotate)
      return;
    ...
    if (view == VPPersp) // No rotations for ortho views
    {
      ...
      if (event == VPPush)
        arcball.beginDrag();
      // else if (event == VPDrag) transform.rotate(arcball.quat_value());  //
      // Update the combined transformation
      else if (event == VPRelease) {
        arcball.endDrag();
        transform.rotate(
            arcball.quat_value()); // Update the combined transformation
        arcball.reset();
      }
    }
    if (event == VPRelease)
  ...
  virtual void handle_pan(VPMouseEvent event, int event_x, int event_y) {
    ...
    if (currenttr != VPNone && currenttr != VPPan)
      return;
    ...
    camera.adjust(x, y, z);
    trcontrol.mouse(x, y, z);
    trcontrol.update();

    if (event == VPPush)
      trcontrol.beginDrag();
    // else if (event == VPDrag) transform.translate(trcontrol.trans_value());//
    // Update the combined transformation
    else if (event == VPRelease) {
      trcontrol.endDrag();
      transform.translate(
          trcontrol.trans_value()); // Update the combined transformation
      trcontrol.reset();
      currenttr = VPNone;
    }
  }
  ...
  virtual void handle_zoom(VPMouseEvent event, int event_x, int event_y = 0) {
    ...
    if (currenttr != VPNone && currenttr != VPZoom)
      return;
    ...
    if (event == VPPush)
      zoomcontrol.beginDrag();
    // else if (event == VPDrag) transform.scale(zoomcontrol.zoom_value());//
    // Update the combined transformation
    else if (event == VPRelease) {
      zoomcontrol.endDrag();
      transform.scale(
          zoomcontrol.zoom_value()); // Update the combined transformation
      zoomcontrol.reset();
      currenttr = VPNone;
    }
  }
  ...
  virtual void handle_dolly(VPMouseEvent event, int event_x, int event_y = 0) {
    // For orthographic views, call handle_zoom, since handle_dolly has no
    // effect
    if (view != VPPersp)
      handle_zoom(event, event_x, event_y);
    else {
      // Don't do anything if we aren't in a neutral state or aren't dollying
      if (currenttr != VPNone && currenttr != VPDolly)
        return;
      ...
      dollycontrol.mouse(z);
      dollycontrol.update();

      if (event == VPPush)
        dollycontrol.beginDrag();
  ...
  virtual bool send_to_current(VPMouseEvent event, int event_x, int event_y) {
    // Send the given event to the subroutine handling the current
    // transformation
    bool handled = true;
    switch (currenttr) {
    case VPPan:
      handle_pan(event, event_x, event_y);
      break;
    case VPZoom:
      handle_zoom(event, event_x, event_y);
      break;
    case VPRotate:
      handle_rotate(event, event_x, event_y);
      break;
    case VPDolly:
      handle_dolly(event, event_x, event_y);
      break;
    case VPNone:
    default:
      handled = false;
