...
DLFLScriptEditor::DLFLScriptEditor(DLFLObjectPtr obj, QWidget *parent,
                                   Qt::WindowFlags f)
    : QWidget(parent), dlfl_module(NULL), dlfl_dict(NULL), mEchoing(true),
      pathPython(""), mTabWidth(3), addToPath(".") {
  ...
  mCommandEdit->setTabStopWidth(fontMetrics().width(" ") *
                                mTabWidth); // req. width in pixels

  QObject::connect(mCommandEdit, SIGNAL(ctrlReturnPressed()), this,
                   SLOT(executeCommand()));
  QObject::connect(this, SIGNAL(addToHistory(const QString &)), mCommandEdit,
                   SLOT(appendHistory(const QString &)));
  ...
  mExecFile = new QAction(QIcon(":images/applications-system.png"),
                          tr("&Execute File"), this);
  ...
  mSaveFile = new QAction(QIcon(":images/document-save-as.png"),
                          tr("&Save File"), this);
  ...
  mSaveOutput = new QAction(QIcon(":images/document-save-as.png"),
                            tr("Save Out&put"), this);
  ...
  mClearHistory = new QAction(QIcon(":images/clear_scriptoutput.png"),
                              tr("Clear &History"), this);
  ...
  mClearInput = new QAction(QIcon(":images/clear_scriptinput.png"),
                            tr("&Clear Input"), this);
  ...
  mToggleEchoing = new QAction(QIcon(":images/accessories-text-editor.png"),
                               tr("&Toggle Echoing"), this);
  ...
  mConvertSpaces = new QAction(QIcon(":images/applications-system.png"),
                               tr("&Convert Spaces To Tabs"), this);
  mConvertSpaces->setStatusTip(
      (tr("Convert each set of spaces into a TAB character")));
  connect(mConvertSpaces, SIGNAL(triggered()), this, SLOT(spacesToTABs()));
  mFormatMenu->addAction(mConvertSpaces);

  mSetTabWidth = new QAction(QIcon(":images/applications-system.png"),
                             tr("&Set Tab Width"), this);
  mSetTabWidth->setStatusTip((tr("Will affect conversion of spaces to Tabs")));
  connect(mSetTabWidth, SIGNAL(triggered()), this,
          SLOT(toggleTabWidthWidget()));
  ...
  connect(mSetTabWidthWidget, SIGNAL(valueChanged(int)), this,
      ...
      for (int i = 0; i < cmdList.size(); i++) {
        ...
        if (si.endsWith(":")) {
          ...
          while (j < cmdList.size() && (cmdList.at(j).startsWith("\t") ||
                                        cmdList.at(j).startsWith(" "))) {
            /*// If it begins with a space figure out if we're in the same
            indentation level if( cmdList.at(j).startsWith(" ") ) { QRegExp rx =
            QRegExp("(^[ ])([ ])*"); int pos = rx.indexIn( cmdList.at(j) );
        ...
        // Check if it is a load command (let TopMod handle this instead of
        // Python)
        ...
        PyObject *rstring =
            PyRun_String(cmd, Py_file_input, main_dict, main_dict);
        ...
        if (rstring != NULL) {
          ...
          // resultObject2 has any possible thing printed to stdout from running
          // the command
          ...
          result =
              QString(string) + QString("  "); // the spaces b/c I chop(2) later
        }
...
void DLFLScriptEditor::loadObject(DLFLObject *obj, QString fileName) {
  ...
  if (dlfl_module)
    PyDLFL_PassObject(
        obj); // pass the object pointer to currObj in DLFLModule.cc
...
void DLFLScriptEditor::execFile() {
  ...
  QString filename = QFileDialog::getOpenFileName(
      this, tr("Exec File"), "$HOME", tr("Python Files (*.py);;All Files (*)"),
...
void DLFLScriptEditor::openFile() {
  QString filename = QFileDialog::getOpenFileName(
      this, tr("Open Python File"), "$HOME",
      tr("Python Files (*.py);;All Files (*)"), 0, QFileDialog::DontUseSheet);
  if (!filename.isEmpty()) {
    QFile file(filename);
...
void DLFLScriptEditor::saveFile() {
  QString filename = QFileDialog::getSaveFileName(
      this, tr("Save Python File"), "myscript",
      tr("Python Files (*.py);;All Files (*)"), 0, QFileDialog::DontUseSheet);
  if (!filename.isEmpty()) {
    QFile file(filename);
...
void DLFLScriptEditor::saveOutput() {
  QString filename =
      QFileDialog::getSaveFileName(this, tr("Save Python File"), "myscript",
...
void DLFLScriptEditor::toggleEchoing() { mEchoing = !mEchoing; }
