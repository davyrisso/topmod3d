...
private:
  ...
  GeometryRenderer(bool gpu = false)
      : useMaterial(false), useColorable(false), useLighting(false),
        useNormal(false), useTexture(false), antialiasing(false),
        useOutline(false), drawFaceCentroids(false), drawVertices(false),
        drawSilhouette(false), drawWireframe(true), drawFaceNormals(false),
        isReversed(false), useGPU(gpu) {
