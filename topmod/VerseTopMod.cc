...
// /* A callback for connection acception: will be called when server accepts
// this client. */ void callback_accept_connect(void *user, uint32 avatar, void
// *address, void *connection, uint8 *host_id) {
//
// 	uint32 i, mask = 0;
...
// 		mask |= 1 << i;
// 	verse_send_node_index_subscribe(mask);     /* Request listing of all
// nodes. */
// }
//
// /* A callback for node creation: is called to report information about
// existing nodes, too. */ void callback_node_create(void *user, VNodeID
// node_id, VNodeType type, VNodeOwner ownership) {
// 	VerseTopMod::Instance()->write(" Node #" + QString(node_id) + " has type
// " + QString(type) + "\n");
// 	// std::cout <<" Node #" << node_id << " has type " << type <<
// std::endl;
// }
...
VerseTopMod::VerseTopMod(QWidget *parent, Qt::WindowFlags f)
  ...
  QObject::connect(mLineEdit, SIGNAL(returnPressed()), this,
                   SLOT(executeCommand()));
...
void VerseTopMod::connectLocalhost() {
  ...
  // verse_callback_set((void *)verse_send_connect_accept, (void
  // *)callback_accept_connect, NULL); verse_callback_set((void
  // *)verse_send_node_create,	  (void *)callback_node_create, NULL);
  //
