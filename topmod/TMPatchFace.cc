...
const int TMPatchFace::CORNERS[][2] = {{0, 0}, {0, 3}, {3, 3}, {3, 0}};

void TMPatchFace::extractPatchDataFromFace(
    BiCubicBezierPatchToDLFLData *dataExtractor) {
  TMPatchPtrArray::iterator begin = this->patcharray.begin(),
                            end = this->patcharray.end();
  while (begin != end) {
...
void TMPatchFace::createPatches(TMPatchMap &patchMap,
                                const struct PatchParameters &params) {
  if (dlflface == NULL)
    return;
  ...
  // Get the centroid and normal for the face - these will be used in all
  // patches
  ...
  Vector3d cn[4]
             [4]; // Grid of control normals used to modify the control points
  ...
  for (int i = 0; i < size; ++i) {
    ...
    for (int corner_index = 0; corner_index < 4; ++corner_index) {
      for (int neighbor_index = 0; neighbor_index < 3; ++neighbor_index) {
        ...
        case ALL_UNIFORMLY_R:
          DLFL::rotateAroundVector(cn[corner_i][corner_j], params.angle, true,
                                   v);
    ...
    patcharray[i]->calculatePatchPoints(cp, cn, isFourSidedFace,
                                        isFourValenceVertex);
