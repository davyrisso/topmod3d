...
TopModPreferences::TopModPreferences(QSettings *settings, StyleSheetEditor *sse,
...
void TopModPreferences::saveSettings() {
  ...
  mSettings->setValue("IncrementalSave",
                      mIncrementalSaveCheckBox->checkState());
  mSettings->setValue("IncrementalSaveMax",
                      mIncrementalSaveMaxSpinBox->value());
  mSettings->endGroup();
  ...
  mSettings->setValue("showStartupDialogAtStartup",
                      ((MainWindow *)mParent)->getShowStartupDialogAtStartup());
  mSettings->setValue("toolOptionsPos",
                      ((MainWindow *)mParent)->mToolOptionsDockWidget->pos());
  mSettings->setValue("CommandCompleterIndex",
                      mCommandCompleterIndexToggle->checkState());
  mSettings->setValue("SingleClickExtrude",
                      mSingleClickExtrudeCheckBox->checkState());

#ifdef WITH_PYTHON
  mSettings->setValue("scriptEditorPos",
                      ((MainWindow *)mParent)->mScriptEditorDockWidget->pos());
  mSettings->setValue("scriptEditorSize",
                      ((MainWindow *)mParent)->mScriptEditorDockWidget->size());
#endif
  mSettings->endGroup();
...
void TopModPreferences::readSettings() {
  ...
  mSettings->beginGroup("Colors");
  mViewportColorDefault = QColor(
      67, 68,
      88); // this is where you set defaults now. only one time in the code it
  mViewportColor =
      mSettings->value("ViewportColor", mViewportColorDefault).value<QColor>();
  mRenderColorDefault = QColor(255, 151, 92);
  mRenderColor =
      mSettings->value("RenderColor", mRenderColorDefault).value<QColor>();
  mCoolLightColorDefault = QColor(27, 26, 23);
  mCoolLightColor = mSettings->value("CoolLightColor", mCoolLightColorDefault)
                        .value<QColor>();
  mWarmLightColorDefault = QColor(255, 250, 226);
  mWarmLightColor = mSettings->value("WarmLightColor", mWarmLightColorDefault)
                        .value<QColor>();
  mWireframeColorDefault = QColor(0, 0, 0, 255);
  mWireframeColor = mSettings->value("WireframeColor", mWireframeColorDefault)
                        .value<QColor>();
  mSilhouetteColorDefault = QColor(0, 0, 0, 255);
  mSilhouetteColor =
      mSettings->value("SilhouetteColor", mSilhouetteColorDefault)
          .value<QColor>();
  mPatchBoundaryColorDefault = QColor(0, 0, 0);
  mPatchBoundaryColor =
      mSettings->value("PatchBoundaryColor", mPatchBoundaryColor)
          .value<QColor>();
  mSelectedVertexColorDefault = QColor(0, 255, 0, 127);
  mSelectedVertexColor =
      mSettings->value("SelectedVertexColor", mSelectedVertexColorDefault)
          .value<QColor>();
  mSelectedEdgeColorDefault = QColor(255, 0, 0, 127);
  mSelectedEdgeColor =
      mSettings->value("SelectedEdgeColor", mSelectedEdgeColorDefault)
          .value<QColor>();
  mSelectedFaceColorDefault = QColor(247, 0, 229, 127);
  mSelectedFaceColor =
      mSettings->value("SelectedFaceColor", mSelectedFaceColorDefault)
          .value<QColor>();
  mVertexIDBgColorDefault = QColor(127, 0, 0, 127);
  mVertexIDBgColor =
      mSettings->value("VertexIDBgColor", mVertexIDBgColorDefault)
          .value<QColor>();
  mEdgeIDBgColorDefault = QColor(0, 127, 0, 127);
  mEdgeIDBgColor =
      mSettings->value("EdgeIDBgColor", mEdgeIDBgColorDefault).value<QColor>();
  mNormalColorDefault = QColor(127, 127, 0);
  mNormalColor =
      mSettings->value("NormalColor", mNormalColorDefault).value<QColor>();
  mFaceCentroidColorDefault = QColor(127, 0, 127);
  mFaceCentroidColor =
      mSettings->value("FaceCentroidColor", mFaceCentroidColorDefault)
          .value<QColor>();
  mFaceIDBgColorDefault = QColor(0, 0, 127, 127);
  mFaceIDBgColor =
      mSettings->value("FaceIDBgColor", mFaceIDBgColorDefault).value<QColor>();
  mSettings->endGroup();

  mSettings->beginGroup("Display");
  mLightIntensityDefault = 2.0;
  mLightIntensity =
      mSettings->value("LightIntensity", mLightIntensityDefault).toDouble();
  mWireframeThicknessDefault = 0.1;
  mWireframeThickness =
      mSettings->value("WireframeThickness", mWireframeThicknessDefault)
          .toDouble();
  mSilhouetteThicknessDefault = 6.0;
  mSilhouetteThickness =
      mSettings->value("SilhouetteThickness", mSilhouetteThicknessDefault)
          .toDouble();
  mVertexThicknessDefault = 5.0;
  mVertexThickness =
      mSettings->value("VertexThickness", mVertexThicknessDefault).toDouble();
  mSelectedVertexThicknessDefault = 8.0;
  mSelectedVertexThickness =
      mSettings
          ->value("SelectedVertexThickness", mSelectedVertexThicknessDefault)
          .toDouble();
  mSelectedEdgeThicknessDefault = 5.0;
  mSelectedEdgeThickness =
      mSettings->value("SelectedEdgeThickness", mSelectedEdgeThicknessDefault)
          .toDouble();
  mNormalThicknessDefault = 0.1;
  mNormalThickness =
      mSettings->value("NormalThickness", mNormalThicknessDefault).toDouble();
  mNormalLengthDefault = 1.0;
  mNormalLength =
      mSettings->value("NormalLength", mNormalLengthDefault).toDouble();
  mFaceCentroidThicknessDefault = 5.0;
  mFaceCentroidThickness =
      mSettings->value("FaceCentroidThickness", mFaceCentroidThicknessDefault)
          .toDouble();
  mSettings->endGroup();
  ...
  mCameraNearPlane =
      mSettings->value("NearPlane", mCameraNearPlaneDefault).toDouble();
  mCameraFarPlaneDefault = 200;
  mCameraFarPlane =
      mSettings->value("FarPlane", mCameraFarPlaneDefault).toDouble();
  ...
  mSaveDirectory =
      mSettings->value("SaveDirectory", mSaveDirectoryDefault).toString();
  ...
  mAutoSaveDelay =
      mSettings->value("AutoSaveDelay", mAutoSaveDelayDefault).toDouble();
  mIncrementalSaveDefault = false;
  mIncrementalSave =
      mSettings->value("IncrementalSave", mIncrementalSaveDefault).toBool();
  mIncrementalSaveMaxDefault = 20;
  mIncrementalSaveMax =
      mSettings->value("IncrementalSaveMax", mIncrementalSaveMaxDefault)
          .toDouble();
  mSettings->endGroup();
  ...
  QPoint toolOptionsPos =
      mSettings->value("toolOptionsPos", QPoint()).toPoint();

  mCommandCompleterIndexDefault = false;
  mCommandCompleterIndex =
      mSettings->value("CommandCompleterIndex", mCommandCompleterIndexDefault)
          .toBool();

  mSingleClickExtrudeDefault = false;
  mSingleClickExtrude =
      mSettings->value("SingleClickExtrude", mSingleClickExtrudeDefault)
          .toBool();

#ifdef WITH_PYTHON
  QSize scriptEditorSize =
      mSettings->value("scriptEditorSize", QSize(500, 300)).toSize();
  QPoint scriptEditorPos =
      mSettings
          ->value("scriptEditorPos",
                  QPoint(20, QApplication::desktop()->height() - 500))
          .toPoint();
#endif
  mShowStartupDialogAtStartup =
      mSettings->value("showStartupDialogAtStartup", false).toBool();
  mSettings->endGroup();

  ((MainWindow *)mParent)->resize(size.width(), size.height());
  ((MainWindow *)mParent)->move(pos);
  ((MainWindow *)mParent)
      ->setShowStartupDialogAtStartup((int)mShowStartupDialogAtStartup);
  ((MainWindow *)mParent)
      ->mToolOptionsDockWidget->setGeometry(
          10 + ((MainWindow *)mParent)->x(), ((MainWindow *)mParent)->y() + 150,
          ((MainWindow *)mParent)->mToolOptionsDockWidget->width(),
          ((MainWindow *)mParent)->mToolOptionsDockWidget->height());
#ifdef WITH_PYTHON
  ((MainWindow *)mParent)
      ->mScriptEditorDockWidget->resize(scriptEditorSize.width(),
                                        scriptEditorSize.height());
  ...
  ((MainWindow *)mParent)
      ->getActive()
      ->setSelectedVertexColor(mSelectedVertexColor);
  ((MainWindow *)mParent)
      ->getActive()
      ->setSelectedEdgeColor(mSelectedEdgeColor);
  ((MainWindow *)mParent)
      ->getActive()
      ->setSelectedFaceColor(mSelectedFaceColor);
  ...
  ((MainWindow *)mParent)
      ->getActive()
      ->setFaceCentroidColor(mFaceCentroidColor);
  ...
  ((MainWindow *)mParent)
      ->getActive()
      ->setFaceCentroidThickness(mFaceCentroidThickness);
  ((MainWindow *)mParent)->getActive()->setNormalLength(mNormalLength);
  ((MainWindow *)mParent)
      ->getActive()
      ->setWireframeThickness(mWireframeThickness);
  ((MainWindow *)mParent)
      ->getActive()
      ->setSilhouetteThickness(mSilhouetteThickness);
  ((MainWindow *)mParent)->getActive()->setVertexThickness(mVertexThickness);
  ((MainWindow *)mParent)
      ->getActive()
      ->setSelectedVertexThickness(mSelectedVertexThickness);
  ((MainWindow *)mParent)
      ->getActive()
      ->setSelectedEdgeThickness(mSelectedEdgeThickness);
  ...
  ((MainWindow *)mParent)
      ->setCommandCompleterIndexToggle(mCommandCompleterIndex);
  ((MainWindow *)mParent)->setSingleClickExtrude(mSingleClickExtrude);
}

void TopModPreferences::discardSettings() { reject(); }
...
/*!
 * \brief load all the default settings and attach this as a signal for a reset
 * button
 */
...
void TopModPreferences::setupMain() {
  ...
  // 				((MainWindow*)mParent),
  // SLOT(switchPerspView()));

  // mShowStartupDialogAtStartupCheckBox = new QCheckBox(tr("Show Tutorial
  // Dialog at Startup"));
  ...
  // camera nearplane
  mCameraNearPlaneSpinBox =
      addSpinBoxPreference(mCameraNearPlaneLabel, tr("Near Plane:"), 0.0, 100,
                           0.1, mCameraNearPlane, 1, mMainLayout, 0, 0);
  connect(mCameraNearPlaneSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(), SLOT(setNearPlane(double)));

  // camera farplane
  mCameraFarPlaneSpinBox =
      addSpinBoxPreference(mCameraFarPlaneLabel, tr("Far Plane:"), 10, 1000,
                           0.1, mCameraFarPlane, 1, mMainLayout, 1, 0);
  connect(mCameraFarPlaneSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(), SLOT(setFarPlane(double)));

  // camera nearplane
  mCameraFovSpinBox =
      addSpinBoxPreference(mCameraFovLabel, tr("Field of View:"), 15, 100, 1,
                           mCameraFov, 0, mMainLayout, 2, 0);
  connect(mCameraFovSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(), SLOT(setFOV(double)));

  // auto save toggle
  mAutoSaveCheckBox = new QCheckBox(tr("Auto Save:"), this);
  mAutoSaveCheckBox->setChecked(mAutoSave);
  connect(mAutoSaveCheckBox, SIGNAL(stateChanged(int)), ((MainWindow *)mParent),
          SLOT(setAutoSave(int)));
  mMainLayout->addWidget(mAutoSaveCheckBox, 3, 0);

  // auto save delay in minutes
  mAutoSaveDelaySpinBox = addSpinBoxPreference(
      mAutoSaveDelayLabel, tr("Auto Save Delay\n(in minutes):"), 1, 30, 1,
      mAutoSaveDelay, 0, mMainLayout, 4, 0);
  connect(mAutoSaveDelaySpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(setAutoSaveDelay(double)));
  ...
  connect(mIncrementalSaveCheckBox, SIGNAL(stateChanged(int)),
          ((MainWindow *)mParent), SLOT(setIncrementalSave(int)));
  mMainLayout->addWidget(mIncrementalSaveCheckBox, 5, 0);

  // max incremental save files on disk
  mIncrementalSaveMaxSpinBox = addSpinBoxPreference(
      mIncrementalSaveMaxLabel, tr("Max Incremental Saves:"), 2, 50, 1,
      mIncrementalSaveMax, 0, mMainLayout, 6, 0);
  connect(mIncrementalSaveMaxSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(setIncrementalSaveMax(double)));
  ...
  connect(mSaveDirectoryLineEdit, SIGNAL(textEdited(QString)),
          ((MainWindow *)mParent), SLOT(setSaveDirectory(QString)));
  connect(mSaveDirectoryLineEdit, SIGNAL(editingFinished()),
          ((MainWindow *)mParent), SLOT(checkSaveDirectory()));
  ...
  mCommandCompleterIndexToggle =
      new QCheckBox(tr("Command Completer\nSingle Word Completion"), this);
  mCommandCompleterIndexToggle->setChecked(mCommandCompleterIndex);
  connect(mCommandCompleterIndexToggle, SIGNAL(stateChanged(int)),
          ((MainWindow *)mParent), SLOT(setCommandCompleterIndexToggle(int)));
  ...
  connect(mSingleClickExtrudeCheckBox, SIGNAL(stateChanged(int)),
          ((MainWindow *)mParent), SLOT(setSingleClickExtrude(int)));
...
QDoubleSpinBox *TopModPreferences::addSpinBoxPreference(
    QLabel *label, QString text, double low, double high, double step,
    double value, int decimals, QGridLayout *layout, int row, int col) {
  label = new QLabel(text);
...
void TopModPreferences::setupColors() {
  ...
  mViewportColorButton =
      addColorPreference(mViewportColorLabel, tr("Viewport Background:"),
                         mViewportColor, mColorsLayout, colorPickerStyle, 0, 0);
  connect(mViewportColorButton, SIGNAL(clicked()), this,
          SLOT(setViewportColor()));
  // object render color
  mRenderColorButton =
      addColorPreference(mRenderColorLabel, tr("Object Render Color:"),
                         mRenderColor, mColorsLayout, colorPickerStyle, 1, 0);
  connect(mRenderColorButton, SIGNAL(clicked()), this, SLOT(setRenderColor()));
  // CoolLight  color
  mCoolLightColorButton = addColorPreference(
      mCoolLightColorLabel, tr("Cool Light Color:"), mCoolLightColor,
      mColorsLayout, colorPickerStyle, 2, 0);
  connect(mCoolLightColorButton, SIGNAL(clicked()), this,
          SLOT(setCoolLightColor()));
  // WarmLight  color
  mWarmLightColorButton = addColorPreference(
      mWarmLightColorLabel, tr("Warm Light Color:"), mWarmLightColor,
      mColorsLayout, colorPickerStyle, 3, 0);
  connect(mWarmLightColorButton, SIGNAL(clicked()), this,
          SLOT(setWarmLightColor()));
  // Wireframe  color
  mWireframeColorButton = addColorPreference(
      mWireframeColorLabel, tr("Wireframe Color:"), mWireframeColor,
      mColorsLayout, colorPickerStyle, 4, 0);
  connect(mWireframeColorButton, SIGNAL(clicked()), this,
          SLOT(setWireframeColor()));
  // Silhouette  color
  mSilhouetteColorButton = addColorPreference(
      mSilhouetteColorLabel, tr("Silhouette Color:"), mSilhouetteColor,
      mColorsLayout, colorPickerStyle, 5, 0);
  connect(mSilhouetteColorButton, SIGNAL(clicked()), this,
          SLOT(setSilhouetteColor()));
  // PatchBoundary  color
  mPatchBoundaryColorButton = addColorPreference(
      mPatchBoundaryColorLabel, tr("Patch Boundary Color:"),
      mPatchBoundaryColor, mColorsLayout, colorPickerStyle, 6, 0);
  connect(mPatchBoundaryColorButton, SIGNAL(clicked()), this,
          SLOT(setPatchBoundaryColor()));
  // SelectedVertex  color
  mSelectedVertexColorButton = addColorPreference(
      mSelectedVertexColorLabel, tr("Selected Vertex Color:"),
      mSelectedVertexColor, mColorsLayout, colorPickerStyle, 7, 0);
  connect(mSelectedVertexColorButton, SIGNAL(clicked()), this,
          SLOT(setSelectedVertexColor()));
  // SelectedEdge  color
  mSelectedEdgeColorButton = addColorPreference(
      mSelectedEdgeColorLabel, tr("Selected Edge Color:"), mSelectedEdgeColor,
      mColorsLayout, colorPickerStyle, 8, 0);
  connect(mSelectedEdgeColorButton, SIGNAL(clicked()), this,
          SLOT(setSelectedEdgeColor()));
  // SelectedFace  color
  mSelectedFaceColorButton = addColorPreference(
      mSelectedFaceColorLabel, tr("Selected Face Color:"), mSelectedFaceColor,
      mColorsLayout, colorPickerStyle, 9, 0);
  connect(mSelectedFaceColorButton, SIGNAL(clicked()), this,
          SLOT(setSelectedFaceColor()));
  // VertexIDBg  color
  mVertexIDBgColorButton = addColorPreference(
      mVertexIDBgColorLabel, tr("Vertex ID Bg Color:"), mVertexIDBgColor,
      mColorsLayout, colorPickerStyle, 10, 0);
  connect(mVertexIDBgColorButton, SIGNAL(clicked()), this,
          SLOT(setVertexIDBgColor()));
  // FaceIDBg  color
  mFaceIDBgColorButton = addColorPreference(
      mFaceIDBgColorLabel, tr("Face ID Bg Color:"), mFaceIDBgColor,
      mColorsLayout, colorPickerStyle, 11, 0);
  connect(mFaceIDBgColorButton, SIGNAL(clicked()), this,
          SLOT(setFaceIDBgColor()));
  // EdgeIDBg  color
  mEdgeIDBgColorButton = addColorPreference(
      mEdgeIDBgColorLabel, tr("Edge ID Bg Color:"), mEdgeIDBgColor,
      mColorsLayout, colorPickerStyle, 12, 0);
  connect(mEdgeIDBgColorButton, SIGNAL(clicked()), this,
          SLOT(setEdgeIDBgColor()));
  // Normal  color
  mNormalColorButton =
      addColorPreference(mNormalColorLabel, tr("Normal Color:"), mNormalColor,
                         mColorsLayout, colorPickerStyle, 13, 0);
  connect(mNormalColorButton, SIGNAL(clicked()), this, SLOT(setNormalColor()));
  // FaceCentroid  color
  mFaceCentroidColorButton = addColorPreference(
      mFaceCentroidColorLabel, tr("Face Centroid Color:"), mFaceCentroidColor,
      mColorsLayout, colorPickerStyle, 14, 0);
  connect(mFaceCentroidColorButton, SIGNAL(clicked()), this,
          SLOT(setFaceCentroidColor()));

  // new column

  // light intensity
  mLightIntensitySpinBox =
      addSpinBoxPreference(mLightIntensityLabel, tr("Light Intensity:"), 0.0,
                           10.0, 0.1, mLightIntensity, 1, mColorsLayout, 0, 2);
  connect(mLightIntensitySpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(),
          SLOT(setLightIntensity(double)));
  // wireframe thickness
  mWireframeThicknessSpinBox = addSpinBoxPreference(
      mWireframeThicknessLabel, tr("Wireframe Thickness:"), 0.1, 30.0, 0.1,
      mWireframeThickness, 1, mColorsLayout, 1, 2);
  connect(mWireframeThicknessSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(),
          SLOT(setWireframeThickness(double)));
  // vertex thickness
  mVertexThicknessSpinBox =
      addSpinBoxPreference(mVertexThicknessLabel, tr("Vertex Size:"), 0.1, 10.0,
                           0.5, mVertexThickness, 1, mColorsLayout, 2, 2);
  connect(mVertexThicknessSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(),
          SLOT(setVertexThickness(double)));
  // silhouette thickness
  mSilhouetteThicknessSpinBox = addSpinBoxPreference(
      mSilhouetteThicknessLabel, tr("Silhouette Thickness:"), 0.1, 15.0, 0.5,
      mSilhouetteThickness, 1, mColorsLayout, 3, 2);
  connect(mSilhouetteThicknessSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(),
          SLOT(setSilhouetteThickness(double)));
  ...
  connect(mSelectedVertexThicknessSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(),
          SLOT(setSelectedVertexThickness(double)));
  ...
  connect(mSelectedEdgeThicknessSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(),
          SLOT(setSelectedEdgeThickness(double)));
  // Normal Size
  mNormalThicknessSpinBox =
      addSpinBoxPreference(mNormalThicknessLabel, tr("Normal Thickness:"), 0.1,
                           15.0, 0.5, mNormalThickness, 1, mColorsLayout, 6, 2);
  connect(mNormalThicknessSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(),
          SLOT(setNormalThickness(double)));
  // SelectedEdge thickness
  mNormalLengthSpinBox =
      addSpinBoxPreference(mNormalLengthLabel, tr("Normal Length:"), 0.1, 15.0,
                           0.5, mNormalLength, 1, mColorsLayout, 7, 2);
  connect(mNormalLengthSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(), SLOT(setNormalLength(double)));
  // SelectedEdge thickness
  ...
  connect(mFaceCentroidThicknessSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent)->getActive(),
          SLOT(setFaceCentroidThickness(double)));
...
void TopModPreferences::setPatchBoundaryColor() {
  mPatchBoundaryColor.setRgba(
      QColorDialog::getRgba(mPatchBoundaryColor.rgba()));
...
void TopModPreferences::setSelectedVertexColor() {
  mSelectedVertexColor.setRgba(
      QColorDialog::getRgba(mSelectedVertexColor.rgba()));
