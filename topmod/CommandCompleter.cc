 ...
 * \brief Constructor
 *
 * @param m the actionList passed from the MainWindow class after each Mode
 * class is instantiated
 * @param parent the MainWindow parent widget
 * @param f window flags for the QDialog constructor
 *
 */
CommandCompleter::CommandCompleter(QWidget *m, QWidget *parent,
                                   Qt::WindowFlags f)
    : QDialog(parent, f) {
  ...
  mQuickCommandLabel =
      new QLabel(tr("<font color=\"white\">Type a command:</font>"));
  ...
  // QPixmap(QString(":images/commandcompleter.png"))); this->setBackgroundMode(
  // Qt::NoBackground );
  this->setPalette(p);
  ...
  // loop through the actions added to the widget *m, add the text to a word
  // list for now
