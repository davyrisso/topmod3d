...
extern DLFLVertexPtrArray DLFLObject::sel_vptr_array; // List of selected
DLFLVertex pointers extern DLFLEdgePtrArray DLFLObject::sel_eptr_array; // List
of selected DLFLEdge pointers extern DLFLFacePtrArray
DLFLObject::sel_fptr_array; // List of selected DLFLFace pointers extern
DLFLFaceVertexPtrArray DLFLObject::sel_fvptr_array; // List of selected
DLFLFaceVertex pointers
*/
// QGLContext *cx;
// QGLFormat f;
// f.setStereo( TRUE );
...
GLWidget::GLWidget(int w, int h, DLFLRendererPtr rp, QColor color,
                   QColor vcolor, DLFLObjectPtr op, const QGLFormat &format,
                   QWidget *parent)
    : QGLWidget(format, parent, NULL), /*viewport(w,h,v),*/ object(op),
      patchObject(NULL), renderer(rp), renderObject(true), mRenderColor(color),
      mViewportColor(vcolor), /*grid(ZX,20.0,10),*/ showgrid(false),
      showaxes(false), mUseGPU(false), mAntialiasing(true) {
  mParent = parent;
  ...
  // take cross product of lookat and up vector - and you're given one that
  // looks right... then cross that with the lookat and you get the real up
  Vector3d Aim(0, 0, 0);
  ...
  // mCamera = new Camera2( eye, lookat,
  // Vector3d(-0.408248,0.816497,-0.408248));

  // mCamera = new Camera2( Pos, Aim, Up);

  // mCamera = new PerspCamera( Pos, Aim, Up);
  mCamera = new PerspCamera(Vector3d(10, 10, 10), Vector3d(0, 0, 0),
                            Vector3d(0, 1, 0));
  ...
  // ArbitraryRotate(Vector3d(1, 0, 0), Vector3d(0, 1, 0), Vector3d(0, 0, 1), 0,
  // 0, Pos, Aim);
...
void GLWidget::redraw() { repaint(); }

void GLWidget::initializeGL() {

  setAutoBufferSwap(true);
  glClearColor(mViewportColor.redF(), mViewportColor.greenF(),
               mViewportColor.blueF(), mViewportColor.alphaF());
...
void GLWidget::enableGLLights() {
  ...
  // 	GLfloat light_position[] = { plight.position[0], plight.position[1],
  // plight.position[1], 1.0 }; //{ -1.0, 30.0, 5.0, 1.0 };
  //
...
void GLWidget::initCg() {
  if (mUseGPU) {
    ...
    cg->vertProgram = cgCreateProgramFromFile(
        cg->context, CG_SOURCE, programName, cg->vertProfile, NULL, NULL);
    checkForCgError("creating vertex program from file");
    // Fragment Program
    sprintf(programName, "%s", "fragShader.cg");
    cg->fragProgram = cgCreateProgramFromFile(
        cg->context, CG_SOURCE, programName, cg->fragProfile, NULL, NULL);
    ...
    // cg->renderToTexSize = cgGetNamedParameter( cg->fProgram,
    // "renderToTexSize" ); cg->objectID = cgGetNamedParameter( cg->fProgram,
    // "objectID" ); checkCgError( cg->context, 5 );
...
void GLWidget::paintEvent(QPaintEvent *event) {
  ...
  if (renderer) {
    ...
    if (mUseGPU) {
      ...
      cgGLSetParameter3f(cg->globalAmbient, mGlobalAmbient.redF(),
                         mGlobalAmbient.greenF(), mGlobalAmbient.blueF());
      // send lighing info
      cgGLSetParameter3f(cg->lightWarmColor, plight.warmcolor.r,
                         plight.warmcolor.g, plight.warmcolor.b);
      cgGLSetParameter3f(cg->lightCoolColor, plight.coolcolor.r,
                         plight.coolcolor.g, plight.coolcolor.b);
      cgGLSetParameter1f(cg->lightIntensity, plight.intensity);
      cgGLSetParameter3f(cg->lightPosition, plight.position[0],
                         plight.position[1], plight.position[2]);
      // object material
      cgSetParameter3f(cg->basecolor, 0.0, 0.0, 0.0);
      cgSetParameter3f(cg->Ka, mRenderColor.redF(), mRenderColor.greenF(),
                       mRenderColor.blueF());
  ...
  drawIDs(&painter, &model[0][0], &proj[0][0],
          &view[0]); // draw vertex, edge and face ids
...
void GLWidget::drawSelectionWindow(QPainter *painter) {
  if (mShowSelectionWindow) {
    ...
    painter->drawRect(mSelectionWindowStartX, height() - mSelectionWindowStartY,
                      x - mSelectionWindowStartX,
                      y - (height() - mSelectionWindowStartY));
...
void GLWidget::drawBrush(QPainter *painter) {
  if (mShowBrush) {
    ...
    int x = mapFromGlobal(QCursor::pos()).x();
    int y = mapFromGlobal(QCursor::pos()).y();
    painter->drawEllipse(x - mBrushSize / 2, y - mBrushSize / 2, mBrushSize,
                         mBrushSize);
...
// number of selected vertices, edges, faces, face vertices, and eventually
// objects other data that could be shown will be added later
void GLWidget::drawHUD(QPainter *painter) {
  if (mShowHUD) {
    ...
    QString s1 =
        "Vertices: " + QString("%1").arg((uint)object->num_vertices()) +
    ...
    painter->drawText(rectangle.left() + 65, rectangle.top() + 5, r3.width(),
                      rectangle.height(), Qt::AlignLeft, s3);
    painter->drawText(/*fm.width(s3)*/ 280, rectangle.top() + 5, r1.width(),
                      rectangle.height(), Qt::AlignLeft, s1);
    painter->drawText(/*fm.width(s3)+fm.width(s1)*/ 460, rectangle.top() + 5,
                      r2.width(), rectangle.height(), Qt::AlignLeft, s2);
  }
}

void GLWidget::drawIDs(QPainter *painter, const GLdouble *model,
  ...
  // Vector3d coord = viewport.camera.getPos(); //use this to find the dist from
  // the camera
  Vector3d coord =
      mCamera->getEye(); // use this to find the dist from the camera
  QBrush brush;
  ...
  if (mShowVertexIDs) {
    ...
    for (it = vparray.begin(); it != vparray.end(); it++) {
      ...
      max_dist =
          (vlsqr_array[count] > max_dist) ? vlsqr_array[count] : max_dist;
      min_dist =
          (vlsqr_array[count] < min_dist) ? vlsqr_array[count] : min_dist;
      count++;
    }
    count = 0;
    for (it = vparray.begin(); it != vparray.end(); it++) {
      ...
      if (max_dist == min_dist)
        ...
        d = (vlsqr_array[count] - min_dist) * (max_alpha - min_alpha) /
                (max_dist - min_dist) +
            min_alpha;
      ...
      painter->setPen(Qt::NoPen);
      brush =
          QBrush(QColor(mVertexIDBgColor.red(), mVertexIDBgColor.green(),
                        mVertexIDBgColor.blue(), min_alpha + max_alpha - d));
      painter->setBrush(brush);
  ...
  if (mShowEdgeIDs) {
    ...
    for (it = eparray.begin(); it != eparray.end(); it++) {
      ...
      max_dist =
          (elsqr_array[count] > max_dist) ? elsqr_array[count] : max_dist;
      min_dist =
          (elsqr_array[count] < min_dist) ? elsqr_array[count] : min_dist;
      count++;
    }
    count = 0;
    for (it = eparray.begin(); it != eparray.end(); it++) {
      ...
      if (max_dist == min_dist)
        ...
        d = (elsqr_array[count] - min_dist) * (max_alpha - min_alpha) /
                (max_dist - min_dist) +
            min_alpha;
      gluProject(x, y, z, model, proj, view, &win_x, &win_y, &win_z);
      win_y = height() - win_y; // y is inverted
      ...
      brush = QBrush(QColor(mEdgeIDBgColor.red(), mEdgeIDBgColor.green(),
                            mEdgeIDBgColor.blue(), min_alpha + max_alpha - d));
      painter->setBrush(brush);
      QRectF rectangle(win_x, win_y, 35, 20);
      painter->drawRoundRect(rectangle, 6, 6);
      painter->setPen(QColor(255, 255, 255, min_alpha + max_alpha - d));
  ...
  if (mShowFaceIDs) {
    ...
    for (it = fparray.begin(); it != fparray.end(); it++) {
      ...
      max_dist =
          (flsqr_array[count] > max_dist) ? flsqr_array[count] : max_dist;
      min_dist =
          (flsqr_array[count] < min_dist) ? flsqr_array[count] : min_dist;
      count++;
    }
    count = 0;
    for (it = fparray.begin(); it != fparray.end(); it++) {
      ...
      if (max_dist == min_dist)
        ...
        d = (flsqr_array[count] - min_dist) * (max_alpha - min_alpha) /
                (max_dist - min_dist) +
            min_alpha;
      gluProject(x, y, z, model, proj, view, &win_x, &win_y, &win_z);
      win_y = height() - win_y; // y is inverted
      painter->setPen(Qt::NoPen);
      brush = QBrush(QColor(mFaceIDBgColor.red(), mFaceIDBgColor.green(),
                            mFaceIDBgColor.blue(), min_alpha + max_alpha - d));
      painter->setBrush(brush);
      QRectF rectangle(win_x, win_y, 35, 20);
      painter->drawRoundRect(rectangle, 6, 6);
      painter->setPen(QColor(255, 255, 255, min_alpha + max_alpha - d));
...
void GLWidget::drawSelectedIDs(QPainter *painter, const GLdouble *model,
                               const GLdouble *proj, const GLint *view) {
  if (mShowSelectedIDs) {
    glDisable(GL_DEPTH_TEST);
    ...
    if (!mShowVertexIDs) {
      if (!object->sel_vptr_array.empty()) {
        ...
        first = object->sel_vptr_array.begin();
        last = object->sel_vptr_array.end();
        while (first != last) {
    ...
    if (!mShowEdgeIDs) {
      if (!object->sel_eptr_array.empty()) {
        ...
        first = object->sel_eptr_array.begin();
        last = object->sel_eptr_array.end();
        while (first != last) {
          QString id = QString::number((*first)->getID());
    ...
    if (!mShowFaceIDs) {
      if (!object->sel_fptr_array.empty()) {
        ...
        first = object->sel_fptr_array.begin();
        last = object->sel_fptr_array.end();
        while (first != last) {
          QString id = QString::number((*first)->getID());
          double x, y, z;
    ...
    if (!mShowFaceVertexIDs) {
      if (!object->sel_fvptr_array.empty()) {
        ...
        first = object->sel_fvptr_array.begin();
        last = object->sel_fvptr_array.end();
        while (first != last) {
...
// 		// resizeGL( QApplication::desktop()->size().width(),
...
DLFLVertexPtr GLWidget::selectVertex(int mx, int my, int w, int h) {
  ...
  // viewport.camera.enterSelectionMode(mx,my,30,30,vp); // Reduced sensitivity
  // for picking points
  mCamera->enterSelectionMode(mx, my, w, h,
                              vp); // Reduced sensitivity for picking points
...
DLFLLocatorPtr GLWidget::selectLocator(int mx, int my, int w, int h) // brianb
{
  ...
  GLint vp[4];
  glGetIntegerv(GL_VIEWPORT, vp);
  // viewport.camera.enterSelectionMode(mx,my,10,10,vp); // Reduced sensitivity
  // for picking points
  mCamera->enterSelectionMode(mx, my, w, h,
                              vp); // Reduced sensitivity for picking points
...
DLFLFacePtr GLWidget::deselectFaces(int mx, int my, int w, int h) {
  ...
  if (hits > 0) {
    ...
    while (hits) {
      ...
      // if
      // (DLFLObject::fparray[selectBuf[index+3]].normalCentroid()*camera.getPos()){
...
void GLWidget::drawSelected(void) {
  ...
  if (!object->sel_vptr_array.empty()) {
    ...
    // glBegin(GL_POINTS);
    glColor4f(mSelectedVertexColor.redF(), mSelectedVertexColor.greenF(),
              mSelectedVertexColor.blueF(), mSelectedVertexColor.alphaF());
    DLFLVertexPtrArray::iterator first, last;
    first = object->sel_vptr_array.begin();
    last = object->sel_vptr_array.end();
    while (first != last) {
  ...
  if (!object->sel_eptr_array.empty()) {
    ...
    glColor4f(mSelectedEdgeColor.redF(), mSelectedEdgeColor.greenF(),
              mSelectedEdgeColor.blueF(), mSelectedEdgeColor.alphaF());
    DLFLEdgePtrArray::iterator first, last;
    first = object->sel_eptr_array.begin();
    last = object->sel_eptr_array.end();
    while (first != last) {
  ...
  if (!object->sel_fptr_array.empty()) {
    glLineWidth(mSelectedEdgeThickness);
    glColor4f(mSelectedFaceColor.redF(), mSelectedFaceColor.greenF(),
              mSelectedFaceColor.blueF(), mSelectedFaceColor.alphaF());
    DLFLFacePtrArray::iterator first, last;
    first = object->sel_fptr_array.begin();
    last = object->sel_fptr_array.end();
    while (first != last) {
...
void GLWidget::wheelEvent(QWheelEvent *event) {
  ...
  // 						!(QApplication::keyboardModifiers() == (Qt::ShiftModifier
  // | Qt::AltModifier) ) ){
  // 	// QMessageBox::about(this, tr("false"),tr("numDegrees =
  // %1").arg(numDegrees)); 	viewport.handle_zoom(VPPush,x,y); // Start zooming
  ...
  // 	viewport.send_to_current(VPPush,x,y);
  //
  // 	viewport.handle_pan(VPPush,x,y+numDegrees*2); // Start panning up and
  // down 	viewport.send_to_current(VPPush,x,y);
  //
  // 	viewport.handle_pan(VPRelease,x,y+numDegrees*2); // Start panning up and
  // down 	viewport.send_to_current(VPRelease,x,y);
  //
  // 	//updateGL();
  // 	repaint();
  // }
...
void GLWidget::mousePressEvent(QMouseEvent *event) {
  ...
  if (QApplication::keyboardModifiers() == Qt::AltModifier ||
    ...
    mCamera->HandleMouseEvent(event->button(), event->type(), event->x(),
                              event->y());
    repaint();
  } else
    event->ignore();
...
// this has been modified for the QT interface, so it handles focus
// automatically
  ...
  if (QApplication::keyboardModifiers() == Qt::AltModifier ||
      event->buttons() == Qt::LeftButton &&
          QApplication::keyboardModifiers() ==
              (Qt::ShiftModifier | Qt::AltModifier) || // ) {
      event->buttons() ==
          Qt::MidButton) { // && QApplication::keyboardModifiers() ==
...
void GLWidget::mouseReleaseEvent(QMouseEvent *event) {
  ...
  // }
  // else if ( event->buttons() == Qt::RightButton &&
  // QApplication::keyboardModifiers() == Qt::ShiftModifier){ 	mBrushStartX = 0;
  // 	// mBrushStartY = 0;
  // }
  // else event->ignore();
}
...
//
// if ( view == VPPersp ) // Change camera settings to zoom in closer in
// perspective view 	viewport.setPerspView(10,10,10,0,0,0,0,1,0);
// }
...
void GLWidget::recomputeNormals(void) // Recompute normals and lighting
{
  ...
  if (mInPatchMode)
    ...
    computeLighting(object, NULL, &plight, mUseGPU);
}
...
void GLWidget::recomputePatches(
    const struct PatchParameters
        &parameters) // Recompute the patches for patch rendering
{

  if (mInPatchMode && patchObject) {
    patchObject->updatePatches(parameters, object);
  }
}
...
void GLWidget::createPatchObject() {
  ...
  // patchObject->loadTools((MainWindow)this->mParent);
  /*
  if( patchObject )
