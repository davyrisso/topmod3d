...
void MainWindow::load_texture() {
  QString fileName = QFileDialog::getOpenFileName(
      ...
      tr("All Supported Files (*.jpg *.png);;JPEG Files (*.jpg);;PNG Files "
         "(*.png);;All Files (*)"),
...
//        textured->setTexture(texfile);
//        texturedlit->setTexture(texfile);
//        mainWindowPtr->getActive()->valid(0); // To ensure new texture is
//        loaded into OpenGL mainWindowPtr->redraw(); Fl ::
//        focus(mainWindowPtr);
//      }
// }
...
void MainWindow::usePatchRenderer(const struct PatchParameters &parameters) {
  setRenderer(patch);
  active->recomputePatches(parameters);
  active->recomputeNormals();
...
void MainWindow::setSaveDirectory(QString s) { mSaveDirectory = s; }
...
void MainWindow::checkSaveDirectory() {
  ...
  if (dir.exists()) {
    ...
    QString s = QString("Directory\n\n%1\n\ndoes not exist, please type a "
                        "different directory name.\n\nCheck to make sure there "
                        "are no unnecessary spaces at the end.")
                    .arg(mSaveDirectory);
...
// Selection Menu.
void MainWindow::select_vertex() { setMode(MainWindow::SelectVertex); }

void MainWindow::edit_vertex() { setMode(MainWindow::EditVertex); }

void MainWindow::select_face() { setMode(MainWindow::SelectFace); }

void MainWindow::select_face_loop() { setMode(MainWindow::SelectFaceLoop); }

void MainWindow::select_edge_ring() { setMode(MainWindow::SelectEdgeRing); }
...
void MainWindow::selection_window() { setMode(MainWindow::SelectionWindow); }

void MainWindow::select_checkerboard_faces() {
  setMode(MainWindow::SelectCheckerboard);
}

void MainWindow::select_edge() { setMode(MainWindow::SelectEdge); }

void MainWindow::select_edge_loop() { setMode(MainWindow::SelectEdgeLoop); }

void MainWindow::select_corner() { setMode(MainWindow::SelectCorner); }
...
void MainWindow::changeWeight1(double value) { MainWindow::nwt1 = value; }

void MainWindow::toggleScherkCollins(int state) {
  scherk_collins = bool(state);
}

void MainWindow::changeNormalTwistingFactor(int factor) // Saied
{
  MainWindow::normal_twist_factor = factor / 180.0;
  qDebug() << "normal twist factor: "
           << QString::number(MainWindow::normal_twist_factor, 'g', 8);
}
...
void MainWindow::changeWeight2(double value) { MainWindow::nwt2 = value; }
...
// for modified corner cutting scheme... it's the same as the first step in the
// wireframe process
...
void MainWindow::performCutting() {
  ...
  undoPush();
  setModified(true);
  // void performCutting( DLFLObjectPtr obj, int type,float offsetE,float
  // offsetV,bool global,bool selected) { DLFL::performCutting(&object,
  // MainWindow::mode,MainWindow::cutOffsetE_factor,MainWindow::cutOffsetV_factor,MainWindow::global_cut,MainWindow::selected_cut);
  switch (selectionmask) {
  case MaskEdges:
    // std::cout << "cut by edge!\n";
    // active->getSelectedEdges(eparray);
    DLFL::cutSelectedEdges(&object, MainWindow::cutOffsetE_factor,
                           MainWindow::cutOffsetV_factor,
                           MainWindow::global_cut);
    break;
  case MaskVertices:
    // std::cout << "cut by vertex!\n";
    // active->getSelectedVertices(vparray);
    DLFL::cutSelectedVertices(&object, MainWindow::cutOffsetE_factor,
                              MainWindow::cutOffsetV_factor,
                              MainWindow::global_cut);
    break;
  case MaskFaces:
    ...
    DLFL::cutSelectedFaces(&object, MainWindow::cutOffsetE_factor,
                           MainWindow::cutOffsetV_factor,
                           MainWindow::global_cut);
    break;
  default:
...
void MainWindow::paintSelectedFaces() {
  ...
  for (int i = 0; i < active->numSelectedFaces(); ++i) {
    active->getSelectedFace(i)->setMaterial(object.addMaterial(
...
// void MainWindow::active->recomputeNormals(void)     // Recompute normals and
// lighting
// {
...
// }
//
// void MainWindow::active->recomputePatches(void) // Recompute the patches for
// patch rendering
// {
//   if(patchObject)
...
void MainWindow::subdivideSelectedEdges(void) // Subdivide all selected edges
{
  ...
  for (int i = 0; i < active->numSelectedEdges(); ++i) {
    subdivideEdge(&object, MainWindow::num_e_subdivs,
                  active->getSelectedEdge(i));
  }
...
void MainWindow::createMultiFaceHandle(
    void) // Create multi-face handle between selected faces
{
...
void MainWindow::subdivideDooSabin(void) // Doo-Sabin subdivision
{
  ...
  // QProgressDialog *progress = new QProgressDialog("Performing Doo Sabin
  // Remeshing...", "Cancel", 0, 1); progress->setMinimumDuration(2000);
  ...
  active->recomputePatches(this->mPatchMode->parameters);
...
{
  undoPush();
  DLFL::root4Subdivide(&object, MainWindow::weight_factor,
                       MainWindow::twist_factor);
  active->recomputePatches(mPatchMode->parameters);
  active->recomputeNormals();
  MainWindow::clearSelected();
...
void MainWindow::subdivideModifiedCornerCutting(
    void) // Modified Corner-cutting subdivision
{
  undoPush();
  DLFL::modifiedCornerCuttingSubdivide(
      &object, MainWindow::modified_corner_cutting_thickness);
  active->recomputePatches(mPatchMode->parameters);
  active->recomputeNormals();
  MainWindow::clearSelected();
...
void MainWindow::subdivideLinearVertexInsertion(
    void) // Bi-linear Vertex-insertion remeshing
{
  undoPush();
...
void MainWindow::subdivideCubicPentagonal(
    void) // Cubic Pentagonal remeshing scheme
{
...
void MainWindow::subdividePentagonPreserving(
    void) // Pentagon preserving remeshing
{
  undoPush();
...
void MainWindow::subdivideDualPentagonPreserving(
    void) // Dual Pentagon preserving remeshing
{
  // Implemented as Dual + Pentagonal + Dual
  undoPush();
  DLFL::createDual(&object, true); // Use accurate method
...
void MainWindow::subdivideDualHexagonPreserving(
    void) // Dual Hexagon Preserving remeshing
{
...
void MainWindow::subdivide1264(void) // 12-6-4 remeshing
{
  ...
  cmd += QString("subdivide(\"dual-12.6.4\",") +
         QString().setNum(MainWindow::dual1264_scale_factor) + QString(")\n");
  cmd += QString("dual()");
  emit echoCommand(cmd);
}

void MainWindow::subdivideDual1264(
    void) // Dual of 12-6-4 remeshing - Bei & Cansin
{
  undoPush();
  DLFL::dual1264Subdivide(&object, MainWindow::dual1264_scale_factor);
  active->recomputePatches(mPatchMode->parameters);
...
void MainWindow::subStellate2(void) // stellate subdivision - Eric
{
  undoPush();
  setModified(true);
  DLFL::twostellateSubdivide(&object, MainWindow::substellate_height,
                             MainWindow::substellate_curve);
  active->recomputePatches(mPatchMode->parameters);
  active->recomputeNormals();
  MainWindow::clearSelected();
...
void MainWindow::subdivideDome(void) // Bei & Cansin
{
  undoPush();
  setModified(true);
  DLFL::domeSubdivide(&object, MainWindow::domeLength_factor,
                      MainWindow::domeScale_factor);
  active->recomputePatches(mPatchMode->parameters);
  active->recomputeNormals();
  MainWindow::clearSelected();
...
void MainWindow::subdivideDooSabinBC(
    void) // Doo-Sabin(BC) subdivision - Bei & Cansin
{
  undoPush();
  setModified(true);
...
void MainWindow::subdivideDooSabinBCNew(void) // Doo-Sabin(BCNew) Bei & Cansin
{
  ...
  cmd +=
      QString().setNum(MainWindow::dooSabinBCnewLength_factor) + QString(")");
  emit echoCommand(cmd);
}
...
void MainWindow::globalStellate(void) {
  // Does not use length parameter for now. Uses subdivideFace method with
  // triangles
  undoPush();
  setModified(true);
...
// when the user presses the rind modeling button, and it will punch out all
// those faces
void MainWindow::createCrust2(bool use_scaling) {
  ...
  QString cmd = QString("rind(") + facelist + QString(",True,") +
                MainWindow::crust_thickness + QString(")");
  emit echoCommand(cmd);
}
...
void MainWindow::makeWireframe2() { // Create a wireframe // dave {
  ...
  undoPush();
  setModified(true);
  DLFL::makeWireframe2(&object, MainWindow::wireframe2_thickness,
...
void MainWindow::makeWireframeWithColumns(
    void) // Create a wireframe using columns
{
  undoPush();
  setModified(true);
  DLFL::makeWireframeWithColumns(&object, MainWindow::column_thickness,
                                 MainWindow::column_segments);
  active->recomputePatches(mPatchMode->parameters);
  active->recomputeNormals();
  MainWindow::clearSelected();
  redraw();
...
void MainWindow::randomAssignTexCoords(
    void) // Randomly assign texture coordinates
{
...
void MainWindow::assignTileTexCoords(
    void) // Assign texture coordinates for tile texturing
{
...
void MainWindow::getCommand() {
  ...
  // tr("%1").arg(text),tr("%1").arg(mCommandList.indexOf(text)));
  if (i > -1)
...
void MainWindow::initializeAnimatedHelp() {
  ...
  // mAnimatedHelpLabel->setSizePolicy(QSizePolicy::Ignored,
  // QSizePolicy::Ignored);
  ...
  mAnimatedHelpDockWidget->setFeatures(QDockWidget::DockWidgetMovable |
  ...
  mAnimatedHelpDockWidget->move(width(),
                                150 + mToolOptionsDockWidget->height() + 200);
}

void MainWindow::applyBranchCovering3v(
    const std::vector<HALF_EDGE_MAPPING> &mapping_guid,
    const BranchCovering_CornerCutting_Parameters &parameters) {
  qDebug() << "Call From applyBranchCovering3v Main window Call back";
}
...
void MainWindow::deleteSelected() {
  ...
  case MaskFaces:
    ...
    // 		for(eit = septrarr.begin(); eit != septrarr.end(); eit++)
    // 			DLFL::collapseEdge( &object, *eit,
    // MainWindow::delete_edge_cleanup);
    ...
    // 		// 			for(eit = septrarr.begin(); eit != septrarr.end();
    // eit++)
    // 		// 				DLFL::deleteEdge( &object, *eit,
    // MainWindow::delete_edge_cleanup);
    // 		// 		}
    // 		// 	}
  ...
  case MaskVertices:
    ...
    if (svptrarr.size() > 0 && svptrarr[0]) {
      ...
      while (!Q.empty()) {
        ...
        for (eit = septrarr.begin(); eit != septrarr.end(); eit++) {
          DLFLVertexPtr vp1, vp2;
          // get the two vertices for each edge, select them if they aren't
          // already selected
  ...
  case MaskFaces:
    ...
    if (sfptrarr.size() > 0 && sfptrarr[0]) {
      ...
      for (fit = sfptrarr.begin(); fit != sfptrarr.end(); fit++, i++) {
        ...
        for (eit = eptrarray.begin(); eit != eptrarray.end(); eit++) {
          // get the 2 faces for the current edge, check if both of them are
          // selected, flag the current face
          (*eit)->getFacePointers(fptr1, fptr2);
...
void MainWindow::reorderSelectedFaces() {
  ...
  // DLFLFacePtrArray sfptrarr;
  // vector<DLFLFacePtr>::iterator fit;
  // //loop through selected corners, get corresponding faces, reorder them to
  // start from the selected fvptr's svptrarr = active->getSelectedVertices();
  // if ( svptrarr[0] ) {
  // 	for(vit = svptrarr.begin(); vit != svptrarr.end(); vit++){
  // 		(*vit)->getFaces(sfptrarr);
  // 		for(fit = sfptrarr.begin(); fit != sfptrarr.end(); fit++){
