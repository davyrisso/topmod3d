...
protected:
  ...
  TMPatchPtrArray
      patcharray;       // Array of pointer to Bezier patches for this face
...
public:
  ...
  TMPatchFace(int psize = 4) : patchsize(psize), patcharray(), dlflface(NULL) {}
...
private:
  ...
  TMPatchFace(const TMPatchFace &tmpf)
      : patchsize(tmpf.patchsize), patcharray(tmpf.patcharray),
        dlflface(tmpf.dlflface) {}

  // Assignment operator
  TMPatchFace &operator=(const TMPatchFace &tmpf) {
    patchsize = tmpf.patchsize;
    patcharray = tmpf.patcharray;
    dlflface = tmpf.dlflface;
    return (*this);
  }
...
public:
  ...
  void createPatches(TMPatchMap &patchMap,
                     const struct PatchParameters &parameters);
  ...
  void computeLighting(LightPtr lightptr) {
    ...
    for (uint i = 0; i < patcharray.size(); ++i) {
      ...
      patcharray[i]->computeLighting(matl->color, matl->Ka, matl->Kd, matl->Ks,
                                     lightptr);
    }
  }
