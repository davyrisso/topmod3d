...
class ShortcutGetter : public QDialog {
  // Q_OBJECT
public:
  ShortcutGetter(QWidget *p = 0) : QDialog(p) {
...
enum NodeType { Null, Menu, Action };
...
void QShortcutDialog::retranslate() { retranslateUi(this); }
          ...
          foreach ( QString s, cxt )
          {
                  ...
                  for ( int j = 0; j < item->childCount(); j++ )
                  {
                          ...
                          if ( child->text(0) == s )
                          {
                                  ...
                                  child = new QTreeWidgetItem(QStringList(s),
  Menu); item->addChild(child); item = child; break;
                          }
                  }
          }
  ...
  */
  ...
  for (i = pManager->m_actions.begin(); i != pManager->m_actions.end(); i++) {
    ...
    QString name = cxt.takeAt(cxt.count() - 1),
            shortcut = i->at(0)->shortcut().toString();
...
void QShortcutDialog::on_twShortcuts_itemDoubleClicked(QTreeWidgetItem *i,
  ...
  ns = ShortcutGetter().exec(
      pManager->node(name, cxt.join("/")).attribute("shortcut"));
