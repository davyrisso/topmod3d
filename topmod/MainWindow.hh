...
public:
  ...
  enum Mode {
    NormalMode =
        0, /**< normal mode. does not allow any selection in the viewport. */
    SelectVertex = 1, /**< select one vertex. */
    SelectEdge = 2,   /**< select one edge. */
    SelectFace = 3,   /**< select one face. */
    SelectCorner = 4, /**< select one corner, which requires selection of a face
                         and a vertex. */
    MultiSelectVertex = 5,  /**< select multiple vertices. */
    MultiSelectEdge = 6,    /**< select multiple edges. */
    MultiSelectFace = 7,    /**< . */
    MultiSelectCorner = 8,  /**< . */
    SelectCheckerboard = 9, /**< select alternating faces forming a checkerboard
                               pattern. works well after Doo Sabin Remeshing. */
    ...
    SpliceCorners =
        15, /**< select two corners and collapse the face in between. */
    ...
    ExtrudeMultipleFaces =
        36,            /**< cubical extrusion for multiple faces at once. */
    ...
    CrustModeling =
        71, /**< create a crust of the current model and poke holes in it. */
    CutEdge = 200,          /**< // ozgur. */
    CutVertex = 201,        /**< // ozgur. */
    ...
    ConvexHullMode =
        301,         /**< create a convex hull or dual convex hull // ozgur. */
    ...
    SelectFaceLoop =
        83, /**< allows user to select one edge in the viewport and selects the
               corresponding face loop. shift select for multiple. */
    SelectSimilar = 84, /**< select all sub objects with the same number of
                           verts edges or faces whatever... . */
    SelectFacesByArea =
        856, /**< select all faces with the a similar surface area. */
    SelectFacesByColor =
        857,         /**< select all faces with the same color material. */
  ...
  /**
   * Enumerations for Selection Masking (e.g. vertices, faces, edges,
   * face-vertices, and eventually objects... ?)
   */
  enum SelectionMask {
    MaskObject = 0,
    MaskVertices,
  ...
  enum SpinBoxMode {
    One = 1, /**< the first spinbox in the current option panel will be
                controlled by the Y key. */
    Two, /**< the second spinbox in the current option panel will be controlled
            by the U key. */
    Three, /**< the third spinbox in the current option panel will be controlled
              by the I key. */
    Four, /**< the fourth spinbox in the current option panel will be controlled
             by the O key. */
    Five, /**< the fifth spinbox in the current option panel will be controlled
             by the P key. */
    Six,  /**< the sixth spinbox in the current option panel will be controlled
             by the [ key. */
  ...
  // Edge deletion
  static bool delete_edge_cleanup; //!< Flag for point-sphere cleanup after edge
                                   //!< deletion
  ...
  static bool
      dual_mesh_edges_check; //!< Flag to check for removal of dual mesh edges
  static bool hexagonalize_dodeca_extrude; //!< Flag to hexagonalize when doing
                                           //!< dodeca extrude
  ...
  static int column_segments; //!< Number of segments in each column for column
                              //!< modeling
  ...
  static double pentagonal_offset; //!< Offset value for pentagonal subdivision
                                   //!< (conversion)
  static double pentagonal_scale;  //!< Scale factor for pentagonal subdivision
                                   //!< (preserving)
  static bool accurate_dual; //!< Flag to indicate that accurate dual method
                             //!< should be used
  static double checkerboard_thickness; //!< Fractional thickness for
                                        //!< checkerboard remeshing
  static double
      modified_corner_cutting_thickness; //!< for modified corner cutting scheme
                                         //!< same as Thickness of crust for
                                         //!< wireframe

  // added by Ryan
  static double pinching_factor; //!< scaling factor for pinched handles
  static double
      pinch_center; //!< 0 to 1, the parametric cetner of the pinching factor
  static double bubble_factor; //!< 0 to 0.5, the offset from the pinch center
                               //!< to the bezier control points

  static double
      holeHandle_pinching_factor; //!< scaling factor for pinched handles
  ...
  static double
      domeExtrudeScale_factor; //!< Scale factor for dome extrusion //dave
  static double
      domeExtrudeRotation_factor; //!< Scale factor for dome extrusion //dave

  //!< Added by Bei & Cansin
  static double domeLength_factor; //!< Length for dome extrusion
  static double domeScale_factor;  //!< Scale factor for dome extrusion
  static double
      dual1264_scale_factor; //!< Scale factor for Dual 12-6-4 remeshing
  static double dooSabinBCnewScale_factor;  //!< Scale factor for new Doo-Sabin
                                            //!< remeshing scheme
  static double dooSabinBCnewLength_factor; //!< Length factor for new Doo-Sabin
                                            //!< remeshing scheme
  static double
      loopLength_factor; //!< Length factor for Loop-style remeshing scheme

  //!< Face subdivision
  static bool use_quads; //!< Flag indicating if face subdivision should use
                         //!< quads or triangles
  ...
  static bool mfh_use_max_offsets;  //!< Flag indicating whether to compute and
                                    //!< use the max. offset distances
  static double mfh_min_adj_factor; //!< Min. value of adjustment factor for
                                    //!< iterative convex hull computation
  static bool mfh_make_connections; //!< Should final connections be made or
                                    //!< should we stop with convex hull?

  //!< Menger Sponge creation
  static double sponge_thickness; //!< Thickness for Menger Sponge
  static double
      sponge_collapse_threshold; //!< Threshold angle for edge collapse
  static bool
      sponge_fractional_thickness; //!< Consider thickness to be fractional
...
protected:
  ...
  // TMPatchObject *patchObject;										//!< the patch
  // object
  ...
  void initialize(int x, int y, int w, int h,
                  DLFLRendererPtr rp); //!< Initialize the viewports, etc.
...
public:
  ...
  /**
   * \brief translation function. see tutorial at
   * http://trolltech.com/developer/knowledgebase/faq.2007-05-23.5871663589/
   */
  ...
  /**
   * \brief clears all currently selected vertices, edges, faces, whatever is
   * selected
   */
  ...
  /**
   * \brief current used for vertex editing, needs to be updated to allow
   * generic and multiple subobject translation/rotation/scale editing
   */
  ...
  void getCheckerboardSelection(
      DLFLFacePtr fptr); //!< \todo  needs to be moved to DLFL namespace
  void getEdgeLoopSelection(
      DLFLEdgePtr eptr); //!< \todo  needs to be moved to DLFL namespace
  void getFaceLoopSelection(
      DLFLEdgePtr eptr, bool start, DLFLFacePtr face_loop_marker,
      bool select_face_loop); //!< \todo  needs to be moved to DLFL namespace
  void getEdgeRingSelection(
      DLFLEdgePtr eptr, bool start, DLFLFacePtr face_loop_marker,
      bool select_face_loop); //!< \todo  needs to be moved to DLFL namespace
  ...
  void
  setExtrusionMode(ExtrusionMode m); //!< Switch to specified operating mode
  Mode getMode() {
    return mode;
  }; //!< returns the current operation mode enum... this needs to return a
     //!< string eventually. don't know how to do that yet
  void setRemeshingScheme(
      RemeshingScheme scheme); //!< switch the current remeshing scheme
  void setSelectionMask(SelectionMask m); //!< set the current selection mask
                                          //!< (verts, edges, faces, multiple?)
  void setToolOptions(
      QWidget *optionsWidget);     //!< set the current tool option widget to be
                                   //!< displayed in mToolOptionsDockWidget
  void loadFile(QString fileName); //!< load an OBJ or a DLFL file

  /**
   * \brief this will store pointers to the current mode's spinboxes so we can
   * do keyboard interaction with them
   */
  void setSpinBoxes(QDoubleSpinBox *one = 0, QDoubleSpinBox *two = 0,
                    QDoubleSpinBox *three = 0, QDoubleSpinBox *four = 0,
                    QDoubleSpinBox *five = 0, QDoubleSpinBox *six = 0);

#ifdef WITH_PYTHON
  DLFLScriptEditor *mScriptEditor; //!< ScriptEditor Object by Stuart
  QDockWidget
      *mScriptEditorDockWidget; //!< docked script editor window for Python
                                //!< Scripting interface by Stuart
#endif

#ifdef WITH_VERSE
  VerseTopMod *mVerseDialog; //!< for a possible future implementation of the
                             //!< Verse protocol http://verse.blender.org
  QDockWidget
      *mVerseDialogDockWidget; //!< for a possible future implementation of the
                               //!< Verse protocol http://verse.blender.org
#endif

  QDockWidget *mToolOptionsDockWidget; //!< the floating window that displays
                                       //!< the current tool's options
                                       //!< (spinboxes, checkboxes, etc...)
  QStackedWidget *mToolOptionsStackedWidget; //!< the widget that references
                                             //!< each tool option widget and
                                             //!< handles switching the display
...
protected:
  void
  closeEvent(QCloseEvent *event); //!< what will execute when the main window is
                                  //!< closed (on application exit/quit)

  SpinBoxMode
      mSpinBoxMode; //!< enum to store which spinbox mode we are in. e.g. 1, 2,
                    //!< 3, 4, 5, to allow mouse motion to update the values

  // Renderers
  static WireframeRendererPtr wired; //!< WireframeRenderer
  static NormalRendererPtr
      normal; //!< Normal Renderer - white color - like a "hidden line" view
  ...
  static ColorableRendererPtr
      colorable; //!< face colors... new ... by dave... 11/07

  BasicsMode *mBasicsMode; //!< widget that holds all displayable option widgets
                           //!< for basic operating modes (InsertEdge,
                           //!< DeleteEdge, CollapseEdge, ConnectEdges, etc...)
  ExtrusionsMode
      *mExtrusionsMode; //!< widget that holds all displayable option widgets
                        //!< for the extrusion operation modes
  ConicalMode *mConicalMode;     //!< for future implementation of Ozgur's
                                 //!< Conical/Planar modeling modes.
  RemeshingMode *mRemeshingMode; //!< widget that holds all displayable option
                                 //!< widgets for all remeshing modes
  HighgenusMode *mHighgenusMode; //!< high genus operation options (e.g.
                                 //!< wireframe, sierpinsky, add handle)
  TexturingMode *mTexturingMode; //!< \todo  texturing mode widgets (not working
                                 //!< at the moment)
  ExperimentalMode *mExperimentalMode; //!< experimental mode widgets like the
                                       //!< new paint bucket tool dave 11.07
  PatchMode *mPatchMode;               //!< The patch mode options
  BranchCovering_CornerCutting_Widget
      *mBranchCovering_CornerCutting; //!< Branch Covering with 3 valence-vertex
                                      //!< DLFL models
  ...
  QStandardItemModel
      *mActionModel; //!< stores all the actions in topmod. sent to
                     //!< CommandCompleter class in order to create an index of
                     //!< the possible actions based on the text and icon
                     //!< associated with each action
  QWidget
      *mActionListWidget; //!< widget that stores all actions availabe in topmod
                          //!< for CommandCompleter autocompletion functionality

private:
  // document modified
  bool mIsModified;   //!< bool to stop program from exiting if file has been
                      //!< edited but not saved
  bool mIsPrimitive;  //!< bool to stop program from exiting if file has been
                      //!< edited but not saved
  bool mWasPrimitive; //!< bool to stop program from exiting if file has been
                      //!< edited but not saved, used in DLFLUndo.cc
  ...
  void createActions(); //!< create all MainWindow actions for menu's and icons,
                        //!< also create operating mode actions in subclasses
  void
  createMenus(); //!< create top-level QMenuBar and add actions to each QMenu
  void createToolBars(); //!< create toolbars with large icons for one-click
                         //!< operations
  void
  createStatusBar(); //!< status bar will display extra info about what
                     //!< different operations or buttons do in TopMod interface
  bool maybeSave();  //!< save before exit?
  void initializeHelp(); //!< initialize the help files / create index / load
                         //!< html files

  void
  createStartupDialog(); //!< initialize the startup screen that will show links
                         //!< to beginner video tutorials / will include link to
                         //!< quicktime website and "disable checkbox"
  void initializeAnimatedHelp(); //!< initialize the in-context help animated
                                 //!< screen captures. these will display in a
                                 //!< small floatable window to the right

  // QAssistantClient *mAssistantClient;						//!< Qt help file
  // viewer, will display html files created by DocBook xml transformation

  // top level and sublevel menus
  QMenuBar
      *menuBar; //!< the main menubar pointer to which all qmenu's will be added
  QMenu *mFileMenu; //!< the file menu which contains save, save as, quit etc...
  QMenu *mExportMenu;
  QMenu *mEditMenu;    //!< undo, redo, clear undo list, and also preferences
                       //!< (except on os/x)
  QMenu *mDisplayMenu; //!< all viewport display functionality (HUD, IDs,
                       //!< renderer, verts, faces, normals, etc...)
  QMenu *mViewMenu; //!< probably going to remove... but could contain choices
                    //!< for which view to use (persp or left / right /top/
                    //!< bottom etc...)
  ...
  QMenu *mPrimitivesMenu; //!< stores actions for one-click loading of primitive
                          //!< objects
  QMenu *mNewMenu; //!< stores same actions as the primitives menu plus a "new
                   //!< file" command that loads a blank object
  QMenu *mObjectMenu;        //!< \todo  needs to be removed. not necessary
  QMenu *mSelectionMenu;     //!< all different selection options
  QMenu *mSelectionMaskMenu; //!< \todo  figure out how to integrate a selection
                             //!< mask functionality into the current
                             //!< implementation of mode switching in TopMod
  QMenu
      *mLanguageMenu; //!< \todo  add multi-lingual support using Qt's Linguist
                      //!< application and make dynamic language changing
                      //!< possible... could be very difficult to do...
  QMenu *mBranchCovering; //!< This menu includes options for branch covering --
                          //!< Saied

  QMenu *mWindowMenu; //!< menu choices for hiding and showing all floating
                      //!< windows and toolbars / also fullscreen mode
  QMenu *mHelpMenu;   //!< about TopMod, about Qt, Local documentation, Online
                      //!< documentation
  ...
  QAction *mExtrudeMultipleAct; //!< temporary for now... not sure how to handle
                                //!< this in the future...
  ...
  QAction
      *mPerspViewAct; //!< switch to perspective view or reset the current one
  ...
  QDoubleSpinBox *mSpinBoxOne, *mSpinBoxTwo, *mSpinBoxThree, *mSpinBoxFour,
      *mSpinBoxFive, *mSpinBoxSix;
...
public slots:
  ...
  void applyBranchCovering3v(
      const std::vector<HALF_EDGE_MAPPING> &mapping_guid,
  ...
  bool appScreenshot();      //!< take a screenshot of the whole app (for forum
                             //!< posting and bug reports)
  ...
  void topModResearch();  //!< open the TopMod research web page in the default
                          //!< browser
  void documentWasModified();
  void toggleFullScreen(); //!< fullscreen mode in the mainwindow class... this
                           //!< is a test...

  void getCommand(); //!< this will open up the quicksilver like interface and
  ...
  void
  changeFaceAreaTolerance(double value); //!< dave - facial area calculation
  ...
  // void writeObjectOBJ(const char * filename, bool with_normals=false, bool
  // with_tex_coords=false); void writeObjectDLFL(const char * filename);
  ...
  // void setPatchRenderer(); // just testing out the signals/slots
  // functionality of Qt
  ...
  void
  redraw(); //!< Override redraw() method to mark subwindows also for redraw
  ...
  void performExtrusion(); //!< Generic method for all extrusion schemes on
                           //!< multiple faces
  ...
  void writeObject(const char *filename, const char *mtlfilename = NULL,
...
signals:
#ifdef WITH_PYTHON
  void loadedObject(DLFLObject *obj,
                    QString fileName); // echo command for loading object
#endif
