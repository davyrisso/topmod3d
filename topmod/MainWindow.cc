...
// face area - dave
float MainWindow::face_area_tolerance = 0.05f;
...
double MainWindow::normal_twist_factor = 0.0; // Saied
...
double MainWindow::sponge_collapse_threshold =
    0.0; // Threshold factor for edge collapse
...
QColor MainWindow::paint_bucket_color = QColor(125, 125, 125);
...
MainWindow::MainWindow(char *filename)
  ...
  connect(mAutoSaveTimer, SIGNAL(timeout()), this,
          SLOT(saveFile(
              /*normals and texture options should go here... eventually*/)));
...
// QSettings Path for windows
  ...
  active = new GLWidget(500, 600, lit, QColor(255, 255, 255, 255),
                        QColor(255, 255, 255, 255), &object, fmt, this);
  ...
  mScriptEditorDockWidget->setAllowedAreas(Qt::BottomDockWidgetArea |
  ...
  connect(this, SIGNAL(loadedObject(DLFLObject *, QString)), mScriptEditor,
          SLOT(loadObject(DLFLObject *, QString)));
  connect(this, SIGNAL(echoCommand(QString)), mScriptEditor,
          SLOT(echoCommand(QString)));
  ...
  connect(mScriptEditor, SIGNAL(cmdExecuted()), this->getActive(),
          SLOT(update()));
  connect(mScriptEditor, SIGNAL(requestObject(QString)), this,
  ...
  mVerseDialogDockWidget->setAllowedAreas(Qt::BottomDockWidgetArea |
                                          Qt::TopDockWidgetArea);
  ...
  mToolOptionsDockWidget =
      new QDockWidget(tr("Tool Options - Insert Edge"), this);
  // mToolOptionsDockWidget->setFeatures(QDockWidget::DockWidgetFloatable |
  ...
  mPreferencesDialog =
      new TopModPreferences(mSettings, mStyleSheetEditor, sm, this);
  ...
  //  mToolOptionsDockWidget->setGeometry(10 +
  //  this->x()/*this->active->width()-mToolOptionsDockWidget->width()*/,this->y()+150,mToolOptionsDockWidget->width(),mToolOptionsDockWidget->height());
...
void MainWindow::createActions() {
  ...
  mSaveAsAct = new QAction(QIcon(":images/document-save-as.png"),
                           tr("Save &As..."), this);
  ...
  mSavePatchesAct =
      new QAction(QIcon(":images/saveas.png"), tr("Save &Patch OBJ..."), this);
  sm->registerAction(mSavePatchesAct, "File Menu", "");
  mSavePatchesAct->setStatusTip(tr("Save a bezier patch .obj file"));
  connect(mSavePatchesAct, SIGNAL(triggered()), this,
          SLOT(saveFileBezierOBJ()));
  mActionListWidget->addAction(mSavePatchesAct);

  mSaveLG3dAct = new QAction(QIcon(":images/saveas.png"),
                             tr("Export LiveGrahpics3D..."), this);
  sm->registerAction(mSaveLG3dAct, "File Menu", "CTRL+ALT+S");
  mSaveLG3dAct->setStatusTip(
      tr("Export a LiveGraphics3D (*.m) file for embedding into the TopMod "
         "Wiki, Warning: you cannot import this file back into TopMod"));
  connect(mSaveLG3dAct, SIGNAL(triggered()), this, SLOT(saveFileLG3d()));
  mActionListWidget->addAction(mSaveLG3dAct);

  mSaveLG3dSelectedAct = new QAction(QIcon(":images/saveas.png"),
                                     tr("Export LG3d (Sel. Faces)..."), this);
  sm->registerAction(mSaveLG3dSelectedAct, "File Menu", "");
  mSaveLG3dSelectedAct->setStatusTip(
      tr("Export a LiveGraphics3D (*.m) of the current selected faces file for "
         "embedding into the TopMod Wiki, Warning: you cannot import this file "
         "back into TopMod"));
  connect(mSaveLG3dSelectedAct, SIGNAL(triggered()), this,
          SLOT(saveFileLG3dSelected()));
  ...
  mExportSTLAct->setStatusTip(
      tr("Export a stereolithography (*.stl) file for use with various rapid "
         "prototyping software and hardware"));
  connect(mExportSTLAct, SIGNAL(triggered()), this, SLOT(saveFileSTL()));
  mActionListWidget->addAction(mExportSTLAct);

  mScreenshotViewportAct = new QAction(QIcon(":images/camera-photo.png"),
                                       tr("Save Viewport Screenshot..."), this);
  sm->registerAction(mScreenshotViewportAct, "File Menu", "0");
  mScreenshotViewportAct->setStatusTip(
      tr("Save a screenshot of the contents of the OpenGL viewport (*.png)"));
  connect(mScreenshotViewportAct, SIGNAL(triggered()), this,
          SLOT(viewportScreenshot()));
  mActionListWidget->addAction(mScreenshotViewportAct);

  mScreenshotAppAct = new QAction(QIcon(":images/image-x-generic.png"),
                                  tr("Save App Screenshot..."), this);
  sm->registerAction(mScreenshotAppAct, "File Menu", "9");
  mScreenshotAppAct->setStatusTip(
      tr("Save a screenshot of the entire main application window (*.png)"));
...
// mActionListWidget->addAction(mQuickCommandAct);
  ...
  mClearUndoListAct->setStatusTip(
      tr("Clear the Undo List to free up some memory"));
  ...
  // connect(mBottomViewAct, SIGNAL(triggered()), this,
  // SLOT(switchBottomView()));
  ...
  mFullscreenAct = new QAction(QIcon(":images/view-fullscreen.png"),
                               tr("&Full Screen"), this);
  ...
  connect(showVerticesAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleVertices()));
  ...
  connect(mShowFaceIDsAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleFaceIDs()));
  ...
  connect(mShowEdgeIDsAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleEdgeIDs()));
  ...
  connect(mShowVertexIDsAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleVertexIDs()));
  ...
  connect(mShowSelectedIDsAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleSelectedIDs()));
  ...
  connect(showSilhouetteAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleSilhouette()));
  ...
  connect(showWireframeAct, SIGNAL(triggered()), this->getActive(),
  ...
  connect(showCoordinateAxesAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleAxes()));
  ...
  connect(objectOrientationAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleObjectOrientation()));
  ...
  connect(mShowNormalsAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleNormals()));
  ...
  // objectOrienationAct->setStatusTip(tr("Copy the current selection's contents
  // to the "
  connect(mShowFaceCentroidsAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleFaceCentroids()));
  ...
  connect(showHUDAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleHUD()));
  ...
  connect(mUseGPUAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleGPU()));
  ...
  connect(mAntialiasingAct, SIGNAL(triggered()), this->getActive(),
          SLOT(toggleAntialiasing()));
  mActionListWidget->addAction(mAntialiasingAct);

#ifdef WITH_PYTHON
  mShowScriptEditorAct =
      mScriptEditorDockWidget
          ->toggleViewAction(); // new QAction(tr("Show Script &Editor"), this);
  mShowScriptEditorAct->setStatusTip(
      tr("Show the script editor to execute DLFL commands"));
...
#ifdef WITH_VERSE
  mShowVerseDialogAct =
      mVerseDialogDockWidget
          ->toggleViewAction(); // new QAction(tr("Show Verse &Dialog"), this);
  // mShowVerseDialogAct->setText(// true);
  mShowVerseDialogAct->setStatusTip(
      tr("Show the verse dialog to view verse server connection status"));
  // connect(mShowVerseDialogAct, SIGNAL(triggered()), this,
  // SLOT(showHideVerseDialog()));
  ...
  mShowStartupDialogAct->setStatusTip(
      tr("Show the startup screen with links to video tutorials"));
  sm->registerAction(mShowStartupDialogAct, "Window Menu", "");
  mActionListWidget->addAction(mShowStartupDialogAct);

  mShowAnimatedHelpAct =
      mAnimatedHelpDockWidget
          ->toggleViewAction(); // new QAction(tr("Show Animated Hel&p"), this);
  // mShowAnimatedHelpAct->setCheckable(true);
  mShowAnimatedHelpAct->setStatusTip(tr("Show/Hide the animated help window"));
  // connect(mShowAnimatedHelpAct, SIGNAL(triggered()), this,
  // SLOT(showHideAnimatedHelp()));
  ...
  connect(polygonalModelingAct, SIGNAL(triggered()), this,
          SLOT(setPolygonalMode()));
  ...
  patchModelingAct->setStatusTip(tr("Switch to patch modeling mode"));
  // set tool options
  mPatchMode = new PatchMode(this);
  mToolOptionsStackedWidget->addWidget(mPatchMode);
  connect(patchModelingAct, SIGNAL(triggered()), this,
          SLOT(setPachModeToolOptions()));
  connect(mPatchMode, SIGNAL(ParametersUpdated()), this, SLOT(setPatchMode()));
  connect(mPatchMode, SIGNAL(ConvertToTopMode()), this,
          SLOT(converPatchesToDLFL()));
  connect(mPatchMode, SIGNAL(SaveAsOBJ()), this, SLOT(SaveDLFLPatchesAsOBJ()));
  connect(mPatchMode, SIGNAL(PatchRenderUpdateRequest()), getActive(),
          SLOT(update()));

  mBranchCovering_CornerCutting = new BranchCovering_CornerCutting_Widget(this);
  mToolOptionsStackedWidget->addWidget(mBranchCovering_CornerCutting);
  connect(mBranchCovering_CornerCutting,
          SIGNAL(Apply(std::vector<HALF_EDGE_MAPPING>,
                       BranchCovering_CornerCutting_Parameters)),
          this,
          SLOT(applyBranchCovering3v(std::vector<HALF_EDGE_MAPPING>,
                                     BranchCovering_CornerCutting_Parameters)));

  emptyToolMode = new QWidget(this);
  mToolOptionsStackedWidget->addWidget(emptyToolMode);
  clearToolOptions();
  ...
  wireframeRendererAct->setStatusTip(
      tr("Switch the current renderer to Wireframe"));
  connect(wireframeRendererAct, SIGNAL(triggered()), this,
          SLOT(useWireframeRenderer()));
  ...
  connect(normalRendererAct, SIGNAL(triggered()), this,
          SLOT(useNormalRenderer()));
  ...
  lightedRendererAct->setStatusTip(
      tr("Switch the current renderer to Lighted"));
  connect(lightedRendererAct, SIGNAL(triggered()), this,
          SLOT(useLightedRenderer()));
  ...
  texturedRendererAct->setStatusTip(
      tr("Switch the current renderer to Textured"));
  connect(texturedRendererAct, SIGNAL(triggered()), this,
          SLOT(useTexturedRenderer()));
  ...
  texturedLightedAct->setStatusTip(
      tr("Switch the current renderer to Textured Lit"));
  connect(texturedLightedAct, SIGNAL(triggered()), this,
          SLOT(useTexturedLitRenderer()));
  ...
  // connect(patchRendererAct, SIGNAL(triggered()), this,
  // SLOT(usePatchRenderer())); mActionListWidget->addAction(patchRendererAct);
  ...
  colorableRendererAct->setStatusTip(
      tr("Switch the current renderer to Colorable"));
  connect(colorableRendererAct, SIGNAL(triggered()), this,
          SLOT(useColorableRenderer()));
  ...
  pOctahedronAct = new QAction(QIcon(":/images/prim_octahedron.png"),
                               tr("&Octahedron"), this);
  ...
  pTetrahedronAct = new QAction(QIcon(":/images/prim_tetrahedron.png"),
                                tr("&Tetrahedron"), this);
  ...
  pDodecahedronAct = new QAction(QIcon(":/images/prim_dodecahedron.png"),
                                 tr("&Dodecahedron"), this);
  sm->registerAction(pDodecahedronAct, "Primitives Menu", "CTRL+4");
  pDodecahedronAct->setStatusTip(tr("Load a dodecahedron"));
  connect(pDodecahedronAct, SIGNAL(triggered()), this,
          SLOT(loadDodecahedron()));
  mActionListWidget->addAction(pDodecahedronAct);

  pIcosahedronAct = new QAction(QIcon(":/images/prim_icosahedron.png"),
                                tr("&Icosahedron"), this);
  ...
  pSoccerBallAct = new QAction(QIcon(":/images/prim_soccerball.png"),
                               tr("&Soccer ball"), this);
  ...
  pGeodesicAct = new QAction(QIcon(":/images/prim_geodesic.png"),
                             tr("&Geodesic Dome"), this);
  ...
  // subdivideAllEdgesAct->setStatusTip(tr("Copy the current selection's
  // contents to the "
  connect(subdivideAllEdgesAct, SIGNAL(triggered()), this,
          SLOT(subdivideAllEdges()));
  ...
  // planarizeAllFacesAct->setStatusTip(tr("Copy the current selection's
  // contents to the "
  connect(planarizeAllFacesAct, SIGNAL(triggered()), this,
          SLOT(planarizeFaces()));
  ...
  // makeObjectSphericalAct->setStatusTip(tr("Copy the current selection's
  // contents to the "
  connect(makeObjectSphericalAct, SIGNAL(triggered()), this,
          SLOT(spheralizeObject()));
  ...
  // mCleanup2gonsAct->setStatusTip(tr("Copy the current selection's contents to
  // the "
  ...
  connect(mCleanupWingedVerticesAct, SIGNAL(triggered()), this,
          SLOT(cleanupWingedVertices()));
  ...
  connect(mSplitValence2VerticesAct, SIGNAL(triggered()), this,
          SLOT(splitValence2Vertices()));
  ...
  // makeObjectSmoothAct->setStatusTip(tr("Copy the current selection's contents
  // to the "
  ...
  mPerformCuttingAct->setStatusTip(tr("Cut selected faces, edges, or vertices "
                                      "based on the current selection mask"));
  connect(mPerformCuttingAct, SIGNAL(triggered()), this,
          SLOT(performCutting()));
  ...
  connect(createCrustScalingAct, SIGNAL(triggered()), this,
          SLOT(crustModeling4()));
  ...
  connect(createCrustThicknessAct, SIGNAL(triggered()), this,
          SLOT(crustModeling3()));
  ...
  makeWireframeAct->setStatusTip(
      tr("Create a wireframe model using the current options"));
  ...
  makeColumnsAct->setStatusTip(
      tr("Create a column model using the current options"));
  connect(makeColumnsAct, SIGNAL(triggered()), this,
          SLOT(makeWireframeWithColumns()));
  ...
  connect(makeSierpinskiAct, SIGNAL(triggered()), this,
          SLOT(multiConnectMidpoints()));
  ...
  // computeLightingAct->setStatusTip(tr("Copy the current selection's contents
  // to the "
  connect(computeLightingAct, SIGNAL(triggered()), getActive(),
          SLOT(recomputeLighting()));
  ...
  // computeNormalsAndLightingAct->setStatusTip(tr("Copy the current selection's
  // contents to the "
  connect(computeNormalsAndLightingAct, SIGNAL(triggered()), getActive(),
          SLOT(recomputeNormals()));
  ...
  // assignTextureCoordinatesAct->setStatusTip(tr("Copy the current selection's
  // contents to the "
  connect(assignTextureCoordinatesAct, SIGNAL(triggered()), this,
          SLOT(assignTileTexCoords()));
  ...
  connect(mSelectionWindowAct, SIGNAL(triggered()), this,
          SLOT(selection_window()));
  ...
  connect(selectMultipleVerticesAct, SIGNAL(triggered()), this,
          SLOT(select_multiple_vertices()));
  ...
  connect(mSelectMultipleAct, SIGNAL(triggered()), this,
          SLOT(select_multiple()));
  ...
  connect(selectFaceLoopAct, SIGNAL(triggered()), this,
          SLOT(select_face_loop()));
  ...
  connect(selectMultipleFacesAct, SIGNAL(triggered()), this,
          SLOT(select_multiple_faces()));
  ...
  connect(selectSimilarFacesAct, SIGNAL(triggered()), this,
          SLOT(select_similar_faces()));
  ...
  connect(selectFacesByAreaAct, SIGNAL(triggered()), this,
          SLOT(select_faces_by_area()));
  ...
  connect(selectFacesByColorAct, SIGNAL(triggered()), this,
          SLOT(select_faces_by_color()));
  ...
  connect(selectCheckerboardFacesAct, SIGNAL(triggered()), this,
          SLOT(select_checkerboard_faces()));
  ...
  connect(mSelectComponentAct, SIGNAL(triggered()), this,
          SLOT(selectComponent()));
  ...
  connect(mShrinkSelectionAct, SIGNAL(triggered()), this,
          SLOT(shrinkSelection()));
  ...
  connect(selectMultipleEdgesAct, SIGNAL(triggered()), this,
          SLOT(select_multiple_edges()));
  ...
  connect(mCollapseSelectedEdgesAct, SIGNAL(triggered()), this,
          SLOT(collapseSelectedEdges()));
  ...
  connect(selectEdgeLoopAct, SIGNAL(triggered()), this,
          SLOT(select_edge_loop()));
  ...
  connect(selectEdgeRingAct, SIGNAL(triggered()), this,
          SLOT(select_edge_ring()));
  ...
  connect(exitSelectionModeAct, SIGNAL(triggered()), this,
          SLOT(exit_selection_mode()));
  ...
  connect(clearSelectedModeAct, SIGNAL(triggered()), this,
          SLOT(clear_selected()));
  ...
  connect(selectEdgesFromFacesAct, SIGNAL(triggered()), this,
          SLOT(selectEdgesFromFaces()));
  ...
  connect(selectEdgesFromVerticesAct, SIGNAL(triggered()), this,
          SLOT(selectEdgesFromVertices()));
  ...
  connect(selectFacesFromEdgesAct, SIGNAL(triggered()), this,
          SLOT(selectFacesFromEdges()));
  ...
  connect(selectFacesFromVerticesAct, SIGNAL(triggered()), this,
          SLOT(selectFacesFromVertices()));
  ...
  connect(selectVerticesFromEdgesAct, SIGNAL(triggered()), this,
          SLOT(selectVerticesFromEdges()));
  ...
  connect(selectVerticesFromFacesAct, SIGNAL(triggered()), this,
          SLOT(selectVerticesFromFaces()));
  mActionListWidget->addAction(selectVerticesFromFacesAct);

  // selection masks
  mSelectVerticesMaskAct =
      new QAction(QIcon(":images/selection-mask-vertices.png"),
                  tr("Select &Vertices"), this);
  mSelectVerticesMaskAct->setCheckable(true);
  sm->registerAction(mSelectVerticesMaskAct, "Selection", "SHIFT+V");
  mSelectVerticesMaskAct->setStatusTip(
      tr("Select by Component type: Vertices"));
  connect(mSelectVerticesMaskAct, SIGNAL(triggered()), this,
          SLOT(selectionMaskVertices()));
  mActionListWidget->addAction(mSelectVerticesMaskAct);

  mSelectEdgesMaskAct = new QAction(QIcon(":images/selection-mask-edges.png"),
                                    tr("Select &Edges"), this);
  ...
  connect(mSelectEdgesMaskAct, SIGNAL(triggered()), this,
          SLOT(selectionMaskEdges()));
  mActionListWidget->addAction(mSelectEdgesMaskAct);

  mSelectFacesMaskAct = new QAction(QIcon(":images/selection-mask-faces.png"),
                                    tr("Select &Faces"), this);
  ...
  connect(mSelectFacesMaskAct, SIGNAL(triggered()), this,
          SLOT(selectionMaskFaces()));
  mActionListWidget->addAction(mSelectFacesMaskAct);

  mSelectCornersMaskAct = new QAction(
      QIcon(":images/selection-mask-corners.png"), tr("Select &Corner"), this);
  ...
  connect(mSelectCornersMaskAct, SIGNAL(triggered()), this,
          SLOT(selectionMaskCorners()));
  ...
  // SETTINGS ACTIONS
  mPreferencesAct = new QAction(QIcon(":images/preferences-system.png"),
                                tr("&Preferences"), this);
  ...
  connect(mVerseConnectLocalhostAct, SIGNAL(triggered()), mVerseDialog,
          SLOT(connectLocalhost()));
  ...
  connect(mVerseConnectAct, SIGNAL(triggered()), mVerseDialog,
          SLOT(connectHost()));
  ...
  connect(mVerseDisconnectAct, SIGNAL(triggered()), mVerseDialog,
          SLOT(disconnectHost()));
  ...
  connect(mVerseDisconnectAllAct, SIGNAL(triggered()), mVerseDialog,
          SLOT(disconnectAll()));
  ...
  connect(mVerseStartServerAct, SIGNAL(triggered()), mVerseDialog,
          SLOT(startServer()));
  ...
  connect(mVerseKillServerAct, SIGNAL(triggered()), mVerseDialog,
          SLOT(killServer()));
  ...
  connect(mPaintSelectedFacesAct, SIGNAL(triggered()), this,
          SLOT(paintSelectedFaces()));
  ...
  connect(mClearMaterialsAct, SIGNAL(triggered()), this,
          SLOT(clearMaterials()));
  ...
  connect(mSubdivideSelectedFacesAct, SIGNAL(triggered()), this,
          SLOT(subdivideSelectedFaces()));
  ...
  connect(mSubdivideSelectedEdgesAct, SIGNAL(triggered()), this,
          SLOT(subdivideSelectedEdges()));
  ...
  mPerformRemeshingAct->setStatusTip(
      tr("Perform the current remeshing scheme"));
  connect(mPerformRemeshingAct, SIGNAL(triggered()), this,
          SLOT(performRemeshing()));
  ...
  mPerformExtrusionAct->setStatusTip(
      tr("Perform the current extrusion operator on the selected faces"));
  connect(mPerformExtrusionAct, SIGNAL(triggered()), this,
          SLOT(performExtrusion()));
  ...
  mExtrudeMultipleAct->setStatusTip(
      tr("Check this if you want to be able to select multiple faces and then "
         "hit CTRL+X to extrude the selected faces"));
  // connect(mExtrudeMultipleAct, SIGNAL(triggered()), this,
  // SLOT(performExtrusion()));
  sm->registerAction(mExtrudeMultipleAct, "Tools Menu", "M");
  mActionListWidget->addAction(mExtrudeMultipleAct);

  // help menu actions
  mHelpAct = new QAction(QIcon(":images/applications-internet.png"),
                         tr("&Online User Manual"), this);
  ...
  mVideoTutorialsAct =
      new QAction(QIcon(":images/applications-internet.png"),
                  tr("&TopMod Video Tutorials on blip.tv"), this);
  ...
  mTopModWebAct = new QAction(QIcon(":images/applications-internet.png"),
                              tr("&TopMod on the Web"), this);
  ...
  mTopModResearchAct = new QAction(QIcon(":images/applications-internet.png"),
                                   tr("&TopMod Research Papers"), this);
  mTopModResearchAct->setStatusTip(tr("Go to the TopMod Research web page"));
  connect(mTopModResearchAct, SIGNAL(triggered()), this,
          SLOT(topModResearch()));
  sm->registerAction(mTopModResearchAct, "Help Menu", "");
  mActionListWidget->addAction(mTopModResearchAct);

  mCheckForUpdatesAct = new QAction(QIcon(":images/applications-internet.png"),
                                    tr("&Check for Updates"), this);
  mCheckForUpdatesAct->setStatusTip(tr("Check for updates to TopMod Online"));
  connect(mCheckForUpdatesAct, SIGNAL(triggered()), this,
...
void MainWindow::createMenus() {
  ...
  // this is now a submenu of Display to make everything on the menu bar more
  // compact
  ...
  // branch covering menu added by Saied
  mBranchCovering = new QMenu(tr("&Branch Covering"));
  menuBar->addMenu(mBranchCovering);
  mBranchCovering_CornerCutting_Action =
      new QAction(tr("Corner Cutting"), this);
  connect(mBranchCovering_CornerCutting_Action, SIGNAL(triggered()), this,
          SLOT(triggerBranchCovering_CornerCutting()));
  mBranchCovering->addAction(mBranchCovering_CornerCutting_Action);
...
void MainWindow::createToolBars() {
  ...
  mBasicsMode->addActions(mToolsActionGroup, mToolsToolBar,
                          mToolOptionsStackedWidget);
  mExtrusionsMode->addActions(mToolsActionGroup, mExtrusionToolBar,
                              mToolOptionsStackedWidget);
  mConicalMode->addActions(mToolsActionGroup, mConicalToolBar,
                           mToolOptionsStackedWidget);

  mRemeshingActionGroup = new QActionGroup(this);
  mRemeshingMode->addActions(mToolsActionGroup, mRemeshingToolBar,
                             mToolOptionsStackedWidget);

  mHighgenusMode->addActions(mToolsActionGroup, mHighgenusToolBar,
                             mToolOptionsStackedWidget);
  mTexturingMode->addActions(mToolsActionGroup, mTexturingToolBar,
                             mToolOptionsStackedWidget);
  mExperimentalMode->addActions(mToolsActionGroup, mExperimentalToolBar,
                                mToolOptionsStackedWidget);
...
void MainWindow::createStartupDialog() {
  ...
  // mStartupDialogDockWidget->setFeatures(QDockWidget::DockWidgetFloatable |
  // QDockWidget::DockWidgetMovable /* | QDockWidget::DockWidgetClosable*/);
  ...
  mTutorialNavigationAct = new QAction(QIcon(":/images/tutorial_01.png"),
                                       tr("Navigation Basics"), this);
  connect(mTutorialNavigationAct, SIGNAL(triggered()), this,
          SLOT(loadNavigationTutorial()));
  ...
  mTutorialInterfaceAct = new QAction(QIcon(":/images/tutorial_02.png"),
                                      tr("Interface Basics"), this);
  connect(mTutorialInterfaceAct, SIGNAL(triggered()), this,
          SLOT(loadInterfaceTutorial()));
  ...
  mTutorialBasicAct = new QAction(QIcon(":/images/tutorial_03.png"),
                                  tr("Basic Operations"), this);
  connect(mTutorialBasicAct, SIGNAL(triggered()), this,
          SLOT(loadBasicTutorial()));
  ...
  mTutorialExtrusionAct = new QAction(QIcon(":/images/tutorial_04.png"),
                                      tr("Extrusion Operations"), this);
  connect(mTutorialExtrusionAct, SIGNAL(triggered()), this,
          SLOT(loadExtrusionTutorial()));
  ...
  mTutorialRemeshingAct = new QAction(QIcon(":/images/tutorial_05.png"),
                                      tr("Remeshing Operations"), this);
  connect(mTutorialRemeshingAct, SIGNAL(triggered()), this,
          SLOT(loadRemeshingTutorial()));
  ...
  mTutorialHighgenusAct = new QAction(QIcon(":/images/tutorial_06.png"),
                                      tr("High genus Operations"), this);
  connect(mTutorialHighgenusAct, SIGNAL(triggered()), this,
          SLOT(loadHighgenusTutorial()));
  ...
  mTutorialTexturingAct = new QAction(QIcon(":/images/tutorial_07.png"),
                                      tr("Texturing Operations"), this);
  connect(mTutorialTexturingAct, SIGNAL(triggered()), this,
          SLOT(loadTexturingTutorial()));
  ...
  mShowStartupDialogAtStartupCheckBox =
      new QCheckBox(tr("Show this dialog at startup"));
  mShowStartupDialogAtStartupCheckBox->setChecked(mShowStartupDialogAtStartup);
  connect(mShowStartupDialogAtStartupCheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(setShowStartupDialogAtStartup(int)));
  ...
  downloadQuicktimeLabel = new QLabel(
      tr("<h5>Quicktime 7.2 or greater recommended.<br /><a "
         "href=\"http://apple.com/quicktime/\">Download now.</a></h5>"));
...
void MainWindow::loadExtrusionTutorial() {
  QDesktopServices::openUrl(
      QUrl::fromLocalFile("video/tutorial_extrusion.mov"));
}

void MainWindow::loadRemeshingTutorial() {
  QDesktopServices::openUrl(
      QUrl::fromLocalFile("video/tutorial_remeshing.mov"));
}

void MainWindow::loadHighgenusTutorial() {
  QDesktopServices::openUrl(
      QUrl::fromLocalFile("video/tutorial_highgenus.mov"));
}

void MainWindow::loadTexturingTutorial() {
  QDesktopServices::openUrl(
      QUrl::fromLocalFile("video/tutorial_texturing.mov"));
}
...
// This function only sets the tool options to the current one.
void MainWindow::setToolOptions(QWidget *optionsWidget) {
  mToolOptionsDockWidget->setWindowTitle(tr("Tool Options - ") +
                                         optionsWidget->windowTitle());
  mToolOptionsStackedWidget->setCurrentWidget(optionsWidget);
  // show or hide the dockwidget options
  // if (optionsWidget->windowTitle() != "" &&
  // mToolOptionsDockWidget->isHidden()) mToolOptionsDockWidget->show();
  // else if (!mToolOptionsDockWidget->isHidden() &&
  // optionsWidget->windowTitle() == "") mToolOptionsDockWidget->hide();
}
...
void MainWindow::openFile(QString fileName) {
  ...
  active->recomputePatches(mPatchMode->parameters);
...
void MainWindow::about() {
  // QString s = QString("TopMod Topological Mesh Modeler v%1 <br /><a
  // href=\"http://www.topmod3d.org/\">TopMod Home Page</a><br /><a
  // href=\"http://code.google.com/p/topmod/downloads/list\">Check for
  // Updates.</a>").arg(VER);
  QString s =
      QString("TopMod Topological Mesh Modeler v2.444 <br /><a "
              "href=\"http://www.topmod3d.org/\">TopMod Home Page</a><br /><a "
              "href=\"http://code.google.com/p/topmod/downloads/list\">Check "
              "for Updates.</a>");
  QMessageBox::about(this, tr("About TopMod"), s);
}

void MainWindow::initializeHelp() {
  // mAssistantClient = new
  // QAssistantClient(QLibraryInfo::location(QLibraryInfo::BinariesPath), this);
  //   QStringList arguments;
  //   arguments << "-profile" << QString("userdoc") + QDir::separator() +
  //   QString("topmod.adp"); mAssistantClient->setArguments(arguments);
}
...
/**
 * \brief will eventually check for updates on the TopMod website and download
 * the necessary files
 */
void MainWindow::checkForUpdates() {
  QDesktopServices::openUrl(
      QUrl("http://code.google.com/p/topmod/downloads/list"));
}
...
void MainWindow::topModResearch() {
  QDesktopServices::openUrl(
      QUrl("http://www.topmod3d.org/wiki/index.php?title=Research"));
}
...
void MainWindow::openPreferences() { mPreferencesDialog->display(); }
...
void MainWindow::setUndoLimit(int limit) { undolimit = limit; }
...
void MainWindow::doDrag(int x, int y) { // brianb
  ...
  case EditVertex: {
    if (GLWidget::numSelectedLocators() > 0) {
      ...
      // Project object coordinates to window coordinates (to get accurate
      // window depth)
  ...
  case SelectionWindow: {
    ...
    active->setSelectionWindowStartX(
        drag_startx); // mapFromGlobal(QCursor::pos()).x());
    active->setSelectionWindowStartY(
        drag_starty); // mapFromGlobal(QCursor::pos()).y());
    // find center of rectangle
    //  int w = abs(mapFromGlobal(QCursor::pos()).x()-drag_startx);
    //  int h =
    //  abs(mapFromGlobal(QCursor::pos()).y()-(active->height()-drag_starty));
...
void MainWindow::doSelection(int x, int y) {
  ...
  case EyeDropper: // dave
    ...
    if (patch != active->getRenderer())
      ...
      // statusBar()->showMessage(QString("Trying to select face at position
      // (%1, %2), got %3").arg(x).arg(y).arg((int)sfptr));
    }
...
/**
 * \brief load custom right click menu options based on the current mode of
 * operation
 *
 */
...
void MainWindow::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (mode != NormalMode && event->buttons() == Qt::LeftButton)
    ...
    doDrag(event->x(),
           this->size().height() - mStatusBar->height() - event->y());
  // else if ( event->buttons() == Qt::RightButton &&
  // QApplication::keyboardModifiers() == Qt::ShiftModifier){
  // 	// event->ignore();
  ...
  // }
    ...
    case One:
      if (mSpinBoxOne != 0)
        mSpinBoxOne->setValue(mSpinBoxOne->value() + d);
      break;
    case Two:
      if (mSpinBoxTwo != 0)
        mSpinBoxTwo->setValue(mSpinBoxTwo->value() + d);
      break;
    case Three:
      if (mSpinBoxThree != 0)
        mSpinBoxThree->setValue(mSpinBoxThree->value() + d);
      break;
    case Four:
      if (mSpinBoxFour != 0)
        mSpinBoxFour->setValue(mSpinBoxFour->value() + d);
      break;
    case Five:
      if (mSpinBoxFive != 0)
        mSpinBoxFive->setValue(mSpinBoxFive->value() + d);
      break;
    case Six:
      if (mSpinBoxSix != 0)
        mSpinBoxSix->setValue(mSpinBoxSix->value() + d);
      break;
    default:
      break;
    };
...
void MainWindow::keyReleaseEvent(QKeyEvent *event) { mSpinBoxMode = None; }
...
void MainWindow::mouseReleaseEvent(QMouseEvent *event) {
  ...
  // If ALT key is pressed we are mainpulating the view which is handled by the
  // viewport
  if (mode != NormalMode) {
    switch (mode) {
    case EditVertex: // brianb
    ...
    case PaintFace:
      if (active->numSelectedFaces() >= 1) {
        ...
        // DLFLMaterialPtr m =
        // object.findMaterial(RGBColor(paint_bucket_color.redF(),paint_bucket_color.greenF(),paint_bucket_color.blueF()
        // ));
        fp->setMaterial(object.addMaterial(
            RGBColor(paint_bucket_color.redF(), paint_bucket_color.greenF(),
    ...
    case EyeDropper:
      if (active->numSelectedFaces() >= 1) {
        ...
        mExperimentalMode->setPaintBucketColor(QColor(fp->material()->color.r,
                                                      fp->material()->color.g,
                                                      fp->material()->color.b));
        active->clearSelectedFaces();
        redraw();
    ...
    case DeleteEdge:
      if (active->numSelectedEdges() >= 1) {
        ...
        if (septr) {
          ...
          /*
                                                                DLFL::deleteEdge(
          &object, septr, MainWindow::delete_edge_cleanup);
    ...
    case ExtrudeMultipleFaces:
      ...
      // 		{
      // 			DLFLFacePtrArray sfptrarr =
      // active->getSelectedFaces(); 			if ( sfptrarr[0] )
      // 				{
      // 					undoPush();
      // 					setModified(true);
      // 					vector<DLFLFacePtr>::iterator
      // it; 					for(it = sfptrarr.begin(); it != sfptrarr.end(); it++) {
        ...
        if (active->numSelectedFaces() >= 1) {
          ...
          if (sfptr) {
            ...
            DLFL::extrudeFaceDodeca(
                &object, sfptr, extrude_angle, num_extrusions, extrude_length1,
                extrude_length2, extrude_length3, hexagonalize_dodeca_extrude);
            // DLFL::extrudeFaceDodeca(&object,sfptr,extrude_dist,num_extrusions,
            // ds_ex_twist,extrude_scale, hexagonalize_dodeca_extrude);
            active->recomputePatches(mPatchMode->parameters);
            active->recomputeNormals();
          }
          active->clearSelectedFaces();
        ...
        if (active->numSelectedFaces() >= 1) {
          ...
          if (sfptr) {
            ...
            DLFL::extrudeFaceIcosa(&object, sfptr, extrude_angle_icosa,
                                   num_extrusions, extrude_length1_icosa,
                                   extrude_length2_icosa,
      ...
      if (active->numSelectedCorners() >= 2) {
        ...
        if (sfvptr1 && sfvptr2) {
          ...
          if (symmetric_weights)
            // the new function also accepts a normal twist parameter
            /*DLFL::hermiteConnectFaces(*/
            DLFL::hermiteConnectFaces2(
                &object, sfvptr1, sfvptr2, num_segments, nwt1, nwt1,
                max_segments, num_extra_twists, pinching_factor, pinch_center,
                bubble_factor, triangulate_new_faces, scherk_collins,
                scherk_collins_pinch, scherk_collins_pinch_center,
                scherk_collins_pinch_width, scherk_collins_hole_twist,
                scherk_collins_hole_init_skip, scherk_collins_hole_skip,
                scherk_collins_hole_seg_skip, scherk_collins_hole_num_segs,
                normal_twist_factor);
          else
            /*DLFL::hermiteConnectFaces(*/
            // the new function also accepts a normal twist parameter
            DLFL::hermiteConnectFaces2(
                &object, sfvptr1, sfvptr2, num_segments, nwt1, nwt2,
                max_segments, num_extra_twists, pinching_factor, pinch_center,
                bubble_factor, triangulate_new_faces, scherk_collins,
                scherk_collins_pinch, scherk_collins_pinch_center,
                scherk_collins_pinch_width, scherk_collins_hole_twist,
    ...
    case ReorderFace:
      if (active->numSelectedCorners() >= 1) {
        ...
        if (sfvptr)
          sfvptr->getFacePtr()->reorder(sfvptr);
        redraw();
      }
      break;
    case SubdivideFace:
    ...
    // 				// DLFL::cutEdge( &object, septr,
    // peelDistance_factor,pnormalBendS_factor,pnormalBendT_factor,
    // peel_all_edges);
    // 			}
    // 		active->clearSelectedEdges();
    // 		redraw();
    ...
    if (sfptrarr[0]) {
      ...
      for (it = sfptrarr.begin(); it != sfptrarr.end(); it++) {
        ...
        case CubicalExtrude:
          ...
          DLFL::extrudeFaceScherkCollins(
              &object, *it, extrude_dist, num_extrusions, extrude_rot,
              extrude_scale, triangulate_new_faces, scherk_collins,
              scherk_collins_pinch, scherk_collins_pinch_center,
              scherk_collins_pinch_width);
          break;
          // case IcosahedralExtrude:
          // DLFL::extrudeFaceIcosa(&object,*it,extrude_dist,num_extrusions,
          // ds_ex_twist,extrude_scale);
        case IcosahedralExtrude:
          // std::cout<< extrude_angle_icosa  << "\t" << num_extrusions  << "\t"
          // << extrude_length1_icosa  << "\t" << extrude_length2_icosa << "\t"
          // << extrude_length3_icosa <<"\n";
          DLFL::extrudeFaceIcosa(&object, *it, extrude_angle_icosa,
                                 num_extrusions, extrude_length1_icosa,
                                 extrude_length2_icosa, extrude_length3_icosa);
          // DLFL::extrudeFaceCubOcta(&object, *it,
          // extrude_angle_icosa,num_extrusions,
          // extrude_length1_icosa,extrude_length2_icosa,extrude_length3_icosa);
          break;
        // DLFLFacePtr extrudeFaceDodeca(DLFLObjectPtr obj, DLFLFacePtr fptr,
        // double angle, int num, double ex_dist1, double ex_dist2, double
        // ex_dist3, bool hexagonalize);
        case DodecahedralExtrude:
          DLFL::extrudeFaceDodeca(&object, *it, extrude_angle, num_extrusions,
                                  extrude_length1, extrude_length2,
                                  extrude_length3, hexagonalize_dodeca_extrude);
          // case DodecahedralExtrude:
          // DLFL::extrudeFaceDodeca(&object,*it,extrude_dist,num_extrusions,
          // ds_ex_twist,extrude_scale, hexagonalize_dodeca_extrude);
          // DLFL::extrudeFaceSmallRhombiCubOcta(&object,*it,extrude_angle,num_extrusions,
          // extrude_length1,extrude_length2,extrude_length3);
          break;
        ...
        case DomeExtrude:
          DLFL::extrudeFaceDome(&object, *it, domeExtrudeLength_factor,
                                domeExtrudeRotation_factor,
                                domeExtrudeScale_factor);
          break;
        default:
          DLFL::extrudeFace(&object, *it, extrude_dist, num_extrusions,
                            extrude_rot, extrude_scale);
...
GLWidget *MainWindow::getActive() { return active; }

// Override redraw() method to mark subwindows also for redraw
void MainWindow::redraw() { active->redraw(); }
...
void MainWindow::readObjectQFile(QString filename) {
  ...
  if (filename.indexOf(".dlfl") == filename.length() - 4 ||
      filename.indexOf(".dlfl") == filename.length() - 4)
    object.readDLFL(filestring, mtlfile);
  else if (filename.indexOf(".OBJ") == filename.length() - 4 ||
           filename.indexOf(".obj") == filename.length() - 4)
...
void MainWindow::openFile(void) {
  QString fileName = QFileDialog::getOpenFileName(
      ...
      tr("All Supported Files (*.obj *.dlfl);;Wavefront OBJ Files "
         "(*.obj);;DLFL Files (*.dlfl);;All Files (*)"),
  ...
  if (!fileName.isEmpty()) {
    ...
    QString mtlfile =
        mSaveDirectory + "/" + QFileInfo(fileName).baseName() + ".mtl";
  ...
  if (curFile != "untitled") {
    ...
    if (!curFile.isEmpty()) {
      ...
      QString fileName = QFileDialog::getSaveFileName(
          this, tr("Save File As..."), mSaveDirectory + "/" + curFileTemp,
          tr("All Supported Files (*.obj *.dlfl);;Wavefront OBJ Files "
             "(*.obj);;DLFL Files (*.dlfl);;All Files (*)"),
          0, QFileDialog::DontUseSheet);
      if (!fileName.isEmpty()) {
        ...
        if (!fileName.indexOf(".obj") || !fileName.indexOf(".dlfl") ||
            !fileName.indexOf(".OBJ") || !fileName.indexOf(".DLFL"))
          fileName.append(".obj");
        setCurrentFile(fileName);
...
bool MainWindow::saveFileAs(bool with_normals, bool with_tex_coords) {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save File As..."), mSaveDirectory + "/" + curFile,
      tr("All Supported Files (*.obj *.dlfl);;Wavefront OBJ Files "
         "(*.obj);;DLFL Files (*.dlfl);;All Files (*)"),
      0, QFileDialog::DontUseSheet);
  if (!fileName.isEmpty()) {
...
bool MainWindow::saveFileBezierOBJ() {
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save Bezier Patch (OBJ)..."), mSaveDirectory + "/" + curFile,
      tr("Wavefront OBJ Files (*.obj);;All Files (*)"), 0,
...
bool MainWindow::saveFileLG3d() {
  QString fileName = QFileDialog::getSaveFileName(
      ...
      tr("Mathematica Graphics3D Files (*.m);;All Files (*)"), 0,
      QFileDialog::DontUseSheet);
  if (!fileName.isEmpty()) {
    QByteArray ba = fileName.toLatin1();
    const char *filename = ba.data();
...
bool MainWindow::saveFileSTL() {
  ...
  // triangulate the mesh first - this isn't the best solution but will work for
  // now.
  //  globalStellate();
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Export STL..."), mSaveDirectory + "/" + curFile,
      tr("STL Files (*.stl);;All Files (*)"), 0, QFileDialog::DontUseSheet);
  if (!fileName.isEmpty()) {
    QByteArray ba = fileName.toLatin1();
    const char *filename = ba.data();
    writeSTL(filename);
...
bool MainWindow::viewportScreenshot() {
  ...
  // viewportPixmap =
  // QPixmap::grabWidget(active,0,0,active->width(),active->height());
  // viewportPixmap =
  // QPixmap::grabWindow(active->winId(),/*mapToGlobal(active->pos()).x()*/0,/*mapToGlobal(active->pos()).y()*/0,active->width(),active->height());
  QImage image = active->grabFrameBuffer(true);
  QString format = "png";
  QString initialPath = mSaveDirectory + tr("/untitled.") + format;

  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save Viewport Screenshot As"), initialPath,
      tr("%1 Files (*.%2);;All Files (*)").arg(format.toUpper()).arg(format), 0,
      QFileDialog::DontUseSheet);
  if (!fileName.isEmpty()) {
    image.save(fileName);
    return true;
...
bool MainWindow::appScreenshot() {
  ...
  //	QString fileName = QFileDialog::getSaveFileName(this, tr("Save App
  //Screenshot As"), 																									initialPath, 																									tr("%1 Files (*.%2);;All Files (*)")
  //																									.arg(format.toUpper())
  //																									.arg(format),
  //																									0,
  //QFileDialog::DontUseSheet); 	if (!fileName.isEmpty()){
  //        appPixmap.save(fileName);
  //		return true;
  //	}
  return false;
}
...
void MainWindow::loadCube() {
  ...
  // iterate through faces and color them red... just to test the renderer
  // out...
...
bool MainWindow::isModified() { return mIsModified; }
...
// recurse through selected faces to get a list of checkerboard selection for
// the entire object
void MainWindow::getCheckerboardSelection(DLFLFacePtr fptr) {
  if (fptr) {
    ...
    if (numShared == 0)
      return; // break out of recursive loop is there are no one vertex sharing
              // faces
  }
}
...
void MainWindow::setPatchMode() {
  ...
  mToolsToolBar->show();
  mHighgenusToolBar->show();
  usePatchRenderer(this->mPatchMode->parameters);
...
void MainWindow::clearToolOptions() {
  mToolOptionsDockWidget->setWindowTitle(tr("Tool Options - Empty"));
  mToolOptionsStackedWidget->setCurrentWidget(emptyToolMode);
}

void MainWindow::triggerBranchCovering_CornerCutting() {
  mToolOptionsDockWidget->setWindowTitle(
      tr("Branch Covering (Corner Cutting)"));
  mToolOptionsStackedWidget->setCurrentWidget(mBranchCovering_CornerCutting);
}
...
void MainWindow::setPachModeToolOptions() {
  mToolOptionsDockWidget->setWindowTitle(tr("Tool Options - ") +
                                         this->mPatchMode->windowTitle());
  mToolOptionsStackedWidget->setCurrentWidget(this->mPatchMode);
}

void MainWindow::converPatchesToDLFL() {
  undoPush();
  // BiCubicBezierPatchToDLFLData
  // dataExtracor(TMPatch::GetPatchGridDimension());
  BiCubicBezierPatchToDLFLData dataExtracor;
  TMPatchFacePtrList patches = active->patchobject()->list();
  TMPatchFacePtrList::iterator first = patches.begin(), last = patches.end();
  TMPatchFacePtr pfp = NULL;
  while (first != last) {
    pfp = (*first);
    ++first;
    pfp->extractPatchDataFromFace(&dataExtracor);
  }
  // dataExtracor.toFile("C:\\Users\\zarrinmehr\\Desktop\\data.obj");
  struct patch_data data = dataExtracor.get_patch_data();
  object.fromData(data.vertices, data.vertex_count, data.face_indices,
                  data.face_count);
  data.release_memory();
  dataExtracor.release_memory();
  setPolygonalMode();
  clearToolOptions();
  //    //this probably can be avoided. The new empty widget is only added to
  //    make the tool options empty QWidget *empty = new QWidget(this);
  //    mToolOptionsStackedWidget->addWidget(empty);
  //    mToolOptionsStackedWidget->setCurrentWidget(empty);
}

void MainWindow::SaveDLFLPatchesAsOBJ() {
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save Patch as OBJ File"), tr(""), tr("OBJ File (*.obj *.OBJ)"));
  ...
  dataExtracor.toFile(fileName);
}

struct PatchParameters MainWindow::getPatchParameters() {
  return this->mPatchMode->parameters;
}

void MainWindow::setLanguageSpanish() { changeLanguage("es"); }

void MainWindow::setLanguageFrench() { changeLanguage("fr"); }

void MainWindow::setLanguageGerman() { changeLanguage("de"); }

void MainWindow::setLanguageTurkish() { changeLanguage("tr"); }

void MainWindow::setLanguageItalian() { changeLanguage("it"); }

void MainWindow::setLanguageHindi() { changeLanguage("hi"); }

void MainWindow::setLanguageCatalan() { changeLanguage("ca"); }

void MainWindow::setLanguageEnglish() { changeLanguage("en"); }
 ...
 *
 * \todo all translatable text will eventually go in this function, but that
 * would take forever! we'll see if there's another way to handle that. it's
 * probably going to be a matter of putting all the QAction->setText() and
 * QAction->setTooltip() calls into one function. This is going to be a pain but
 * could be beneficial in the long run I'm testing it out right now. we'll see
 * how this goes...
 *
 */
...
#endif
  // Edit Menu Actions
  mDeleteSelectedAct->setText(
      /*QIcon(":images/edit-undo.png"),*/ tr("&Delete Selected"));
  mDeleteSelectedAct->setStatusTip(tr("Delete Selected"));
...
DLFLObjectPtr MainWindow::GetObject() { return &object; }
