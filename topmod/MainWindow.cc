...
void MainWindow::createActions() {
  ...
  // Fenghui: fix this, use constant for mode
    ...
    connect(patchModelingAct, SIGNAL(triggered()), this, SLOT(setPachModeToolOptions()));
    connect(mPatchMode,SIGNAL(ParametersUpdated()), this, SLOT(setPatchMode()));
    connect(mPatchMode, SIGNAL(ConvertToTopMode()), this, SLOT(converPatchesToDLFL()));
    connect(mPatchMode, SIGNAL(SaveAsOBJ()), this, SLOT(SaveDLFLPatchesAsOBJ()));
    connect(mPatchMode, SIGNAL(PatchRenderUpdateRequest()),getActive(),SLOT(update()));
    ...
    connect(mBranchCovering_CornerCutting,SIGNAL(Apply(std::vector<HALF_EDGE_MAPPING>,BranchCovering_CornerCutting_Parameters)),
            this,SLOT(applyBranchCovering3v(std::vector<HALF_EDGE_MAPPING>,BranchCovering_CornerCutting_Parameters)));
...
void MainWindow::createMenus(){
    ...
    mBranchCovering_CornerCutting_Action = new QAction(tr("Corner Cutting"),this);
    connect(mBranchCovering_CornerCutting_Action,SIGNAL(triggered()),this,SLOT(triggerBranchCovering_CornerCutting()));
							...
							if ( sfvptr1 && sfvptr2 )
								{
									undoPush();
									setModified(true);
                                    DLFL::bezierConnectFaces(&object,sfvptr1,sfvptr2, num_segments,nwt1,nwt2);
									active->clearSelectedFaces();
									active->clearSelectedCorners();
									num_sel_faceverts = 0; num_sel_faces = 0;
                                    active->recomputePatches(mPatchMode->parameters);
...
void MainWindow::triggerBranchCovering_CornerCutting(){
    mToolOptionsDockWidget->setWindowTitle(tr("Branch Covering (Corner Cutting)"));
...
void MainWindow::setPachModeToolOptions(){
    mToolOptionsDockWidget->setWindowTitle(tr("Tool Options - ") + this->mPatchMode->windowTitle());
    mToolOptionsStackedWidget->setCurrentWidget(this->mPatchMode);
}

void MainWindow::converPatchesToDLFL(){
    undoPush();
    //BiCubicBezierPatchToDLFLData dataExtracor(TMPatch::GetPatchGridDimension());
    ...
    object.fromData(data.vertices, data.vertex_count, data.face_indices, data.face_count);
    data.release_memory();
    dataExtracor.release_memory();
    setPolygonalMode();
    clearToolOptions();
//    //this probably can be avoided. The new empty widget is only added to make the tool options empty
...
void MainWindow::SaveDLFLPatchesAsOBJ()
{
    QString fileName =
            QFileDialog::getSaveFileName(
                this, tr("Save Patch as OBJ File"),
                tr(""),
                tr("OBJ File (*.obj *.OBJ)"));
