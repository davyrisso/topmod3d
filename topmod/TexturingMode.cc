...
TexturingMode::TexturingMode(QWidget *parent, QShortcutManager *sm,
  ...
  connect(mTileTexturingAction, SIGNAL(triggered()), this,
          SLOT(triggerTileTexturing()));
...
void TexturingMode::addActions(QActionGroup *actionGroup, QToolBar *toolBar,
...
QDoubleSpinBox *TexturingMode::createDoubleSpinBox(
...
void TexturingMode::setupTileTexturing() {
  ...
  tileTexturingNumTilesSpinBox =
      createDoubleSpinBox(mTileTexturingLayout, tileTexturingNumTilesLabel,
                          tr("Tiling Number"), 2, 8, 1, 2, 0, 0, 0);
  connect(tileTexturingNumTilesSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(changeTileTexNum(double)));

  tileTexturingAssignButton =
      new QPushButton(tr("Assign Texture\nCoordinates"), this);
  connect(tileTexturingAssignButton, SIGNAL(clicked()), ((MainWindow *)mParent),
