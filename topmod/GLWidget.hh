...
// DLFLVertex pointers extern DLFLEdgePtrArray DLFLObject::sel_eptr_array; //
// List of selected DLFLEdge pointers extern DLFLFacePtrArray
...
class GLWidget : public QGLWidget {
  Q_OBJECT // must include this if you use Qt signals/slots
      ...
      GLWidget(int w, int h, DLFLRendererPtr rp, QColor color, QColor vcolor,
               DLFLObjectPtr op, const QGLFormat &format, QWidget *parent = 0);
...
public slots:
  ...
  void setWireframeColor(QColor c) {
    if (renderer)
      renderer->setWireframeColor(c);
    redraw();
  }
  void setSilhouetteColor(QColor c) {
    if (renderer)
      renderer->setSilhouetteColor(c);
    redraw();
  }
  ...
  void setNormalColor(QColor c) {
    if (renderer)
      renderer->setNormalColor(c);
    redraw();
  }
  void setFaceCentroidColor(QColor c) {
    if (renderer)
      renderer->setFaceCentroidColor(c);
    redraw();
  }
  void setFaceCentroidThickness(double t) {
    if (renderer)
      renderer->setFaceCentroidThickness(t);
    redraw();
  }
  void setNormalThickness(double t) {
    if (renderer)
      renderer->setNormalThickness(t);
    redraw();
  }
  void setNormalLength(double l) {
    if (renderer)
      renderer->setNormalLength(l);
    redraw();
  }
  ...
  void zoomIn() {
    mCamera->HandleMouseWheel(100, width(), height());
    repaint();
  }
  void zoomOut() {
    mCamera->HandleMouseWheel(-100, width(), height());
  ...
  void recomputePatches(const struct PatchParameters &parameters);
  ...
  void setSelectionWindowStartX(double x) { mSelectionWindowStartX = x; }

  void setSelectionWindowStartY(double y) { mSelectionWindowStartY = y; }
  ...
  void toggleObjectOrientation(void) {
    if (renderer)
      renderer->toggleObjectOrientation();
    this->repaint();
  }

  void toggleNormals(void) {
    if (renderer)
      renderer->toggleFaceNormals();
    this->repaint();
  }

  void toggleFaceCentroids(void) {
    if (renderer)
      renderer->toggleFaceCentroids();
    this->repaint();
  }
...
protected:
  ...
  void drawIDs(QPainter *painter, const GLdouble *model, const GLdouble *proj,
               const GLint *view);
  void drawSelectedIDs(QPainter *painter, const GLdouble *model,
  ...
  static DLFLLocatorPtrArray
      sel_lptr_array; // List of selected DLFLLocator pointers  // brianb
  // static DLFLVertexPtrArray sel_vptr_array; // List of selected DLFLVertex
  // pointers static DLFLEdgePtrArray sel_eptr_array; // List of selected
  // DLFLEdge pointers static DLFLFacePtrArray sel_fptr_array; // List of
  // selected DLFLFace pointers static DLFLFaceVertexPtrArray sel_fvptr_array;
  // // List of selected DLFLFaceVertex pointers

  // Viewport viewport;   // Viewport for the window
  PerspCamera *mCamera; // new camera originally from Don House and Chris Root,
                        // modified by Stuart and then Dave
...
public:
  ...
  void setBrushSize(double s) {
    mBrushSize = max(2.0, min(200.0, s));
    repaint();
  }
  ...
  void setModeString(QString s) { mModeString = s; }

  void setExtrusionModeString(QString s) { mExtrusionModeString = s; }

  void setRemeshingSchemeString(QString s) { mRemeshingSchemeString = s; }

  void setSelectionMaskString(QString s) { mSelectionMaskString = s; }
  ...
  //--- If size of array is smaller than index item will be added to end of
  //array ---//
  void setSelectedLocator(int index, DLFLLocatorPtr vp) // brianb
  {
    if (vp && index >= 0) {
      if (index < sel_lptr_array.size())
        sel_lptr_array[index] = vp;
      else
        sel_lptr_array.push_back(vp);
    }
  }

  void setSelectedVertex(int index, DLFLVertexPtr vp) {
    if (vp && index >= 0) {
      if ((uint)index < object->sel_vptr_array.size())
        object->sel_vptr_array[index] = vp;
      else
        object->sel_vptr_array.push_back(vp);
    }
  }
  ...
  void setSelectedEdge(int index, DLFLEdgePtr ep) {
    if (ep && index >= 0) {
      if ((uint)index < object->sel_eptr_array.size())
        object->sel_eptr_array[index] = ep;
      else
        object->sel_eptr_array.push_back(ep);
    }
  }
  ...
  void setSelectedFace(int index, DLFLFacePtr fp) {
    if (fp && index >= 0) {
      if ((uint)index < object->sel_fptr_array.size())
        object->sel_fptr_array[index] = fp;
      else
        object->sel_fptr_array.push_back(fp);
    }
  }
  ...
  void setSelectedFaceVertex(int index, DLFLFaceVertexPtr fvp) {
    if (fvp && index >= 0) {
      if ((uint)index < object->sel_fvptr_array.size())
        object->sel_fvptr_array[index] = fvp;
      else
        object->sel_fvptr_array.push_back(fvp);
    }
  }
  ...
  {
    if (index < sel_lptr_array.size())
      return sel_lptr_array[index];
    return NULL;
  }

  DLFLVertexPtr getSelectedVertex(int index) {
    if ((uint)index < object->sel_vptr_array.size())
      return object->sel_vptr_array[index];
    return NULL;
  }

  DLFLEdgePtr getSelectedEdge(int index) {
    if ((uint)index < object->sel_eptr_array.size())
      return object->sel_eptr_array[index];
    return NULL;
  }

  DLFLFacePtr getSelectedFace(int index) {
    if ((uint)index < object->sel_fptr_array.size())
      return object->sel_fptr_array[index];
    return NULL;
  }
  ...
  DLFLFaceVertexPtr getSelectedFaceVertex(int index) {
    if ((uint)index < object->sel_fvptr_array.size())
      return object->sel_fvptr_array[index];
    return NULL;
  }
  ...
  int numSelectedVertices(void) { return object->sel_vptr_array.size(); }

  int numSelectedEdges(void) { return object->sel_eptr_array.size(); }

  int numSelectedFaces(void) { return object->sel_fptr_array.size(); }

  int numSelectedCorners(void) { return object->sel_fvptr_array.size(); }

  //--- Clear individual selection lists ---//
  static void clearSelectedLocators(void) { sel_lptr_array.clear(); }

  void clearSelectedVertices(void) { object->sel_vptr_array.clear(); }

  void clearSelectedEdges(void) { object->sel_eptr_array.clear(); }

  void clearSelectedFaces(void) { object->sel_fptr_array.clear(); }

  void clearSelectedCorners(void) { object->sel_fvptr_array.clear(); }

  void clearSelectedFace(DLFLFacePtr fp) {
    vector<DLFLFacePtr>::iterator fiterator;
    for (fiterator = object->sel_fptr_array.begin();
         fiterator != object->sel_fptr_array.end(); fiterator++) {
      if (fp == *fiterator) {
        // delete fp;
  ...
  void clearSelectedEdge(DLFLEdgePtr ep) {
    vector<DLFLEdgePtr>::iterator it;
    for (it = object->sel_eptr_array.begin();
         it != object->sel_eptr_array.end(); it++) {
      if (ep == *it) {
  ...
  void clearSelectedVertex(DLFLVertexPtr vp) {
    vector<DLFLVertexPtr>::iterator it;
    for (it = object->sel_vptr_array.begin();
         it != object->sel_vptr_array.end(); it++) {
      if (vp == *it) {
  ...
  void clearSelectedFaceVertex(DLFLFaceVertexPtr fvp) {
    vector<DLFLFaceVertexPtr>::iterator it;
    for (it = object->sel_fvptr_array.begin();
         it != object->sel_fvptr_array.end(); it++) {
      if (fvp == *it) {
  ...
  void clearSelectedFaceVertices(void) { object->sel_fvptr_array.clear(); }
  ...
  void setPatchMode(bool patch_mode) { mInPatchMode = patch_mode; }
  bool isInPatchMode() { return mInPatchMode; }

  DLFLRendererPtr getRenderer() { return renderer; }

  // Set the render flags
  void setRenderFlags(int rflags) {
    if (renderer)
      renderer->setRenderFlags(rflags);
    this->repaint();
  }
  ...
  DLFLLocatorPtr selectLocator(int mx, int my, int w = 10,
                               int h = 10); // brianb
  ...
  DLFLFaceVertexPtr selectFaceVertex(DLFLFacePtr fp, int mx, int my, int w = 40,
                                     int h = 40);
  DLFLFaceVertexPtrArray selectFaceVertices(DLFLFacePtr fp, int mx, int my,
                                            int w = 40, int h = 40);

  // Event handler for the viewport. handles only mouse events when glwidget has
  // focus which relate to navigational controls (ALT + push/drag/release)
