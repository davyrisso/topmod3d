...
@{
        ...
        \class GeometryRenderer
                \brief functions for rendering edges, faces, vertices of DLFL
Objects
...
void GeometryRenderer::renderFace(DLFLFacePtr df, bool useAttrs) const {
  ...
  if (useGPU) {
    ...
    cgSetParameter3f(CgData::instance()->basecolor, basecolor.r, basecolor.g,
                     basecolor.b);
...
void GeometryRenderer::renderFaceVertex(DLFLFaceVertexPtr dfv,
                                        bool useAttrs) const {
  if (dfv->vertex) {
    if (useAttrs) {
      if (useNormal) {
...
// RGBColor rgb( 0.1, 0.1, 0.1 );
        ...
        if (!useGPU) {
          rgb = product(
              rgb,
              dfv->color); // this multiplies corresponding elements together
                           // (e.g. RGBColor(r1*r2, g1*g2, b1*b2)  )
        }
#else
        rgb = product(
            rgb, dfv->color); // this multiplies corresponding elements together
                              // (e.g. RGBColor(r1*r2, g1*g2, b1*b2)  )
#endif
        ...
        if (useLighting) {
          glColor(dfv->color);
        } // Actually means use lighting
        if (useMaterial) {
          glColor4dv(renderColor);
        }
        if (useColorable) {
...
// }
//
void GeometryRenderer::renderFaceCentroids(DLFLObjectPtr obj,
                                           double size) const {
...
void GeometryRenderer::renderFaceNormals(DLFLObjectPtr obj, double width,
                                         double length) const {
  DLFLFacePtrList::iterator it;
  glPushMatrix();
  {
