 ...
 *
 */
ExperimentalMode::ExperimentalMode(QWidget *parent, QShortcutManager *sm,
                                   QWidget *actionList)
    : QWidget(parent) {
  setParent(0);
  mParent = parent;
  ...
  mPaintBucketAct =
      new QAction(QIcon(":images/color-fill.png"), tr("Paint Bucket"), this);
  ...
  connect(mPaintBucketAct, SIGNAL(triggered()), this,
          SLOT(triggerPaintBucket()));
...
void ExperimentalMode::addActions(QActionGroup *actionGroup, QToolBar *toolBar,
...
QPushButton *ExperimentalMode::addColorPreference(QLabel *label, QString text,
                                                  QColor color,
                                                  QGridLayout *layout,
...
void ExperimentalMode::setupPaintBucket() {
  ...
  mPaintBucketColorButton = addColorPreference(
      mPaintBucketColorLabel, tr("Paint Bucket\nColor:"), mPaintBucketColor,
      mPaintBucketLayout, colorPickerStyle, 0, 0);
  connect(mPaintBucketColorButton, SIGNAL(clicked()), this,
          SLOT(setPaintBucketColor()));

  // paint selcted faces
  mPaintSelectedFacesButton = new QPushButton(tr("Paint Selected Faces"), this);
  connect(mPaintSelectedFacesButton, SIGNAL(clicked()), ((MainWindow *)mParent),
  ...
  connect(mResetMaterialsButton, SIGNAL(clicked()), ((MainWindow *)mParent),
          SLOT(clearMaterials()));
