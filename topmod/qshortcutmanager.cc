...
**
** Notes :	Parts of the project are derivative work of Trolltech's QSA
*library
...
/*!
        ...
        This class manages shortcuts for the complete application, plugins
   included. The only thing needed is to register actions and to make sure not
   to override shortcuts of managed actions.
...
/*!
        \brief Constructor
*/
QShortcutManager::QShortcutManager() : sLang("en_us") {
...
QShortcutDialog *QShortcutManager::getShortcutDialog() { return pDialog; }

/*!
        \brief Shows the shortcut configuration dialog
*/
void QShortcutManager::configure() { pDialog->exec(); }
...
#ifdef _DEBUG_
  qDebug("context = %s; name = %s; shortcut = %s;", qPrintable(context),
         qPrintable(name), qPrintable(s));
#endif
...
*/
void QShortcutManager::registerAction(QAction *a, const QString &cxt,
                                      const QString &def) {
  if (!a || contains(a))
    return;

#ifdef _DEBUG_
  qDebug() << "adding action : " << a->text() << " from " << cxt << " with "
           << def;
#endif
