...
struct PatchParameters {
  ...
  PatchParameters()
      : rotation_method(ALL_UNIFORMLY_R),
        ...
        angle(0.0), sin_angle(0.0), cos_angle(1.0), scale(4.0 / 3.0),

        grid_size(6), patch_nesting_factor(0),
        polygonRegularizationIteration(0),

        local_patch_nesting(false), regularizePolygons(false) {}
};
...
private:
  ...
  QSlider *angleSlider, *scaleSlider, *sudivisionSlider,
      *polygonRegularizationIterationSlider;
  QRadioButton *allOpt, *alternateOpt, *edgesOpt, *vertexOpt;
  QLabel *angleLable, *scaleLable, *rotationLable, *angleTitle, *scaleTitle,
      *subDivisionLabel, *patchNestingFactorLabel,
      *patchNestingFactorNumberLabel,
      *polygonRegularizationIterationStringValue;
  QSpinBox *subDivisionBox;
  QCheckBox *localPatchNestingChecked, *renderPatchBoundaries,
      *showPatchControlMesh, *renderDooSabinPolygons,
      *renderModifiedDooSabinPolygons, *regularizePolygons;
  void updated();
  bool resetMode;
