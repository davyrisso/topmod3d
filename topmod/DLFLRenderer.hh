...
// The class also provides static functions for rendering the object as a
// wireframe, silhouette and vertices, which can be used by derived classes
...
public:
  ...
  // ShowFaceNormals=8, ShowFaceCentroids=16 };
...
// mWireframeThickness(0.1);mSilhouetteThickness(8.0);mVertexThickness(5.0);mFaceCentroidThickness(5.0);mNormalThickness(1.0);mNormalLength(0.5)
  ...
  DLFLRenderer(QColor wc, double wt, QColor sc, double st, QColor vc, double vt,
               QColor fc, double ft, QColor nc, double nt, bool gpu = false,
               bool aa = false) {
    // mWireframeThickness(wt), mSilhouetteThickness(st), mVertexThickness(vt),
    // mWireframeColor(wc), mSilhouetteColor(sc),
    // mVertexColor(vc),mFaceCentroidColor(fc), mFaceCentroidThickness(ft),
...
public:
  /* Assignment operator */
  DLFLRenderer &operator=(const DLFLRenderer &) { return (*this); }
  ...
  void toggleObjectOrientation(void) {
    ...
    if (gr)
      gr->isReversed = DLFLRenderer::reverse_object;
  ...
  static bool isReversed(void) { return DLFLRenderer::reverse_object; };

  /* Get the render flags */
  int getRenderFlags(void) const { return render_flags; };
...
public:
  ...
  */
  ...
  void drawWireframe(DLFLObjectPtr object) {
    ...
    glColor4f(DLFLRenderer::mWireframeColor.redF(),
              DLFLRenderer::mWireframeColor.greenF(),
              DLFLRenderer::mWireframeColor.blueF(),
              DLFLRenderer::mWireframeColor.alphaF());
  ...
  void drawSilhouette(DLFLObjectPtr object) {
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    glColor4f(DLFLRenderer::mSilhouetteColor.redF(),
              DLFLRenderer::mSilhouetteColor.greenF(),
              DLFLRenderer::mSilhouetteColor.blueF(),
              DLFLRenderer::mSilhouetteColor.alphaF());
    glDepthRange(0.1, 1.0);
    if (DLFLRenderer::
            reverse_object) // object->renderEdges(mWireframeThickness);
      gr->renderEdges(object, DLFLRenderer::mWireframeThickness);
  ...
  void drawVertices(DLFLObjectPtr object) {
    glColor4f(DLFLRenderer::mVertexColor.redF(),
              DLFLRenderer::mVertexColor.greenF(),
              DLFLRenderer::mVertexColor.blueF(),
              DLFLRenderer::mVertexColor.alphaF());
  ...
  void drawFaceCentroids(DLFLObjectPtr object) {
    ...
    if (DLFLRenderer::useGPU) {
      ...
      // cgSetParameter3f(CgData::instance()->basecolor, mVertexColor.redF(),
      // mVertexColor.greenF(), mVertexColor.blueF());
...
#endif
    glColor4f(DLFLRenderer::mFaceCentroidColor.redF(),
              DLFLRenderer::mFaceCentroidColor.greenF(),
              DLFLRenderer::mFaceCentroidColor.blueF(),
              DLFLRenderer::mFaceCentroidColor.alphaF());
    glDepthRange(0.0, 1.0 - 0.00075);
    // object->renderVertices(mVertexThickness);
  ...
  void drawFaceNormals(DLFLObjectPtr object) {
    glColor4f(DLFLRenderer::mNormalColor.redF(),
              DLFLRenderer::mNormalColor.greenF(),
              DLFLRenderer::mNormalColor.blueF(),
              DLFLRenderer::mNormalColor.alphaF());
    glDepthRange(0.0, 1.0 - 0.00075);
    // object->renderVertices(mVertexThickness);
    gr->renderFaceNormals(object, DLFLRenderer::mNormalThickness,
                          DLFLRenderer::mNormalLength);
  }
  ...
  */
  void drawOverlays(DLFLObjectPtr object) {
    ...
    if (gr->drawSilhouette)
      drawSilhouette(object);
    if (gr->drawVertices)
      drawVertices(object);
    if (gr->drawFaceNormals)
      drawFaceNormals(object);
    if (gr->drawFaceCentroids)
      drawFaceCentroids(object);
  };
  ...
  void setFaceCentroidThickness(double t) {
    DLFLRenderer::mFaceCentroidThickness = t;
  }
