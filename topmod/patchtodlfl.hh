...
public:
  ...
  uint id;
  double &operator[](uint index) { return elems[index]; }

  double operator[](uint index) const { return elems[index]; }
  Vector3d toVector3d() { return Vector3d(elems[0], elems[1], elems[2]); }
  void print() {
...
public:
  uint &operator[](uint index) { return indices[index]; }

  uint operator[](uint index) const { return indices[index]; }
};
...
public:
  ...
  // element access only
  double &operator[](uint index) { return values[index]; }
  void print() const {
    qDebug() << STRING(values[0]) << "\t" << STRING(values[1]) << "\t"
