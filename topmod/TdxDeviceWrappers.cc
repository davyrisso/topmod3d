...
static void tdx_drv_handler(io_connect_t connection, natural_t messageType,
...
/* -------------------------------------------------------------------------- */
long TdxInitDevice(UInt32 appID, Boolean showOnlyMyClientEvents, UInt16 mode,
                   UInt32 mask) {
  ...
  // err = TdxInitDevice('3Dm1', TRUE, kConnexionClientModeTakeOver,
  // (kConnexionMaskAxis | kConnexionMaskButtons)); gDevWrapperInfo.clientID =
  // RegisterConnexionClient(appID, (UInt8*)"\pTopMod", mode, mask);
  gDevWrapperInfo.clientID =
      RegisterConnexionClient('TopM', (UInt8 *)"\pTopMod",
