 ...
 *
 */
QDoubleSpinBox *createDoubleSpinBox(QGridLayout *layout, QLabel *label,
                                    QString s, double low, double high,
                                    double step, double value, double decimals,
                                    int row, int col);
...
CreateTorusTool::CreateTorusTool(QWidget *parent) {
  ...
  num_segment_spin_ =
      createDoubleSpinBox(layout_, createTorusNumSegmentsLabel,
                          tr("# Segments"), 3, 100, 1, 8, 0, 0, 0);
  connect(num_segment_spin_, SIGNAL(valueChanged(double)),
          (MainWindow *)parent_, SLOT(changeNumSegments(double)));

  QLabel *createTorusRadiusLabel = new QLabel(this);
  radius_spin_ = createDoubleSpinBox(layout_, createTorusRadiusLabel,
  ...
  connect(size_cross_section_spin_, SIGNAL(valueChanged(double)), this,
          SLOT(ChangeSizeCrossSection(double)));

  QLabel *createTorusCrossSectionRadiusLabel = new QLabel(this);
  cross_section_radius_spin_ =
      createDoubleSpinBox(layout_, createTorusCrossSectionRadiusLabel,
                          tr("Radius of CS"), 0.1, 5, 0.1, 2.0, 3, 3, 0);

  QLabel *createTorusCrossSectionRotationLabel = new QLabel(this);
  cross_section_rotation_spin_ =
      createDoubleSpinBox(layout_, createTorusCrossSectionRotationLabel,
                          tr("Rotation of CS"), 0, 360, 10, 0, 0, 4, 0);

  QLabel *createTorusNumTwistLabel = new QLabel(this);
  num_twist_spin_ = createDoubleSpinBox(layout_, createTorusNumTwistLabel,
                                        tr("# Twists"), 0, 10, 1, 0, 0, 5, 0);

  QLabel *createTorusNumPartialTwistLabel = new QLabel(this);
  num_partial_twist_spin_ =
      createDoubleSpinBox(layout_, createTorusNumPartialTwistLabel,
...
void CreateTorusTool::Apply() {
  ...
  for (int i = 0; i < segmentCount; ++i) {
    ...
    for (int j = 0; j < CS_VertCount; ++j) {
      ...
      float cT = cos(degree);
      float sT = sin(degree);
      CS_Trans[j].set(CS_Trans[j][0], CS_Trans[j][1] * cT + CS_Trans[j][2] * sT,
                      -CS_Trans[j][1] * sT + CS_Trans[j][2] * cT);
      // translate cross section center by torus radius
      CS_Trans[j].set(CS_Trans[j][0], CS_Trans[j][1],
                      CS_Trans[j][2] + torusRadius);
  ...
  // connect each of the cross section faces with connectCorners() to create the
  // torus Some thing might go wrong if we have odd number of faces.
  int twist = 0;
  ...
  for (int i = 0; i < basic_faces.size() / 2; ++i) {
    ...
    if ((2 * i + 2) % basic_faces.size() == 0)
      for (int j = 0; j < CS_VertCount - numPartialTwists; ++j)
        fvp1 = fvp1->next();
    if (triangulate_new_faces)
      ...
      if (scherk_collins &&
          num_segments_processed % (scherk_collins_hole_seg_skip + 1) == 0) {
        ScherkCollinsHandle(
            object, new_edges, scherk_collins_pinch,
...
HighgenusMode::HighgenusMode(QWidget *parent, QShortcutManager *sm,
  ...
  mAddHoleHandleAction =
      new QAction(QIcon(":images/highgenus_addholehandle.png"),
                  tr("Add Hole/Handle"), this);
  ...
  connect(mAddHoleHandleAction, SIGNAL(triggered()), this,
          SLOT(triggerAddHoleHandle()));
  actionList->addAction(mAddHoleHandleAction);

  mAddHoleHandleCVAction =
      new QAction(QIcon(":images/highgenus_addholehandlecv.png"),
                  tr("Add Hole/Handle CV"), this);
  ...
  connect(mAddHoleHandleCVAction, SIGNAL(triggered()), this,
          SLOT(triggerAddHoleHandleCV()));
  actionList->addAction(mAddHoleHandleCVAction);

  mAddHandleSIAction = new QAction(QIcon(":images/highgenus_addhandlesi.png"),
                                   tr("Add Handle SI"), this);
  mAddHandleSIAction->setCheckable(true);
  sm->registerAction(mAddHandleSIAction, "High Genus Operations", "");
  mAddHandleSIAction->setStatusTip(
      tr("Enter Add Handle (Shape Interpolation) Mode"));
  mAddHandleSIAction->setToolTip(tr("Add Handle (Shape Interpolation) Mode"));
  connect(mAddHandleSIAction, SIGNAL(triggered()), this,
          SLOT(triggerAddHandleSI()));
  actionList->addAction(mAddHandleSIAction);

  mRindModelingScalingAction =
      new QAction(QIcon(":images/highgenus_rindscaling.png"),
                  tr("Rind Modeling Scaling"), this);
  mRindModelingScalingAction->setCheckable(true);
  sm->registerAction(mRindModelingScalingAction, "High Genus Operations", "");
  mRindModelingScalingAction->setStatusTip(
      tr("Enter Rind Modeling Scaling Mode"));
  mRindModelingScalingAction->setToolTip(tr("Rind Modeling Scaling Mode"));
  connect(mRindModelingScalingAction, SIGNAL(triggered()), this,
          SLOT(triggerRindModelingScaling()));
  actionList->addAction(mRindModelingScalingAction);

  mRindModelingThicknessAction =
      new QAction(QIcon(":images/highgenus_rindthickness.png"),
                  tr("Rind Modeling Thickness"), this);
  mRindModelingThicknessAction->setCheckable(true);
  sm->registerAction(mRindModelingThicknessAction, "High Genus Operations", "");
  mRindModelingThicknessAction->setStatusTip(
      tr("Enter Rind Modeling Thickness Mode"));
  mRindModelingThicknessAction->setToolTip(tr("Rind Modeling Thickness Mode"));
  connect(mRindModelingThicknessAction, SIGNAL(triggered()), this,
          SLOT(triggerRindModelingThickness()));
  actionList->addAction(mRindModelingThicknessAction);

  mWireframeModelingAction = new QAction(
      QIcon(":images/highgenus_wireframe.png"), tr("Wireframe Modeling"), this);
  ...
  connect(mWireframeModelingAction, SIGNAL(triggered()), this,
          SLOT(triggerWireframeModeling()));
  actionList->addAction(mWireframeModelingAction);

  mWireframeModeling2Action =
      new QAction(QIcon(":images/highgenus_wireframe.png"),
                  tr("Wireframe Modeling 2"), this);
  mWireframeModeling2Action->setCheckable(true);
  sm->registerAction(mWireframeModeling2Action, "High Genus Operations", "");
  mWireframeModeling2Action->setStatusTip(
      tr("Enter Wireframe Modeling 2 Mode"));
  mWireframeModeling2Action->setToolTip(tr("Wireframe Modeling 2 Mode"));
  connect(mWireframeModeling2Action, SIGNAL(triggered()), this,
          SLOT(triggerWireframeModeling2()));
  actionList->addAction(mWireframeModeling2Action);

  mColumnModelingAction = new QAction(QIcon(":images/highgenus_column.png"),
                                      tr("Column Modeling"), this);
  ...
  connect(mColumnModelingAction, SIGNAL(triggered()), this,
          SLOT(triggerColumnModeling()));
  actionList->addAction(mColumnModelingAction);

  mSierpinskyAction = new QAction(QIcon(":images/highgenus_sierpinsky.png"),
                                  tr("Sierpinsky"), this);
  ...
  connect(mSierpinskyAction, SIGNAL(triggered()), this,
          SLOT(triggerSierpinsky()));
  actionList->addAction(mSierpinskyAction);

  mMultiFaceHandleAction =
      new QAction(QIcon(":images/highgenus_multifacehandle.png"),
                  tr("Multi-face Handle"), this);
  ...
  connect(mMultiFaceHandleAction, SIGNAL(triggered()), this,
          SLOT(triggerMultiFaceHandle()));
  actionList->addAction(mMultiFaceHandleAction);

  mMengerSpongeAction = new QAction(QIcon(":images/highgenus_mengersponge.png"),
                                    tr("Menger Sponge"), this);
  ...
  connect(mMengerSpongeAction, SIGNAL(triggered()), this,
          SLOT(triggerMengerSponge()));
...
void HighgenusMode::addActions(QActionGroup *actionGroup, QToolBar *toolBar,
...
void HighgenusMode::setupAddHoleHandle() {
  ...
  addHoleHandleNumSegmentsSpinBox =
      createDoubleSpinBox(mAddHoleHandleLayout, addHoleHandleNumSegmentsLabel,
                          tr("# Segments"), 1, 100, 1, 10, 0, 0, 0);
  connect(addHoleHandleNumSegmentsSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(numSegmentsValueChanged(double)));

  // addHoleHandleNumSegmentsConnectLabel = new QLabel(this);
  // addHoleHandleNumSegmentsConnectSpinBox =
  // createDoubleSpinBox(mAddHoleHandleLayout,
  // addHoleHandleNumSegmentsConnectLabel, tr("# Segments to\nConnect
  // (-1=all)"), -1, 10, 1, -1, 0, 1,0);
  // connect(addHoleHandleNumSegmentsConnectSpinBox,
  // SIGNAL(valueChanged(double)), this,
  // SLOT(numSegmentsConnectValueChanged(double)));

  addHoleHandlePinching = new QLabel(this);
  addHoleHandlePinchingConnectSpinBox =
      createDoubleSpinBox(mAddHoleHandleLayout, addHoleHandlePinching,
                          tr("Pinching"), -1, 10, 0.1, 1.0, 3, 2, 0);
  connect(addHoleHandlePinchingConnectSpinBox, SIGNAL(valueChanged(double)),
          this, SLOT(changeHoleHandlePinch(double)));

  addHoleHandlePinchingCenter = new QLabel(this);
  addHoleHandlePinchingCenterConnectSpinBox =
      createDoubleSpinBox(mAddHoleHandleLayout, addHoleHandlePinchingCenter,
                          tr("Pinch Center"), 0.1, 1, 0.1, 0.5, 3, 3, 0);
  connect(addHoleHandlePinchingCenterConnectSpinBox,
          SIGNAL(valueChanged(double)), this,
          SLOT(changeHoleHandlePinchCenter(double)));

  addHoleHandlePinchingWidth = new QLabel(this);
  addHoleHandlePinchingWidthConnectSpinBox =
      createDoubleSpinBox(mAddHoleHandleLayout, addHoleHandlePinchingWidth,
                          tr("Pinch Width"), 0.001, 10, 0.1, 1.0, 3, 4, 0);
  connect(addHoleHandlePinchingWidthConnectSpinBox,
          SIGNAL(valueChanged(double)), this,
          SLOT(changeHoleHandlePinchWidth(double)));
...
void HighgenusMode::setupAddHoleHandleCV() {
  ...
  addHoleHandleCVNumSegmentsSpinBox = createDoubleSpinBox(
      ...
      1, 100, 1, 10, 0, 0, 0);
  connect(addHoleHandleCVNumSegmentsSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(numSegmentsValueChanged(double)));

  // addHoleHandleCVNumSegmentsConnectLabel = new QLabel(this);
  // addHoleHandleCVNumSegmentsConnectSpinBox =
  // createDoubleSpinBox(mAddHoleHandleCVLayout,
  // addHoleHandleCVNumSegmentsConnectLabel, tr("# Segments to\nConnect
  // (-1=all)"), 0, 10, 1, -1, 0, 1,0);
  // connect(addHoleHandleCVNumSegmentsConnectSpinBox,
  // SIGNAL(valueChanged(double)), this,
  // SLOT(numSegmentsConnectValueChanged(double)));

  mAddHoleHandleCVLayout->setRowStretch(4, 1);
  mAddHoleHandleCVLayout->setColumnStretch(2, 1);
  mAddHoleHandleCVWidget->setWindowTitle(
      tr("Add Hole/Handle (Closest Vertex)"));
...
void HighgenusMode::setupAddHandleSI() {
  ...
  addHandleSINumSegmentsSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSINumSegmentsLabel,
                          tr("# Segments"), 1, 100, 1, 10, 0, 0, 0);
  connect(addHandleSINumSegmentsSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(numSegmentsValueChanged(double)));

  // number of segments to connect
  /*
        addHandleSINumSegmentsConnectLabel = new QLabel(this);
        addHandleSINumSegmentsConnectSpinBox =
  ...
  */
  ...
  addHandleSIWeight1SpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIWeight1Label,
                          tr("Weight 1"), -100, 100, 0.1, 25.0, 3, 2, 0);
  ...
  addHandleSISymmetricWeightsCheckBox =
      new QCheckBox(tr("Symmetric Weights"), this);
  addHandleSISymmetricWeightsCheckBox->setChecked(Qt::Checked);
  connect(addHandleSISymmetricWeightsCheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(toggleSymmetricWeightsFlag(int)));
  mAddHandleSILayout->addWidget(addHandleSISymmetricWeightsCheckBox, 3, 0, 1,
                                2);
  // weight 2 - must be disabled when the checkbox is unchecked...
  addHandleSIWeight2Label = new QLabel(this);
  addHandleSIWeight2SpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIWeight2Label,
                          tr("Weight 2"), -100, 100, 0.1, 25.0, 3, 4, 0);
  ...
  addHandleSITwistsConnectSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSITwistsConnectLabel,
                          tr("Extra Twists"), 0, 100, 1, 0, 0, 5, 0);
  connect(addHandleSITwistsConnectSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(changeExtraTwists(double)));

  // pinching controls
  addHandleSIPinchConnectLabel = new QLabel(this);
  addHandleSIPinchConnectSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIPinchConnectLabel,
                          tr("Pinching"), -1, 10, 0.1, 1.0, 3, 6, 0);
  connect(addHandleSIPinchConnectSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(changePinch(double)));

  // pinching center
  addHandleSIPinchCenterConnectLabel = new QLabel(this);
  addHandleSIPinchCenterConnectSpinBox = createDoubleSpinBox(
      mAddHandleSILayout, addHandleSIPinchCenterConnectLabel,
      tr("Pinch Center"), 0.001, 0.999, 0.05, 0.5, 3, 7, 0);
  connect(addHandleSIPinchCenterConnectSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(changePinchCenter(double)));

  // bubble controls
  addHandleSIBubbleConnectLabel = new QLabel(this);
  addHandleSIBubbleConnectSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIBubbleConnectLabel,
                          tr("Pinch Width"), 0.001, 10.0, 0.1, 1.0, 3, 8, 0);
  connect(addHandleSIBubbleConnectSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(changeBubble(double)));

  addHandleTriangulateNewFacesCheckBox =
      new QCheckBox(tr("Triangulate New Faces"), this);
  connect(addHandleTriangulateNewFacesCheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(toggleTriangulateFace(int)));
  mAddHandleSILayout->addWidget(addHandleTriangulateNewFacesCheckBox, 9, 0, 1,
                                2);
  ...
  addHandleSIPinchSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIPinchLabel,
                          tr("Pinch"), -1, 10, 0.1, 1.0, 2, 11, 0);
  connect(addHandleSIPinchSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(ChangeScherkCollinsPinch(double)));

  QLabel *addHandleSIPinchCenterLabel = new QLabel(this);
  addHandleSIPinchCenterSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIPinchCenterLabel,
                          tr("Pinch Center"), 0.1, 1, 0.1, 0.5, 3, 12, 0);
  connect(addHandleSIPinchCenterSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(ChangeScherkCollinsPinchCenter(double)));

  QLabel *addHandleSIPinchWidthLabel = new QLabel(this);
  addHandleSIPinchWidthSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIPinchWidthLabel,
                          tr("Pinch Width"), 0.001, 10, 0.1, 1.0, 3, 13, 0);
  connect(addHandleSIPinchWidthSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(ChangeScherkCollinsPinchWidth(double)));

  QLabel *addHandleSIHoleNumSegmentsLabel = new QLabel(this);
  addHandleSIHoleNumSegmentsSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIHoleNumSegmentsLabel,
                          tr("#segs in holes"), 1, 10, 1, 2.0, 0, 14, 0);
  connect(addHandleSIHoleNumSegmentsSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(ChangeScherkCollinsHoleNumSegments(double)));

  QLabel *addHandleSIHoleInitSkipLabel = new QLabel(this);
  addHandleSIHoleInitSkipSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIHoleInitSkipLabel,
                          tr("Init skip"), 0, 10, 1, 1.0, 0, 15, 0);
  connect(addHandleSIHoleInitSkipSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(ChangeScherkCollinsHoleInitSkip(double)));

  QLabel *addHandleSIHoleSkipLabel = new QLabel(this);
  addHandleSIHoleSkipSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIHoleSkipLabel,
                          tr("Skip"), 0, 10, 1, 0.0, 0, 16, 0);
  connect(addHandleSIHoleSkipSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(ChangeScherkCollinsHoleSkip(double)));

  QLabel *addHandleSIHoleTwistLabel = new QLabel(this);
  addHandleSIHoleTwistSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIHoleTwistLabel,
                          tr("Twist of holes"), 0, 10, 1, 1.0, 0, 17, 0);
  connect(addHandleSIHoleTwistSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(ChangeScherkCollinsHoleTwist(double)));

  QLabel *addHandleSIHoleSegSkipLabel = new QLabel(this);
  addHandleSIHoleSegSkipSpinBox =
      createDoubleSpinBox(mAddHandleSILayout, addHandleSIHoleSegSkipLabel,
                          tr("Skip in segments"), 0, 10, 1, 0.0, 0, 18, 0);
  connect(addHandleSIHoleSegSkipSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(ChangeScherkCollinsHoleSegSkip(double)));
  ...
  connect(normalTwistFactor_slider, SIGNAL(sliderMoved(int)), this,
          SLOT(changeNormalTwistingFactor(int)));
...
void HighgenusMode::setupRindModelingScaling() {
  ...
  rindModelingScalingSpinBox =
      createDoubleSpinBox(mRindModelingScalingLayout, rindModelingScalingLabel,
                          tr("Scale"), 0, 10.0, 0.01, 0.9, 3, 0, 0);
  connect(rindModelingScalingSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(changeCrustScaleFactor(double)));
  // symmetric weights checkbox
  rindModelingScalingCleanupCheckBox =
      new QCheckBox(tr("Cleanup when peeling"), this);
  rindModelingScalingCleanupCheckBox->setChecked(Qt::Unchecked);
  connect(rindModelingScalingCleanupCheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(toggleCrustCleanupFlag(int)));
  mRindModelingScalingLayout->addWidget(rindModelingScalingCleanupCheckBox, 1,
                                        0, 1, 2);
  // create crust button
  rindModelingScalingCreateCrustButton =
      new QPushButton(tr("Create Crust"), this);
  connect(
      rindModelingScalingCreateCrustButton, SIGNAL(clicked()),
      ((MainWindow *)mParent),
      SLOT(crustModeling4())); // ((MainWindow*)mParent),
                               // SLOT(crust_modeling1())); //crust_modeling3
  mRindModelingScalingLayout->addWidget(rindModelingScalingCreateCrustButton, 2,
                                        0, 1, 2);
...
void HighgenusMode::setupRindModelingThickness() {
  ...
  rindModelingThicknessSpinBox = createDoubleSpinBox(
      mRindModelingThicknessLayout, rindModelingThicknessLabel,
      tr("Thickness:"), -1.0, 1.0, 0.01, 0.5, 3, 0, 0);
  connect(rindModelingThicknessSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent), SLOT(changeCrustThickness(double)));
  // symmetric weights checkbox
  rindModelingThicknessCleanupCheckBox =
      new QCheckBox(tr("Cleanup when peeling"), this);
  rindModelingThicknessCleanupCheckBox->setChecked(Qt::Unchecked);
  connect(rindModelingThicknessCleanupCheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(toggleCrustCleanupFlag(int)));
  ...
  mRindModelingThicknessLayout->addWidget(rindModelingThicknessCreateButton, 2,
                                          0, 1, 2);
  // set layout and add stretch
...
void HighgenusMode::setupWireframeModeling() {
  ...
  wireframeModelingThicknessSpinBox = createDoubleSpinBox(
      mWireframeModelingLayout, wireframeModelingThicknessLabel,
  ...
  connect(wireframeSplitCheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(toggleWireframeSplit(int)));
  ...
  mWireframeModelingLayout->addWidget(wireframeModelingCreateButton, 2, 0, 1,
                                      2);
  // set layout and add stretch
...
void HighgenusMode::setupWireframeModeling2() {
  ...
  wireframeModeling2ThicknessSpinBox = createDoubleSpinBox(
      mWireframeModeling2Layout, wireframeModeling2ThicknessLabel,
      tr("Thickness:"), 0.0, 1.0, 0.01, 0.25, 3, 0, 0);
  ...
  connect(wireframeModeling2WidthSpinBox, SIGNAL(valueChanged(double)),
...
void HighgenusMode::setupColumnModeling() {
  ...
  columnModelingThicknessSpinBox =
      createDoubleSpinBox(mColumnModelingLayout, columnModelingThicknessLabel,
                          tr("Thickness:"), 0.0, 1.0, 0.01, 0.25, 3, 0, 0);
  ...
  columnModelingNumSegmentsSpinBox =
      createDoubleSpinBox(mColumnModelingLayout, columnModelingNumSegmentsLabel,
                          tr("# Segments:"), 4.0, 100, 1, 4, 0, 1, 0);
  connect(columnModelingNumSegmentsSpinBox, SIGNAL(valueChanged(double)),
...
void HighgenusMode::setupSierpinsky() {
  ...
  connect(sierpinskyButton, SIGNAL(clicked()), ((MainWindow *)mParent),
...
void HighgenusMode::setupMultiFaceHandle() {
  ...
  connect(multiFaceAlgorithmGroupBox, SIGNAL(toggled(bool)), this,
          SLOT(changeMultiFaceAlgorithm(bool)));
  ...
  multiFaceHandleScaleLabel = new QLabel(this);
  multiFaceHandleScaleSpinBox =
      createDoubleSpinBox(mMultiFaceAlgorithmLayout, multiFaceHandleScaleLabel,
                          tr("Scale Factor:"), 0.0, 5.0, 0.01, 1.0, 3, 0, 0);
  connect(multiFaceHandleScaleSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent),
          SLOT(changeMultiFaceHandleScaleFactor(double)));
  ...
  mMultiFaceAlgorithmLayout->addWidget(multiFaceHandleExtrudeDistanceLabel, 1,
                                       0);
  mMultiFaceAlgorithmLayout->addWidget(multiFaceHandleExtrudeDistanceSpinBox, 1,
                                       1);
  multiFaceHandleExtrudeDistanceLabel = new QLabel(this);
  multiFaceHandleExtrudeDistanceSpinBox = createDoubleSpinBox(
      mMultiFaceAlgorithmLayout, multiFaceHandleExtrudeDistanceLabel,
      tr("Extrude Dist.\nFactor:"), -2.0, 2.0, 0.1, 0.5, 3, 1, 0);
  connect(multiFaceHandleExtrudeDistanceSpinBox, SIGNAL(valueChanged(double)),
          ((MainWindow *)mParent),
          SLOT(changeMultiFaceHandleExtrudeDist(double)));

  // use max. offsets
  multiFaceHandleMaxOffsetsCheckBox =
      new QCheckBox(tr("Use max offsets"), this);
  connect(multiFaceHandleMaxOffsetsCheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(toggleMultiFaceHandleUseMaxOffsetFlag(int)));
  mMultiFaceAlgorithmLayout->addWidget(multiFaceHandleMaxOffsetsCheckBox, 2, 0,
                                       1, 2);
  ...
  connect(multiFaceHandleButton, SIGNAL(clicked()), ((MainWindow *)mParent),
          SLOT(createMultiFaceHandle()));
...
void HighgenusMode::changeMultiFaceAlgorithm(bool on) {
  ...
  if (!on) {
    ...
    // check if the checkbox is checked first or not, then enable widgets
    // accordingly
